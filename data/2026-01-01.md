...
TL;DR: 提出基于材料的混合专家模型RUL-QMoE，用于锂离子电池剩余寿命预测，专门处理异质电池化学的复杂性，实现不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 现有RUL预测模型通常忽略阴极材料和设计的影响，导致模型缺乏泛化性。电池材料规格可能不易获得，且电池可能包含多种材料混合，需要解决这些挑战。

Method: 提出RUL-QMoE模型，将混合专家集成到概率框架中，包含五个电池类型的专家网络（LFP、NCA、NMC、LCO、NMC-LCO），通过门控机制动态分配相关性，利用非交叉分位数回归产生可解释的预测分布。

Result: RUL-QMoE模型能够处理电池操作条件，实现不确定性量化，产生相干且可解释的电池RUL预测分布，提高预测的鲁棒性。

Conclusion: 提出的基于材料的混合专家方法有效解决了异质电池化学的RUL预测挑战，通过概率框架和不确定性量化提高了预测的可靠性和泛化能力。
...
TL;DR: 提出一种用于大规模MIMO非地面网络的分布式波束成形框架，采用基于熵的多智能体深度强化学习方法，结合傅里叶神经算子和低秩分解技术，在非完美CSI下最大化下行链路和速率。


<details>
  <summary>Details</summary>
Motivation: 针对大规模MIMO非地面网络中，在非完美信道状态信息条件下实现分布式波束成形并最大化下行链路和速率的挑战。传统方法如WMMSE、ZF、MRT等在可扩展性、鲁棒性和计算效率方面存在不足。

Method: 提出基于熵的多智能体深度强化学习框架，每个非地面基站使用傅里叶神经算子独立计算波束成形向量，以捕捉频域中的长程依赖关系。结合基于共轭先验机制的迁移学习和低秩分解技术，确保可扩展性和鲁棒性。

Result: 仿真结果表明，该方法在平均和速率、对CSI非完美的鲁棒性、用户移动性、网络规模和用户密度变化下的可扩展性方面优于WMMSE、ZF、MRT、CNN-based DRL和DDPG等基准方法。同时相比CNN-based和WMMSE方法显著提高计算效率，相比共享critic DRL方法减少通信开销。

Conclusion: 提出的分布式波束成形框架通过结合傅里叶神经算子、熵多智能体强化学习和低秩分解技术，有效解决了大规模MIMO非地面网络中的波束成形优化问题，在性能、鲁棒性、可扩展性和计算效率方面均表现出优越性。
...
TL;DR: 提出基于多模态Transformer的InSAR位移图单步预测模型，用于EGMS时间序列的下一纪元位移预报


<details>
  <summary>Details</summary>
Motivation: 近实时区域尺度地面形变监测对城市规划、关键基础设施管理和自然灾害缓解至关重要。虽然InSAR和EGMS提供了密集的历史运动观测，但由于长期趋势、季节周期和突发不连续性的叠加以及空间异质性，预测下一观测仍具挑战性。

Method: 提出多模态基于块的Transformer模型，用于EGMS时间序列的单步固定间隔下一纪元位移图预测。模型输入包括近期位移快照、静态运动学指标（平均速度、加速度、季节振幅）以及谐波年日编码。静态指标仅从训练窗口计算以避免数据泄露。

Result: 在爱尔兰东部图块(E32N34)上，多模态Transformer明显优于CNN-LSTM、CNN-LSTM+Attn和多模态STGCN，在测试集上达到RMSE = 0.90 mm和R² = 0.97的最佳阈值精度。

Conclusion: 多模态Transformer模型能够有效预测EGMS位移时间序列的下一纪元位移图，为近实时地面形变监测提供了有前景的解决方案。
...
TL;DR: 提出一种基于可移动天线(MA)的多区域波束覆盖增强方法，通过联合优化发射波束成形和天线位置向量，最大化目标子区域的最小波束增益，相比固定天线性能显著提升且复杂度低。
...
Method: 将多区域覆盖问题类比为经典多陷波滤波器设计，构建基于空间多陷波滤波器的发射波束成形向量。提出顺序更新算法选择最优MA位置子集，并结合Gibbs采样避免局部最优。

Result: 数值结果表明，所提算法相比传统固定位置天线性能显著提升，且能达到与交替优化算法相当的性能，但复杂度大幅降低。

Conclusion: 可移动天线技术通过天线位置优化能有效增强多区域波束覆盖性能，所提算法在性能和复杂度之间取得了良好平衡，为MA系统设计提供了有效解决方案。
...
TL;DR: 本文提出了系统算法寻找N>10的最优RMRAs，并通过闭式表达式构建了近/次优RMRA家族，扩展了已知最优RMRA目录。


<details>
  <summary>Details</summary>
Motivation: 现代稀疏阵列虽然经济，但单个传感器故障就会失效。现有的TFRSAs和RMRAs虽然具有鲁棒性，但最优RMRA配置仅知N=6到10。需要开发N>10的最优RMRA算法和闭式表达式描述的近/次优RMRA。

Method: 使用MATLAB实现穷举搜索技术解决组合优化问题寻找RMRAs；通过模式挖掘和代数泛化，从穷举搜索获得的阵列中推导出新TFRSA的闭式表达式。

Result: 成功找到N=11到14的最优RMRAs，确定了N=15到20的近/次优阵列；获得了大量有效近/次优RMRAs目录；推导出传感器位置、可用孔径和自由度DOFs的闭式表达式，经验证对N≥8有效。

Conclusion: 本文的创新点有两方面：扩展了已知最优RMRA目录，并构建了遵循闭式表达式的次优RMRA，为大规模稀疏阵列设计提供了系统化解决方案。
...
TL;DR: 该论文比较了标量和矢量磁力计网络在海上目标跟踪中的性能，发现矢量网络在跟踪精度和鲁棒性方面显著优于标量网络。


<details>
  <summary>Details</summary>
Motivation: 海上环境的磁监测对于航运监控优化和国家安全至关重要。新型紧凑光纤耦合量子磁力计的发展为构建传感器网络提供了新机会，需要评估最佳网络架构。

Method: 采用无迹卡尔曼滤波器方法进行目标跟踪，比较标量与矢量磁力计网络架构的性能差异。

Result: 矢量网络在目标跟踪方面表现显著优于标量网络，特别是在跟踪精度和鲁棒性方面有明显提升。

Conclusion: 矢量磁力计网络架构在海上目标识别和跟踪应用中具有明显优势，为构建更有效的磁监测系统提供了重要指导。
...
<details>
  <summary>Details</summary>
Motivation: 巨型星座部署推动空间数据中心发展，但卫星间带宽和能量受限的链路条件动态变化，使得分布式联邦学习的频繁聚合面临挑战。传统相干空中计算需要严格的相位对齐，在空间环境中难以维持。

Method: 提出OptiVote框架，集成符号SGD与多数投票聚合原则和脉冲位置调制。卫星通过激活正交PPM时隙传输本地梯度符号，聚合节点通过非相干能量累积进行多数投票检测。还开发了重要性感知、无需CSI的动态功率控制方案来缓解异构FSO信道引起的聚合偏差。

Result: 通过理论分析，在统计FSO信道下表征了聚合错误概率，并为非凸目标建立了收敛保证。该方法将相位敏感的场叠加转换为相位无关的光强度组合，消除了精确相位同步的需求。

Conclusion: OptiVote为空间数据中心的联邦学习提供了一个鲁棒且通信高效的非相干自由空间光空中计算框架，能够在动态损伤下提高弹性，无需额外信令即可平衡接收能量。
...
TL;DR: 提出一种动态频谱整形框架，使用预优化脉冲集通过简单变换调整OFDM信号的频谱形状，无需解决NP-hard优化问题


<details>
  <summary>Details</summary>
Motivation: 传统矩形脉冲OFDM信号频谱限制性差，在认知无线电环境中需要动态适应非连续、时变的可用频谱

Method: 结合主动干扰消除(AIC)和自适应符号转换(AST)的预优化脉冲集，通过简单变换适应频谱位置和宽度变化，对接收端透明

Result: 实现了动态频谱整形，通信设备可在线适应传输频带变化，避免了现有方法需要解决NP-hard优化问题的复杂性

Conclusion: 该框架为OFDM信号提供了一种高效、动态的频谱整形解决方案，特别适用于认知无线电环境中的频谱共享需求
...
TL;DR: Wigner-Ville变换作为信息测量工具，在噪声背景下检测和定位信息信号方面具有显著优势，比能量测量方法提供超过15dB的灵敏度优势，无需大量训练。


<details>
  <summary>Details</summary>
Motivation: 传统信号处理方法在检测噪声和杂乱背景中的信息信号时存在局限性，特别是在资源受限（时间/空间集成压力、瞬态/微弱信号）的应用场景中，需要更有效的检测和测量工具。

Method: 将Wigner-Ville变换重新解释为信息测量工具，结合Tsallis熵和信息相关泛函，用于检测和定位信息信号，并测量其信息体积。在射频通信等场景中应用该方法。

Result: Wigner-Ville基检测方法相比能量基测量方法提供显著灵敏度优势，在某些场景下超过15dB的优势，且无需大量训练程序。在资源受限的应用中，该方法可将传感效率提高多个数量级。

Conclusion: Wigner-Ville变换作为信息测量工具具有重要应用价值，特别是在资源受限的传感场景中，能够显著提高检测性能，为多个应用领域带来潜在进展。
...
TL;DR: 本文研究多可移动天线多用户系统中的天线定位与波束成形设计，在概率视距阻塞约束下最小化功率，为单天线系统提供全局最优解，为多天线系统开发高效算法。


<details>
  <summary>Details</summary>
Motivation: 可移动天线具有灵活调整天线位置的优点，能够缓解无线网络中的大规模衰减问题，但需要解决天线定位和波束成形的联合优化设计。

Method: 针对单可移动天线系统，证明了简化问题的凸性并获得了全局最优解；针对多可移动天线系统，推导了闭式波束成形结构，并开发了高效的一阶算法以获得高质量局部解。

Result: 数值结果验证了所提设计的有效性，并表明可移动天线系统相比传统固定天线系统在节能方面具有显著性能优势。

Conclusion: 可移动天线系统通过优化天线定位和波束成形设计，能够在满足用户信噪比约束的同时显著降低功耗，为缓解无线网络大规模衰减提供了有效解决方案。
...
TL;DR: 提出BC-TAS框架，通过多天线选择策略解决混合无线传感器网络中高吞吐量通信与无源背向散射传感器低功耗需求之间的冲突，实现双包络调节和干扰抑制。


<details>
  <summary>Details</summary>
Motivation: 解决混合无线传感器网络中高吞吐量主通信与无源背向散射传感器严格功率要求之间的物理层冲突，需要在保证通信可靠性的同时为无源传感器提供稳定的射频能量。

Method: 提出BC-TAS框架，采用基于子载波的天线选择策略，使用多目标成本函数联合最大化链路可靠性、稳定无源SAW传感器的入射射频能量包络、抑制对共存接收器的干扰，并利用MD-OFDM的稀疏性实现双包络调节。

Result: 在IEEE 802.11be色散信道模型和非线性Rapp功率放大器下，BC-TAS相比传统MU-MIMO基线实现了数量级的中断概率改进和显著的能效提升，同时在降低功率放大器回退下确保频谱掩模合规。
...
TL;DR: 本文研究OTFS双功能雷达通信系统的最优信号设计，提出联合优化导频符号设计和数据符号功率分配的方法，在延迟多普勒域统一感知与通信。


<details>
  <summary>Details</summary>
Motivation: OTFS信号在高移动性无线环境中很有前景，但较长的帧持续时间在雷达和通信分离执行时会引入显著延迟。通过双功能雷达通信模式，OTFS系统可以同时执行感知和数据传输，从而减少延迟。然而，最优的OTFS DFRC信号策略尚未充分探索。

Method: 推导考虑信道估计误差的通信信道容量下界指标；推导包含数据符号随机性和确定性导频符号的感知综合旁瓣电平指标；基于这些指标构建平衡雷达和通信性能的优化问题，采用交替优化框架求解。

Result: OTFS DFRC强制使用在延迟多普勒域具有集中峰值的确定性导频信号，为感知和信道估计提供共同结构，数据在其他DD网格复用。相比传统OTFS信号，提出的OTFS DFRC信号扩展了可实现的感知-通信性能区域，至少提供9.45 dB的ISL抑制（感知）和4.82 dB的SINR增益（通信）。

Conclusion: 本文提出的OTFS DFRC信号设计通过联合优化导频和数据符号，在延迟多普勒域统一了感知和通信功能，显著提升了系统性能，为高移动性环境下的双功能系统提供了有效解决方案。
...
TL;DR: 提出一种用于宽带集成感知通信的层次化感知框架，利用OFDM系统的波束斜视效应实现高效二维角度估计


<details>
  <summary>Details</summary>
Motivation: 宽带OFDM系统中的波束斜视效应为角度估计提供了新机会，但传统方法效率较低，需要更高效的感知框架来同时满足感知和通信需求

Method: 采用层次化感知框架：先搜索俯仰角，再基于估计的俯仰角搜索方位角；利用真时延线和移相器联合配置覆盖多个网格点；将角度估计建模为稀疏信号恢复问题，开发改进的匹配追踪算法；设计功率分配策略最小化总发射功率

Result: 数值结果表明，所提框架相比传统感知方法具有更优性能，同时减少了感知功率消耗

Conclusion: 提出的层次化感知框架有效利用波束斜视效应，实现了高效的二维角度估计，在降低功率消耗的同时提升了感知性能，为宽带集成感知通信系统提供了实用解决方案
...
TL;DR: 提出一种基于二进制补码的数字AirComp编码方案，通过多子载波传输和LMMSE检测器实现渐近无误差计算，在低信噪比下优于现有数字AirComp方法


<details>
  <summary>Details</summary>
Motivation: 克服模拟信号在无线计算（AirComp）中的固有局限性，开发兼容数字调制的数字AirComp实现方案

Method: 1. 使用二进制补码将量化离散值编码为二进制序列，通过多子载波传输
2. 设计解码器构建叠加数字调制信号与计算目标之间的函数映射
3. 采用截断反转策略预处理以减轻信道衰落影响
4. 推导闭式最优LMMSE检测器并提出低复杂度截断选择算法
5. 基于编码输出重要性差异实施跨子载波非均匀功率分配

Result: 数值结果表明该方案优于现有数字AirComp方法，特别是在低信噪比区域表现优异

Conclusion: 提出的二进制补码编码方案为数字AirComp提供了有效的实现方法，通过优化的检测器和功率分配策略显著提高了计算精度
...
TL;DR: 本文研究无线供能的集成感知与通信系统，通过联合优化无线能量传输时长、ISAC传输时间分配和发射功率，在满足目标定位精度约束下最大化最小吞吐量。


<details>
  <summary>Details</summary>
Motivation: 无线供能ISAC系统面临能量供应、通信吞吐量和感知精度之间的基本权衡。现有研究需要解决如何在满足目标定位精度要求的同时，优化系统性能的问题。

Method: 提出一种基于SCA的迭代算法：首先通过变量替换和对数函数单调性对非凸问题进行重构，然后采用逐次凸逼近方法求解联合优化问题。

Result: 仿真结果表明算法收敛性好，相比基准方案获得显著性能增益，验证了协调时间-功率优化在平衡感知精度和通信性能方面的重要性。

Conclusion: 无线供能ISAC系统需要联合优化WPT时长、ISAC传输时间分配和发射功率，所提SCA算法能有效解决这一非凸优化问题，实现感知精度和通信性能的良好平衡。
...
TL;DR: 提出基于5G ISAC的免接触人体姿态识别系统，通过上行SRS信号实现2D姿态估计，在室内环境中显著优于现有主流方案。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉或RF的HPR技术存在隐私问题、遮挡敏感、专用设备、分辨率有限等挑战，需要更自然的免控制器交互方法。

Method: 利用5G ISAC技术，从上行SRS信号中提取多域特征，通过编码器在潜在空间统一对齐表示，融合低维特征输出人体姿态状态。

Result: 实验表明，在典型室内环境中，该系统在HPR性能上显著优于当前主流基线解决方案。

Conclusion: 基于5G ISAC的HPR系统为通用人机交互提供了坚实的技术基础，解决了现有方案的多种挑战。
...
TL;DR: 该论文为6G极大规模天线阵列近场感知理论的第一部分，推导了目标位置、速度和RCS联合估计的克拉美-罗下界及其近似表达式，揭示了阵列孔径、目标距离等参数对估计性能的影响。


<details>
  <summary>Details</summary>
Motivation: 6G极大规模天线阵列工作在极高频率时会进入近场区域，传统远场模型失效，需要建立近场感知的理论基础来理解其基本估计极限。

Method: 建立统一的窄带近场信号模型，利用Slepian-Bangs公式推导目标位置、速度和RCS联合估计的闭式克拉美-罗下界，并建立可解释的远场和近场近似表达式。

Result: 推导出闭式CRB表达式及其近似形式，揭示了估计性能如何随阵列孔径、目标距离、载波波长和相干积分长度变化，明确了菲涅尔修正项的重要性。

Conclusion: 该研究为近场感知系统建立了理论基础，提供了阵列设计和算法设计的指导原则，仿真结果验证了理论推导的准确性。
...
<details>
  <summary>Details</summary>
Motivation: 6G系统中极大规模天线阵列的宽带近场传感需要在延迟、多普勒和空间效应紧密耦合的宽带环境下工作，需要建立统一的估计理论框架。

Method: 建立精确的宽带近场信号模型，捕捉频率相关传播、球面波几何以及目标和运动参数在子载波和慢时间上的内在耦合，使用Slepian-Bangs公式推导Fisher信息矩阵和CRB。

Result: 推导了宽带Fisher信息矩阵和CRB，展示了宽带信息如何在正交子载波上聚合，建立了可处理的远场和近场近似，提供了带宽、相干积分长度和阵列孔径作用的见解。

Conclusion: 仿真结果验证了推导的CRB及其近似，与理论缩放定律高度一致，为宽带近场传感系统设计提供了理论基础和设计指导。
...
<details>
  <summary>Details</summary>
Motivation: 现有相关码的研究主要基于仿真，缺乏代数分析方法。本文旨在通过代数构造方法，为分层准循环码提供新的参数界限和理论分析框架。

Method: 使用Kautz和Singleton（1964）的叠加码构造，从Reed-Solomon码出发代数构造分层准循环码。分析层次数、索引与域大小的关系，并扩展到多项式求值码类。

Result: 证明了层次数和索引由域大小决定；给出了显式码参数和性质；提供了秩和距离等参数的界限；k=2时得到围长为6的Tanner图；部分码达到已知最佳最小距离。

Conclusion: 首次代数构造分层准循环码，建立了代数分析方法，为相关码的参数分析提供了新框架，部分码达到最优性能，具有理论和实际意义。
...
TL;DR: 提出基于切比雪夫多项式展开的框架，从信道协方差中恢复连续角功率谱，将病态反演问题转化为有限维线性回归，并通过半定规划和导数正则化实现平滑非负APS重建


<details>
  <summary>Details</summary>
Motivation: 传统角功率谱反演是病态问题，需要有效方法从信道协方差中恢复连续角功率谱，特别是在FDD系统中实现上下行协方差预测

Method: 利用切比雪夫多项式在变换域的正交性，推导协方差的精确级数表示，将APS反演转化为有限维线性回归问题，引入非负APS的半定表征和导数正则化器

Result: 仿真结果显示该框架能准确重建APS，在FDD设置中实现可靠的下行协方差预测，近似误差随角度平滑度增加而快速衰减

Conclusion: 在切比雪夫域中联合利用平滑性和非负性，为多天线系统中的协方差域处理提供了有效工具
...
TL;DR: 本文提出了一种与物理信道解耦的随机复用技术，适用于任意范数有界且谱收敛的信道矩阵，通过构建随机变换域中的等效输入各向同性信道矩阵，实现统计衰落信道遍历性，并保证AMP类检测器在特定条件下的渐近最优性。
...
Method: 提出随机复用技术，构建随机变换域中的等效输入各向同性信道矩阵，实现统计衰落信道遍历性。采用低复杂度跨域记忆AMP（CD-MAMP）检测器，利用时域信道稀疏性和等效信道随机性。推导最优功率分配以最小化BER和最大化约束容量。

Result: 随机复用技术保证AMP类检测器对任意范数有界、谱收敛信道矩阵和信号配置的渐近最优性（在唯一固定点假设下）。CD-MAMP检测器在随机复用系统中具有最优编码原理和约束容量最优性。该技术在各种无线应用中展现出良好的通用性。

Conclusion: 随机复用技术提供了一种与物理信道解耦的通用框架，适用于动态现实环境中的任意信道矩阵，通过统计方法实现可靠通信，为高移动性无线通信系统提供了新的解决方案。
...
TL;DR: 提出太赫兹无线数据中心（THz-WDC）愿景，利用太赫兹频段实现超高带宽、低延迟、高能效的短中距离（1-100m）数据中心内部连接，以应对AI工作负载对互连架构的挑战。
...
Method: 提出THz-WDC架构，探索关键技术：数字孪生编排、低复杂度波束操纵技术、全硅太赫兹收发器、低复杂度模拟基带架构，并进行数值分析比较太赫兹与光/铜互连的性能。

Result: 太赫兹无线链路可实现单链路1Tbps、通过空间复用达10Tbps总吞吐量、单跳延迟低于50ns、20米距离内低于10pJ/bit的能效，在特定距离和吞吐量领域超越传统有线解决方案。

Conclusion: 太赫兹无线连接为未来量子化和chiplet模块化架构的数据中心提供灵活互连机制，是实现无线定义、可重构、可持续AI数据中心的路线图。
...
TL;DR: 本文提出了针对扭曲广义里德-所罗门码和Roth-Lempel码的高效列表解码和唯一解码算法，显著改进了解码时间复杂度和支持更多扭曲数量。


<details>
  <summary>Details</summary>
Motivation: MDS码在实际应用中非常重要，但大多数已知MDS码都是广义里德-所罗门码，非GRS码的研究相对较少。研究非GRS码既有理论意义，也有实际价值，因为GRS码的强代数结构在密码学中可能不受欢迎。扭曲广义里德-所罗门码和Roth-Lempel码是两种重要的非GRS码，但它们的解码问题研究相对不足。

Method: 基于Guruswami-Sudan算法，为TGRS码和Roth-Lempel码设计列表解码和唯一解码算法。在合适的参数条件下，算法达到接近线性的时间复杂度。对于TGRS码，支持最多O(n^2)个扭曲；对于Roth-Lempel码，提供了首个高效解码器。同时将代数操作检测码整合到列表解码框架中。

Result: 解码算法的时间复杂度从之前的二次改进到接近线性。TGRS解码器支持最多O(n^2)个扭曲，显著扩展了之前仅处理单扭曲的情况。为Roth-Lempel码提供了首个高效解码器。列表解码器在广泛参数范围内超越了经典唯一解码半径。

Conclusion: 本文为非GRS MDS码的解码提供了高效算法，填补了TGRS码和Roth-Lempel码解码研究的空白，显著改进了解码性能并扩展了应用范围，同时通过整合AMD码增强了消息恢复的可靠性。
...
TL;DR: 研究有限域上随机空间耦合LDPC码，分析其最小距离和迭代译码阈值饱和特性


<details>
  <summary>Details</summary>
Motivation: 研究有限域上空间耦合LDPC码的性能，特别是最小距离和迭代译码阈值特性，比较不同变量节点边扩展规则对性能的影响

Method: 定义标准耦合和增强耦合两种随机Tanner图结构，使用独立均匀随机单项式映射；建立对称概率测度分析框架，研究q元输入对称信道的消息分布特性

Result: 证明两种耦合结构都具有渐进良好的最小距离和最小停止集大小；增强耦合结构比标准耦合结构具有更好的距离性能；建立了通用阈值饱和结果

Conclusion: 有限域上空间耦合LDPC码在q元输入对称信道上具有阈值饱和特性，增强耦合结构能提供更好的距离性能
...
TL;DR: 研究确定性构造图，使得低秩矩阵在任意值下都能唯一补全，通过分析图模式与补全性的关系，构建无限图族实现精确稳定补全。


<details>
  <summary>Details</summary>
Motivation: 研究在什么图结构下，低秩矩阵补全问题能够保证唯一解，无论矩阵元素的具体数值如何。这对于矩阵补全的理论保证和实际应用具有重要意义。

Method: 分析图模式与补全性的关系，特别关注由双邻接矩阵支撑生成的格图子图中的特定模式（自回避行走的并集）。通过构造方法设计无限图族。

Result: 建立了图模式与矩阵补全性的理论联系，构造了无限图族，使得对于任意固定秩矩阵，都能通过平方和层次结构实现精确且稳定的补全。

Conclusion: 确定性图构造为低秩矩阵补全提供了理论保证，通过分析图结构模式可以设计出保证补全性的图族，为矩阵补全问题提供了新的理论框架和构造方法。
...
TL;DR: 本文提出了一种基于深度强化学习的无人机搭载可重构智能表面系统优化方法，用于解决三维无人机抖动和级联信道不确定性下的吞吐量最大化问题。


<details>
  <summary>Details</summary>
Motivation: 无人机搭载的可重构智能表面（RIS）可以按需重塑无线传播，但其性能对无人机抖动和级联信道不确定性非常敏感。现有方法在处理这些实际损伤时存在局限性，需要开发更鲁棒的优化框架。

Method: 提出了一种基于上下文多臂赌博机的模型无关深度强化学习框架，使用可微分可行性层将连续动作映射到可行解，奖励函数采用期望吞吐量的蒙特卡洛估计。具体实现了不使用目标网络的约束型DDPG和TD3算法。

Result: 在严重抖动和低信道状态信息质量下，所提算法比传统的交替优化加权最小均方误差基准方法获得更高吞吐量。在不同场景下，性能与基于样本平均近似的基准方法相当或略低（相对差距0-12%），但在线推理时间仅为0.6毫秒，远低于基准方法的370-550毫秒。

Conclusion: 所提出的深度强化学习方法能有效处理无人机抖动和信道不确定性，在保持良好性能的同时显著降低计算延迟，为无人机搭载RIS系统的实时优化提供了可行方案。
...
Method: 提出了两种改进方法：1）OA-GD-MAMP通过技术手段避免中间变量溢出；2）CR-GD-MAMP通过算法优化将每次迭代的矩阵向量乘积从3次减少到2次。

Result: OA-GD-MAMP有效解决了浮点数溢出问题，CR-GD-MAMP在几乎不影响收敛速度的情况下将计算复杂度降低了1/3。

Conclusion: 改进后的GD-MAMP算法在保持原有性能优势的同时，解决了数值稳定性和计算效率问题，使其更适合实际应用。
...
TL;DR: 提出一种与信道矩阵解耦的随机调制技术，适用于任意范数有界和谱收敛的信道矩阵，通过构建等效密集随机信道矩阵确保充分统计信道衰落，保证AMP检测器的渐近最优性，并设计低复杂度CD-MAMP检测器，推导最优功率分配方案，相比现有技术可获得2-3dB性能增益。


<details>
  <summary>Details</summary>
Motivation: 现有调制技术（如OFDM/OTFS/AFDM）通常与特定信道结构耦合，限制了其在任意信道矩阵下的应用。需要一种通用的调制方案，能够适应各种信道条件，同时保证检测性能的最优性。

Method: 提出随机调制技术，构建与信道矩阵解耦的等效密集随机信道矩阵；设计低复杂度跨域记忆近似消息传递（CD-MAMP）检测器，利用时域信道稀疏性和随机变换域信道随机性；推导最小化BER和最大化约束容量的最优功率分配方案。

Result: 随机调制在平均和优化功率分配下，相比现有OFDM/OTFS/AFDM结合5G-NR LDPC码的方案，可获得2-3dB的BER和BLER性能增益；CD-MAMP检测器实现低复杂度检测；功率分配方案有效提升系统性能。

Conclusion: 随机调制技术为任意范数有界和谱收敛信道矩阵提供通用解决方案，保证AMP检测器的渐近最优性，结合CD-MAMP检测器和最优功率分配，显著提升系统性能，具有实际应用价值。
