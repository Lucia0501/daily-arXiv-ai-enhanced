...
Motivation: 现有无线电环境映射模型在多发射源共存时性能下降，因为阴影效应和相邻发射源干扰的复合影响。需要准确的多发射源频谱地图来支持高效频谱共享和主次用户共存。

Method: 扩展单发射源的物理信息神经网络(PINN)方法，推导多发射源的接收信号强度指示(RSSI)偏微分方程(PDE)公式，提出ReVeal-MT模型，将多源PDE残差集成到神经网络损失函数中。

Result: 在ARA无线实验室的真实测量数据上验证，ReVeal-MT在多发射源场景下取得显著精度提升：在370平方公里区域仅用45个样本实现2.66dB的RMSE，同时保持低计算复杂度。

Conclusion: ReVeal-MT显著推进了现实多发射源条件下的无线电环境映射，具有实现细粒度频谱管理和精确主次用户共存的强大潜力。
...
TL;DR: 该论文提出了一种可重构的空间阵列架构，作为针对无线通信和感知任务中常见内核优化的通用核心，评估了其在效率方面接近专用核心的可行性。


<details>
  <summary>Details</summary>
Motivation: 大规模MIMO是下一代无线通信的基石，但未来系统需要同时具备可扩展性和频谱灵活性，以应对高频操作、宽带宽、共存、集成感知和动态干扰等新兴需求，这给底层处理硬件带来了效率和可重构性的压力。

Method: 提出了一种定制设计的空间阵列架构，作为针对一类常见无线内核优化的可重构通用核心。使用高层次综合（HLS）将该可重构设计与各内核的专用核心进行比较，并在32纳米工艺中综合评估延迟、吞吐量、面积和功耗。

Result: 研究结果确定了通用脉动阵列架构在何种条件下能够接近专用核心的效率，为构建更可扩展和灵活的无线系统铺平了道路。

Conclusion: 可重构的空间阵列架构能够在保持接近专用核心效率的同时提供灵活性，是实现未来可扩展、灵活无线通信系统的可行方案。
...
TL;DR: 本文研究了低空无线网络中用于集成感知与通信的夹持天线系统，通过联合优化天线位置和发射波束成形，提升无人机感知和通信性能。


<details>
  <summary>Details</summary>
Motivation: 在低空无线网络中，无人机在广阔范围内飞行时面临路径损耗问题，需要提升感知和通信能力。夹持天线系统通过自由部署天线位置来缓解路径损耗，但需要优化天线位置和波束成形以实现最佳性能。

Method: 采用分段波导夹持天线系统，首先提出基于交替优化的SWISAC-AO算法，推导最优发射波束成形结构。然后提出图神经网络SWISAC-GNN，从AO算法中获得启发，联合学习天线位置和发射波束成形。

Result: 数值结果表明，GNN在感知性能上与AO算法相当或更好，同时更好地满足通信需求。SWISAC-GNN具有更低的实现复杂度，支持实时部署。

Conclusion: 提出的SWISAC-GNN方法能够有效联合优化夹持天线位置和发射波束成形，在低复杂度下实现高性能的集成感知与通信，适用于低空无人机网络。
...
TL;DR: 提出了一种使用混合精度的快速傅里叶变换实现，采用逐块微缩放、全局2的幂次预缩放和预量化低比特旋转因子，在MRI数据集上评估了不同精度格式对图像质量的影响。


<details>
  <summary>Details</summary>
Motivation: 研究如何在保持计算效率的同时，通过混合精度和量化技术降低FFT计算的内存和计算需求，特别是在医学成像等应用中平衡数值精度与性能。

Method: 使用逐块微缩放（MX）技术、全局2的幂次预缩放和预量化的低比特旋转因子，在两种公开MRI数据集上评估前向和往返FFT保真度，分析不同低精度格式、图像大小和MX块大小对图像质量的影响。

Result: 结果显示在MX缩放下，尾数精度是主要限制因素；消融实验表明图像大小依赖性较弱，但块大小存在明显权衡——较大的块大小能带来更好的数值性能。

Conclusion: 混合精度FFT实现通过适当的缩放和量化策略可以在保持图像质量的同时显著降低计算精度要求，为高效FFT计算提供了可行的技术路径。
...
TL;DR: 该综述总结了基于RRAM的模拟矩阵计算在Massive MIMO无线通信中的应用进展，包括OFDM调制解调、MIMO检测与预编码、矩阵求逆等，并讨论了可靠性、精度、可扩展性等挑战及未来机遇。


<details>
  <summary>Details</summary>
Motivation: 随着6G时代对Massive MIMO信号处理的需求日益增长，传统数字计算面临能效瓶颈。基于RRAM的模拟矩阵计算提供了一种高效解决方案，但需要系统性地总结其在无线通信中的应用进展、机遇与挑战。

Method: 综述性研究方法，系统总结RRAM-based AMC在Massive MIMO中的多种应用：1) 使用MVM电路实现OFDM的DFT/IDFT计算；2) 基于MVM的迭代算法实现MIMO检测和预编码；3) 利用矩阵求逆和广义逆电路实现快速一步求解；4) 探索压缩感知恢复、特征值电路等新机会。

Result: 展示了RRAM-based AMC在Massive MIMO信号处理中的多样化应用潜力，包括调制解调、检测预编码、矩阵求逆等关键功能，为6G时代的高效信号处理提供了有前景的技术路径。

Conclusion: 通过器件-电路-算法协同设计，基于RRAM的模拟矩阵计算有望为6G时代的(超)大规模MIMO信号处理提供高效、高可靠性的解决方案，但需要克服器件可靠性、电路精度、阵列可扩展性和数据转换瓶颈等关键挑战。
...
TL;DR: 该论文提出将语义通信与智能体网络融合，构建面向6G的语义原生智能无线接入网，通过任务导向的语义交换和分布式智能体协作，超越传统比特级通信范式。


<details>
  <summary>Details</summary>
Motivation: 6G网络需要将智能作为原生能力，但现有AI驱动的O-RAN方案仍局限于比特中心和任务孤岛，缺乏语义理解和智能体自主性，无法满足沉浸式XR、车联网等新兴应用的需求。

Method: 提出三维分类法：语义抽象层次（符号/特征/意图/知识）、智能体自主与协调粒度（单/多/分层智能体）、RAN控制位置（PHY/MAC/近实时RIC/非实时RIC），并系统介绍任务导向语义编解码、多智能体强化学习、基础模型辅助RAN智能体等关键技术。

Result: 构建了语义-智能体融合的统一框架，分析了沉浸式XR、车联网V2X、工业数字孪生等6G典型用例，展示了语义智能体融合在实际场景中的应用潜力。

Conclusion: 语义通信与智能体网络的融合是6G AI-RAN的关键方向，但仍面临语义表示标准化、可扩展可信智能体协调、O-RAN互操作性、能效AI部署等开放挑战，需要进一步研究以实现可操作的语义智能体AI-RAN。
...
Result: 与未修改的SC IR-HARQ方案相比，在码长2048的极码中实现了72%的节点遍历减少，仿真结果确认特殊节点修改不会导致FER性能下降

Conclusion: 提出的特殊节点修改方法有效支持了极码IR-HARQ与SC解码的结合，为下一代无线通信系统的高吞吐量应用提供了可行的解决方案
...
### [8] [Pinching-Antenna System Design under Random LoS and NLoS Channels](https://arxiv.org/abs/2512.04719)
*Yanqing Xu,Yang Lu,Zhiguo Ding,Tsung-Hui Chang*

Main category: eess.SP

TL;DR: 本文研究了多用户pinching天线系统在复合概率信道模型下的优化设计，提出了基于平均SNR和中断约束的两种优化问题，并开发了低复杂度二分算法实现全局最优解。


<details>
  <summary>Details</summary>
Motivation: 现有pinching天线研究多关注理想视距环境，忽略了实际无线传播的随机特性。本文旨在研究更实用的多用户pinching天线系统，考虑包含距离依赖的视距阻塞和非视距散射的复合概率信道模型。

Method: 采用复合概率信道模型，考虑两种设计指标：表征长期吞吐和公平性的平均SNR指标，以及确保规定可靠性水平的中断约束指标。基于这些指标，提出了两个优化问题：最大化用户间最大最小平均SNR，以及在用户中断约束下最大化保证SNR阈值。虽然问题非凸，但利用其单调结构开发了低复杂度二分算法，通过简单标量评估实现全局最优解。

Result: 广泛的仿真验证了所提方法的有效性，表明即使在随机视距和非视距信道下，pinching天线系统也显著优于传统的固定天线设计。

Conclusion: 本文为多用户pinching天线系统在随机无线环境下的优化设计提供了实用解决方案，所提出的低复杂度算法能够实现全局最优性能，为未来灵活天线技术的实际部署提供了理论基础。
...
### [9] [Nonlinear EM-based Signal Processing](https://arxiv.org/abs/2512.04595)
*Mattia Fabiani,Giulia Torcolacci,Davide Dardari*

Main category: eess.SP

TL;DR: 论文提出利用可重构无源线性和非线性散射元件在信号源反应近场中的相互作用，实现直接在电磁域进行信号处理，以减少硬件复杂度和RF链数量。


<details>
  <summary>Details</summary>
Motivation: 高频段和超大天线阵列（XL-MIMO）使当前技术在硬件复杂度、延迟和功耗方面达到极限，需要寻找可扩展且可持续的解决方案。

Method: 研究利用可重构无源线性和非线性散射元件在信号源反应近场中的相互作用，实现直接在电磁域进行线性和非线性信号处理的新型架构。

Result: 数值结果表明，该方法能显著降低系统复杂度和RF链数量，同时在到达方向估计和位置估计等应用中达到关键性能指标，无需额外的模拟或数字处理。

Conclusion: 将部分信号处理直接转移到电磁域是实现可扩展和可持续无线系统的有前景方法，能有效解决高频段XL-MIMO系统的硬件挑战。
...
### [10] [Beampattern Synthesis for Discrete Phase RIS in Communication and Sensing Systems](https://arxiv.org/abs/2512.04881)
*Xiao Cai,Hei Victor Cheng,Daniel E. Lucani*

Main category: eess.SP

TL;DR: 提出一种针对RIS离散相位约束的阵列模式合成方法，通过惩罚函数和MM算法生成宽波束，显著提升方向估计性能


<details>
  <summary>Details</summary>
Motivation: 现有RIS研究主要针对已知目标方向的被动波束赋形优化，但实际应用中目标方向未知。需要开发宽波束合成技术以最大化整个目标区域的接收功率，但RIS的离散相位约束带来了独特挑战

Method: 提出针对RIS离散相位约束的阵列模式合成方法：1) 引入惩罚函数将约束推向凸包边界；2) 使用Minorization-Maximization (MM)方法将问题重构为凸优化问题

Result: 1) 算法生成的宽波束模式与幅度相位可调的全功率约束方法相当；2) 相比传统波束扫描技术，在低到中SNR下AOA的MSE降低数个数量级；3) 达到高检测概率所需的SNR降低8dB

Conclusion: 提出的方法有效解决了RIS离散相位约束下的宽波束合成问题，显著提升了方向估计性能，为RIS在目标检测和方向估计中的应用提供了有效解决方案
...
### [11] [CIG-MAE: Cross-Modal Information-Guided Masked Autoencoder for Self-Supervised WiFi Sensing](https://arxiv.org/abs/2512.04723)
*Gang Liu,Yanling Hao,Yixuan Zou*

Main category: eess.SP

TL;DR: CIG-MAE：一种用于WiFi CSI动作识别的跨模态信息引导掩码自编码器，通过自适应掩码策略和Barlow Twins正则化，在无监督学习中超越监督基线


<details>
  <summary>Details</summary>
Motivation: WiFi CSI动作识别具有设备无关性和隐私保护优势，但现有监督方法受限于标注成本高和数据规模小。现有自监督对比学习方法的数据增强与无线电信号物理语义冲突，且需要大批量训练，不适合CSI数据。

Method: 提出CIG-MAE（跨模态信息引导掩码自编码器）：1）对称双流架构重建CSI的幅度和相位；2）自适应信息引导掩码策略，动态关注高信息密度时频区域；3）引入Barlow Twins正则化对齐跨模态表示，无需负样本。

Result: 在三个公开数据集上，CIG-MAE一致超越现有SOTA自监督方法，甚至超过完全监督基线，展示了优越的数据效率、鲁棒性和表示泛化能力。

Conclusion: CIG-MAE通过物理语义一致的掩码自编码框架，有效解决了WiFi CSI动作识别中的标注瓶颈问题，为无线感知领域提供了高效的自监督学习方案。
...
### [12] [Secret Key Generation on Aerial Rician Fading Channels Against a Curious Receiver](https://arxiv.org/abs/2512.04756)
*Mattia Piana,Stefano Tomasin*

Main category: eess.SP

TL;DR: 提出一种利用无人机移动性在Rician衰落信道下生成物理层密钥的新方法，通过优化无人机轨迹位置来最大化密钥容量下界。


<details>
  <summary>Details</summary>
Motivation: 物理层密钥生成是下一代网络的关键技术，但现有方法在无人机网络中存在挑战。当窃听者是网络内部节点（好奇设备）时，需要更安全的密钥生成方案。无人机移动性为利用信道变化生成密钥提供了新机会。

Method: 提出两阶段协议：1）训练阶段；2）无人机Alice选择空间位置轨迹，从每个位置向地面Bob发送消息；Bob根据接收信号估计信道增益，从中提取密钥。通过优化无人机位置选择来最大化密钥容量下界。

Result: 通过数值仿真验证了所提方法的有效性，表明通过优化无人机轨迹位置可以显著提高物理层密钥生成的安全性能。

Conclusion: 利用无人机移动性和Rician衰落信道特性，通过优化轨迹位置选择，可以有效生成物理层密钥，为无人机网络提供安全通信保障，特别适用于内部窃听者场景。
...
### [13] [Movable Antenna Assisted Flexible Beamforming for Integrated Sensing and Communication in Vehicular Networks](https://arxiv.org/abs/2512.04802)
*Luyang Sun,Zhiqing Wei,Haotian Liu,Kan Yu,Zhendong Li,Zhiyong Feng*

Main category: eess.SP

TL;DR: 该论文提出两种算法优化可移动天线辅助的车路协同ISAC系统，通过联合优化天线位置、波束成形和功率分配，提升通信和感知性能。


<details>
  <summary>Details</summary>
Motivation: 第六代无线网络中，集成感知与通信（ISAC）是关键技术，而可移动天线（MA）提供的额外空间自由度可显著提升ISAC系统性能。车路协同（V2I）网络在动态环境中需要实时优化以同时满足通信和感知需求。

Method: 结合扩展卡尔曼滤波预测和实时优化，提出两种算法：1）无感知服务质量约束场景下的预处理-舒尔补-投影梯度上升算法；2）有感知QoS约束场景下的启发式反射投影动态粒子群优化算法，联合优化发射天线位置、波束成形和功率分配向量。

Result: 仿真结果表明，所提方法能同时提升通信总速率和运动参数估计的克拉美罗下界下限，验证了MA辅助波束成形在动态V2I ISAC网络中的有效性。

Conclusion: 可移动天线辅助的波束成形能显著提升动态V2I ISAC网络的性能，所提出的两种算法分别适用于有无感知QoS约束的场景，均能在少量迭代下实现性能增益。
...
### [14] [Cute but Cunning: Effective Closed-Form Alternatives to the Exact Lognormal Statistics](https://arxiv.org/abs/2512.04872)
*Carlos Rafael Nogueira da Silva,Maria Cecilia Luna Alvarado,Fernando Darío Almeida García,Michel Daoud Yacoub*

Main category: eess.SP

TL;DR: 提出两种基于Nakagami-m和逆Nakagami-m乘积的对数正态分布替代模型，解决无线通信中大规模衰落建模的分析难题


<details>
  <summary>Details</summary>
Motivation: 对数正态分布在无线通信中是大规模衰落的主要统计模型，但其分析上的难处理性给信道表征和性能分析带来持续挑战，需要寻找数学上可处理的替代模型

Method: 1) 使用Nakagami-m和逆Nakagami-m变数的乘积构建两种替代模型；2) 开发矩匹配框架将对数正态参数映射到替代模型参数；3) 提出随机混合方法利用两种分布的互补近似特性增强收敛性；4) 将方法扩展到包含α-μ、κ-μ和η-μ变数的异构级联衰落信道

Result: 获得了关键性能指标的渐近精确闭式表达式（特征函数、误码率、香农容量），实现了复合α-μ-对数正态衰落模型的概率密度函数和累积分布函数的分析可处理表达式，数值结果证实了方法的准确性和效率

Conclusion: 提出的替代模型为精确对数正态统计提供了实用可靠的替代方案，解决了无线通信中大规模衰落建模的分析难题，方法可扩展到更复杂的异构衰落信道
...
TL;DR: 提出CAMD框架用于MIMO系统中的自动调制识别，通过信道补偿和多域特征提取解决多天线信道干扰问题。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习AMR方法主要关注SISO系统，对MIMO系统研究有限。多天线信道干扰会影响MIMO信号的统计特性，使识别变得困难。

Method: 提出CAMD框架：1) 通过高效信道补偿模块重构发射信号；2) 提取并整合多域特征，包括天线内时间相关性和天线间信道相关性。

Result: 在复杂移动信道环境的MIMOSig-Ref数据集上验证，实验证明CAMD优于现有最优方法。

Conclusion: CAMD框架通过信道补偿和多域特征提取，有效解决了MIMO系统中AMR的挑战，在复杂信道环境下表现出优越性能。
...
TL;DR: 智能手机U-Turn测试能准确、可重复地测量多发性硬化患者的转身速度，不受手机佩戴位置和测试环境的影响，在临床试验中具有应用潜力。


<details>
  <summary>Details</summary>
Motivation: 开发一种基于智能手机的动态平衡评估工具（U-Turn测试），用于多发性硬化患者的远程监测和临床试验评估。

Method: 研究纳入96名EDSS≤6.5的多发性硬化患者，在实验室环境（监督设置）使用6个不同身体位置的智能手机与运动捕捉系统对比，并在2周远程期间（非监督设置）进行日常测试。

Result: 智能手机检测转身的准确率高（F1分数>95%），转身速度测量与运动捕捉系统高度一致（ICC: 0.87-0.92），测试-重测信度>0.90，且与临床和患者报告指标显著相关（r=-0.79至-0.61）。

Conclusion: U-Turn测试能准确、可重复地测量转身速度，不受智能手机佩戴位置和测试环境的影响，是多发性硬化临床试验中有价值的评估工具。
...
TL;DR: 论文提出了一种分布式黎曼优化算法——黎曼扩散自适应，用于解决流形上非凸优化问题，证明了算法的网络一致性、一阶驻点收敛性，并在满足PL条件时具有线性收敛性，最后应用于Grassmann流形上的去中心化鲁棒PCA问题。


<details>
  <summary>Details</summary>
Motivation: 研究分布式黎曼优化问题，其中智能体的成本函数是测地光滑但可能测地非凸的。传统分布式优化策略如扩散自适应主要针对欧几里得空间，需要扩展到黎曼流形上处理更复杂的几何结构。

Method: 将扩散自适应策略扩展到黎曼流形，提出黎曼扩散自适应算法。该算法在具有有界曲率的流形上最小化测地光滑非凸函数和，通过理论分析证明其收敛性质。

Result: 1) 算法能够近似实现网络一致性（智能体间迭代的Fréchet方差小）；2) 对于一般测地非凸成本函数，保证收敛到一阶驻点；3) 当全局成本函数满足黎曼Polyak-Lojasiewicz条件时，在恒定步长下线性收敛到稳态误差；4) 在Grassmann流形上的去中心化鲁棒PCA应用中验证了算法的收敛性和性能。

Conclusion: 黎曼扩散自适应算法是处理分布式黎曼优化问题的有效方法，特别适用于流形上的非凸优化，具有良好的理论保证和实际应用价值，为去中心化流形学习提供了新工具。
...
TL;DR: 提出一种新的单光子激光雷达模拟器，通过马尔可夫更新过程建模光子计数统计，在保持高精度的同时实现快速仿真。


<details>
  <summary>Details</summary>
Motivation: 现有单光子激光雷达模拟器面临两难：快速但不准确的泊松模型，或准确但计算成本过高的顺序模型。需要一种既能保持物理保真度又能高效计算的解决方案。

Method: 1) 提出马尔可夫更新过程(MRP)公式，首次解析预测死区时间下注册光子计数的均值和方差；2) 引入谱截断规则高效计算复杂协方差统计；3) 证明过程的平移不变性，通过预计算查找表将单像素模型扩展到完整直方图立方体生成。

Result: 该方法生成的3D立方体与顺序黄金标准无法区分，但速度快几个数量级，实现了大规模、物理保真的数据生成。

Conclusion: 该方法打破了单光子激光雷达模拟器在精度和速度之间的折衷，为基于学习的SP-LiDAR重建提供了大规模、物理保真的数据生成能力。
...
TL;DR: 提出一种基于漏泄同轴电缆的低频广义夹持天线系统，将原本仅适用于高频的波导夹持天线概念扩展到低频应用，通过可重构辐射槽实现可控LOS链路、降低路径损耗，并开发了联合优化框架提升吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有波导夹持天线技术主要局限于高频场景，无法应用于低频无线网络。为了将夹持天线的优势（如可重构LOS链路、降低路径损耗等）扩展到低频6G网络，需要开发新的实现方案。

Method: 采用漏泄同轴电缆实现低频夹持天线系统，通过可控辐射槽继承波导夹持天线的关键能力。开发了双阶段传播模型来表征LOS和NLoS路径的引导传输和无线辐射，并设计了基于博弈论关联和凸功率分配的联合优化框架来最大化吞吐量。

Result: 分析结果显示系统具有强局部增益和快速距离相关衰减特性。仿真结果表明，相比传统固定天线基准，该系统能获得显著的性能提升。

Conclusion: 提出的漏泄同轴电缆实现成功将夹持天线概念扩展到低频操作，为6G网络提供了灵活可重构的天线架构，通过联合优化框架实现了吞吐量的最大化，具有实际部署潜力。
...
TL;DR: 提出针对感知子空间码的高效解码算法，将复杂度从立方降为二次方，并在复杂度与错误性能之间提供可调节的权衡


<details>
  <summary>Details</summary>
Motivation: 稀疏天线阵列感知中的方向到达角估计需要高效解码算法，现有MAP解码器复杂度为天线数量的立方，需要降低复杂度

Method: 提出新颖的高效解码算法，通过调整参数在复杂度与错误性能之间进行权衡，复杂度可降至二次方

Result: 通过蒙特卡洛仿真验证，新解码器在SNR范围内表现良好，当复杂度从二次方增加到三次方时，性能平滑接近MAP解码器
...
### [21] [Constructing Low-Redundancy Codes via Distributed Graph Coloring](https://arxiv.org/abs/2512.04197)
*Yuting Li,Ryan Gabrys,Farzad Farnoud*

Main category: cs.IT

TL;DR: 提出基于分布式图着色的纠错码构造框架，利用混淆图的独立集与有效编码的对应关系，通过改进Linial着色算法实现多项式时间编码解码，获得多种类型错误下的高效编码方案。


<details>
  <summary>Details</summary>
Motivation: 传统纠错码构造方法如综合征压缩存在局限性，依赖好的基础码且不够灵活通用。需要一种更灵活、可推广且不依赖基础码的纠错码构造框架，能够处理各种类型的错误模式。

Method: 基于分布式图着色（LOCAL模型），利用混淆图中独立集与有效编码的对应关系，通过改进Linial着色算法实现多项式时间的顶点着色计算，从而构建高效的编码解码方案。还提出了超图标记扩展用于列表可解码码。

Result: 1) 针对任意类型常数个错误的唯一可解码码，冗余度仅为Gilbert-Varshamov界的两倍；2) 通过超图标记实现列表可解码码；3) 编辑距离未知时的增量同步方案，降低平均通信量；4) 针对无界长度编辑突发的最优码（达到最优的8倍因子内）。

Conclusion: 基于分布式图着色的纠错码构造框架比传统综合征压缩方法更灵活通用，不依赖好的基础码，在多种参数下实现了改进的冗余度，为各种错误模式提供了高效的编码解决方案。
...
### [22] [Joint Low-Rank and Sparse Bayesian Channel Estimation for Ultra-Massive MIMO Communications](https://arxiv.org/abs/2512.04470)
*Jianghan Ji,Cheng-Xiang Wang,Shuaifei Chen,Chen Huang,Xiping Wu,Emil Björnson*

Main category: cs.IT

TL;DR: 提出一种联合低秩和稀疏贝叶斯估计算法(LRSBE)，用于超大规模MIMO系统的空间非平稳信道估计，利用波束域的低秩性和稀疏性，在估计精度和复杂度方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 超大规模MIMO通信中的信道估计面临挑战，特别是空间非平稳信道。现有方法未能充分利用信道在波束域的低秩性和稀疏性特性，需要更高效准确的估计算法。

Method: 提出联合低秩和稀疏贝叶斯估计算法(LRSBE)，在期望最大化框架下，结合稀疏贝叶斯学习和软阈值梯度下降，利用波束域的低秩性和稀疏性进行信道估计。

Result: 仿真结果表明，在不同信噪比条件下，所提算法在估计精度和总体复杂度方面显著优于现有最先进方法。

Conclusion: LRSBE算法通过有效利用超大规模MIMO信道在波束域的低秩性和稀疏性，实现了高性能的信道估计，为超大规模MIMO通信系统提供了有效的解决方案。
...
### [23] [One-Step Generative Channel Estimation via Average Velocity Field](https://arxiv.org/abs/2512.04501)
*Zehua Jiang,Fenghao Zhu,Siming Jiang,Chongwen Huang,Zhaohui Yang,Richeng Jin,Zhaoyang Zhang,Merouane Debbah*

Main category: cs.IT

TL;DR: 提出一种用于无线通信中一步生成式信道估计的新方法，通过直接学习平均速度场来绕过传统生成模型的迭代去噪过程，显著降低延迟并提高性能。


<details>
  <summary>Details</summary>
Motivation: 生成模型在无线通信中通过学习复杂信道数据分布展现出巨大潜力，但其迭代去噪过程在延迟敏感的信道估计场景中带来显著挑战。

Method: 提出一步生成式信道估计方法，通过直接学习平均速度场来绕过传统模型的迭代步骤，避免时间消耗。

Result: 通过大量仿真验证，该方法比现有最先进的基于扩散的方法性能更好，归一化均方误差降低达2.65 dB，延迟减少约90%。

Conclusion: 该方法在信道估计性能方面具有显著潜力，通过一步生成过程解决了延迟敏感无线通信场景中的关键挑战。
...
### [24] [Timely Information for Strategic Persuasion](https://arxiv.org/abs/2512.04679)
*Ahmet Bugra Gundogan,Melih Bastopcu*

Main category: cs.IT

TL;DR: 研究动态贝叶斯劝说问题，发送者通过控制信息揭示时机影响接收者信念，双方目标不一致形成Stackelberg博弈，发送者需在采样约束和激励相容约束下最大化接收者估计为1的时间比例。


<details>
  <summary>Details</summary>
Motivation: 研究动态环境下的信息劝说问题，发送者希望影响接收者的信念估计（使其估计为1），而接收者希望准确估计真实状态。这种目标不一致导致需要设计激励相容的信息揭示策略，同时考虑资源约束。

Method: 采用Stackelberg博弈框架，发送者为领导者选择信息揭示策略，接收者为跟随者决定是否跟随消息。考虑连续时间马尔可夫链状态演化，发送者目标是在总采样约束和激励相容约束下最大化接收者估计为1的长期平均时间。

Result: 单源问题中，发送者最优策略是对不希望的状态0分配最小采样率（仅满足激励相容约束），剩余采样率分配给希望的状态1。多源扩展中，每个源有不同的最小采样率要求，发送者可以利用信息及时性影响接收者获得更高效用。

Conclusion: 研究表明在动态贝叶斯劝说中，发送者可以通过控制信息揭示时机有效影响接收者信念，最优策略涉及在激励相容约束下的采样率分配，信息及时性成为影响效用的重要因素。
...
### [25] [Rotatable Antenna-Enhanced Cell-Free Communication](https://arxiv.org/abs/2512.04742)
*Kecheng Pan,Beixiong Zheng,Yanhua Tan,Emil Björnson,Robert Schober,Rui Zhang*

Main category: cs.IT

TL;DR: 提出了一种基于可旋转天线的无小区系统，通过联合优化AP-用户关联和天线波束方向来最大化用户总速率


<details>
  <summary>Details</summary>
Motivation: 可旋转天线技术能够通过灵活调整天线的三维波束方向来开发新的空间自由度，这为提升无小区系统的性能提供了新的机会

Method: 采用两阶段策略解决AP-用户关联问题，然后使用分数规划和逐次凸逼近技术优化可旋转天线的波束方向

Result: 数值结果表明，所提出的可旋转天线增强型无小区系统在各种基准方案中表现显著优越

Conclusion: 可旋转天线技术能够有效提升无小区系统的性能，通过联合优化天线波束方向和AP-用户关联可以显著提高用户总速率
...
### [26] [Robust Precoding Designs of RSMA for Multiuser MIMO Systems](https://arxiv.org/abs/2512.04750)
*Wentao Zhou,Yijie Mao,Di Zhang,Mérouane Debbah,Inkyu Lee*

Main category: cs.IT

TL;DR: 提出一种基于广义互信息的鲁棒预编码设计，用于不完美CSI下的RSMA MU-MIMO系统，在保持性能的同时显著降低计算复杂度。


<details>
  <summary>Details</summary>
Motivation: RSMA在多用户MIMO系统中，尤其是在发射机信道状态信息不完美的情况下表现出色，但现有的和速率最大化预编码设计通常计算复杂度很高，需要一种高效的实现方案。

Method: 首先采用广义互信息构建和速率最大化问题中目标函数的下界，然后应用非光滑和速率目标函数的平滑下界构建新优化问题。通过揭示广义信干噪比与最小均方误差矩阵之间的关系，将问题转化为可处理形式，并分解为三个子问题，基于顺序解研究新的交替预编码设计。

Result: 仿真结果表明，所提出的预编码方案在保持与传统方法相当性能的同时，显著降低了计算复杂度。

Conclusion: 该研究提出了一种高效的RSMA鲁棒预编码设计，解决了不完美CSI下MU-MIMO系统的计算复杂度问题，为实际系统部署提供了可行的解决方案。
...
### [27] [Exact 3-D Channel Impulse Response for Spherical Receivers with Arbitrary Drift Directions](https://arxiv.org/abs/2512.04858)
*Yen-Chi Lee,Ping-Cheng Yeh,Chia-Han Lee*

Main category: cs.IT

TL;DR: 本文首次推导出任意方向均匀漂移下球形接收器的精确信道冲激响应，解决了分子MIMO系统中三维信道建模的关键挑战。


<details>
  <summary>Details</summary>
Motivation: 现有分子通信系统的信道建模存在局限性：精确解仅限于一维/无漂移场景或平面接收器几何形状，而任意流动方向下的球形接收器信道冲激响应仍是一个未解决的挑战。

Method: 采用Girsanov定理，将平稳介质中的命中时间分布解析地转换到漂移介质中，从而推导出任意方向均匀漂移下球形接收器的闭合形式信道冲激响应表达式。

Result: 获得了首个三维分子通信系统中球形接收器在任意方向均匀漂移下的精确解析信道冲激响应，消除了先前近似方法对漂移与传输轴夹角的忽略误差。

Conclusion: 提出的闭合形式表达式不仅提高了信道建模精度，还实现了关键系统参数的高效探索，为现实分子MIMO系统设计提供了理论基础。
...
### [28] [Bounds on Maximal Leakage over Bayesian Networks](https://arxiv.org/abs/2512.04955)
*Anuran Makur,Japneet Singh*

Main category: cs.IT

TL;DR: 本文研究有限字母表贝叶斯网络中最大泄漏的行为，建立了基于耦合特征的边界条件，并针对|X|=4的情况提供了更一般的耦合特征条件。


<details>
  <summary>Details</summary>
Motivation: 最大泄漏量化了数据X由于观测Y导致的信息泄漏。虽然最大泄漏的基本性质（如数据处理、次可加性及其与互信息的联系）已确立，但其在贝叶斯网络中的行为尚未充分理解，现有边界主要限于二元X。

Method: 利用满足特定条件的信道存在的基于耦合的特征来建立最大泄漏边界，为|X|=4的情况提供更一般的耦合特征条件，并提出新的最大泄漏指数的同时耦合结果。

Result: 建立了有限字母表贝叶斯网络中最大泄漏的边界，扩展了耦合特征的应用范围，并通过示例说明了所提边界的有效性。

Conclusion: 本文推进了对贝叶斯网络中最大泄漏行为的理解，提供了更一般的边界条件，为有限字母表情况下的信息泄漏分析提供了理论工具。
...
### [29] [Environment-Aware Channel Inference via Cross-Modal Flow: From Multimodal Sensing to Wireless Channels](https://arxiv.org/abs/2512.04966)
*Guangming Liang,Mingjie Yang,Dongzhu Liu,Paul Henderson,Lajos Hanzo*

Main category: cs.IT

TL;DR: 该论文提出了一种基于多模态感知数据的免导频信道推断方法，利用相机图像、LiDAR点云和GPS坐标直接估计完整CSI，通过跨模态流匹配框架实现高效实时信道估计。


<details>
  <summary>Details</summary>
Motivation: 在大规模MIMO系统和高多普勒环境中，通过导频获取信道状态信息会产生大量开销。随着环境感知数据的可用性增加，研究如何利用多模态观测数据直接推断完整CSI，避免导频开销。

Method: 提出数据驱动的跨模态流匹配框架：1) 将多模态特征融合到信道域的潜在分布中；2) 学习速度场，将潜在分布连续变换到信道分布；3) 重新表述为条件流匹配目标，加入模态对齐损失；4) 采用低延迟推理机制实现实时CSI估计。

Result: 基于Sionna和Blender构建程序化数据生成器进行实验验证。系统级评估显示，在信道估计精度和下游波束成形任务的频谱效率方面，相比基于导频和感知的基准方法有显著提升。

Conclusion: 通过多模态感知数据驱动的跨模态流匹配框架，实现了高效、准确的免导频信道推断，为大规模MIMO系统在高动态环境中的可靠通信提供了新方案。
...
### [30] [Performance Analysis of Fluid Reconfigurable Intelligent Surface over Covert Communications](https://arxiv.org/abs/2512.05085)
*Farshad Rostami Ghadi,Masoud Kaveh,Hanjiang Hong,Kai-Kit Wong,Riku Jantti,F. Javier Lopez-Martinez*

Main category: cs.IT

TL;DR: FRIS在隐蔽通信中的优势：相比固定位置RIS，FRIS在低到中等发射功率下能同时提升隐蔽性和可靠性，但在高功率下固定位置RIS可能因减少向敌方的泄漏而具有稍高的成功概率。


<details>
  <summary>Details</summary>
Motivation: 研究流体可重构智能表面（FRIS）对隐蔽通信的影响，探索如何利用FRIS技术实现更有效的隐蔽信息传输，同时对抗敌方的检测企图。

Method: 分析虚假警报和漏检概率，推导隐蔽中断概率的闭式表达式，在最优检测阈值下表征成功概率，通过数值结果比较FRIS与固定位置RIS的性能差异。

Result: FRIS在低到中等发射功率下相比固定位置RIS具有明显优势，能同时改善可靠性和增强隐蔽性；但在极高功率水平下，固定位置RIS可能因减少向敌方的泄漏而维持稍高的成功概率。

Conclusion: FRIS为隐蔽通信提供了新的技术途径，在特定功率范围内能更好地平衡隐蔽性和可靠传输之间的权衡，为未来无线安全通信系统设计提供了重要参考。
