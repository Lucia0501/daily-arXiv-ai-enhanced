...
TL;DR: 提出一种基于稀疏Grassmannian设计的预编码码本，通过稀疏结构实现低PAPR、低计算复杂度和低存储成本，同时性能接近最优码本。


<details>
  <summary>Details</summary>
Motivation: 传统Grassmannian预编码码本设计计算复杂度高，而现有码本在功率效率、存储和计算成本方面存在不足。需要一种既能保持性能又具有实际部署优势的码本设计方法。

Method: 基于Grassmann流形上的Schubert胞腔分解构造稀疏码本。利用稀疏性简化目标函数，显著减少搜索空间，降低设计复杂度。

Result: 数值仿真表明，在非相关瑞利衰落信道下，稀疏码本渐近接近最优码本，在可达速率上优于5G NR现有码本，同时PAPR显著低于传统密集设计。

Conclusion: 提出的稀疏Grassmannian码本可作为传统码本的实用且功率高效的替代方案，适用于广泛的上行链路传输场景。
...
TL;DR: 提出改进的自适应多项式混沌展开方法，结合双向抛物波方程，首次对不规则地形传播中的实际天线配置进行不确定性量化，仅需30次仿真即可准确预测路径损耗的均值和百分位范围。


<details>
  <summary>Details</summary>
Motivation: 在不规则地形环境中，无线电波传播的精确建模对可靠无线通信系统设计至关重要，但确定性模型未能量化天线配置的不确定性。需要开发高效的不确定性量化方法来评估实际天线配置对传播特性的影响。

Method: 改进自适应多项式混沌展开方法，结合双向抛物波方程。通过方差贡献扩展多项式基，采用结合验证误差和样本-基比率的复合终止准则，通过最小二乘回归实现稳定系数估计，无需额外正则化。

Result: 收敛分析显示训练样本增加时误差单调衰减，产生紧凑的低交互模型。对于两个实际地形剖面，仅需30次PWE仿真即可准确预测路径损耗的均值和5%-95%百分位范围，与蒙特卡洛参考结果匹配。在固定采样预算下，APCE优于标准和稀疏PCE方法。

Conclusion: 提出的改进APCE方法首次实现了不规则地形传播中实际天线配置的不确定性量化，能够高效准确地预测路径损耗统计特性，为无线通信系统设计提供了可靠的工具。
...
TL;DR: 维也纳4G/5G路测数据集：首个结合被动扫描仪和主动手机日志的城市级开放数据集，支持移动网络分析与优化研究


<details>
  <summary>Details</summary>
Motivation: 移动网络分析、规划和优化研究常受限于缺乏大规模、全面的真实世界数据集。现有数据集往往规模有限或覆盖不完整，难以支持环境感知学习、传播建模等高级研究需求。

Method: 在奥地利维也纳全市范围内收集地理参考的LTE和5G NR测量数据，结合被动宽带扫描仪观测和主动手机日志，提供网络侧和用户侧互补视角。数据包含时间位置对齐、基站部署参数推断，并提供高分辨率建筑和地形模型。

Result: 创建了包含扫描仪数据、手机数据、估计小区信息和城市模型组件的综合数据集。数据集支持可重复的基准测试，涵盖环境感知学习、传播建模、覆盖分析和射线追踪校准等工作流程。

Conclusion: 该数据集填补了移动网络研究领域的数据空白，为研究人员提供了城市级真实测量数据，支持从机器学习到确定性建模的多种分析方法，有望推动移动网络分析与优化研究的发展。
...
TL;DR: 提出一种用于零填充仿射频分复用系统的低复杂度单抽头均衡器，通过参数选择和域变换实现高效符号恢复。


<details>
  <summary>Details</summary>
Motivation: 仿射频分复用是双选择性信道的鲁棒解决方案，但现有均衡器复杂度较高。需要设计低复杂度均衡器来降低计算负担，同时保持性能。

Method: 1) 选择AFDM参数c1和c2，使c1值较高，c2依赖于c1，简化仿射域输入输出关系；2) 对发送仿射域符号进行零填充，对接收符号进行重构操作，使有效仿射域IOR近似为循环卷积；3) 将仿射域符号变换到频率-仿射域，在该域设计单抽头均衡器。

Result: 数值结果表明，所提出的单抽头均衡器在c1值较高时特别有效，且不损害性能鲁棒性。该方法显著降低了均衡复杂度。

Conclusion: 通过参数优化、零填充和域变换，成功实现了ZP-AFDM系统的低复杂度单抽头均衡，为双选择性信道通信提供了高效解决方案。
...
TL;DR: 该论文系统评估了输电系统中电能质量参数的个体和集成预测方法，基于德国和爱沙尼亚的700多个周时间序列数据，证明集成预测在准确性和鲁棒性上优于个体模型。


<details>
  <summary>Details</summary>
Motivation: 随着电力电子技术的广泛应用，输电系统电能质量监测变得日益重要。虽然运营商收集了大量数据，但这些数据通常仅用于合规性评估。中长期预测可以通过支持主动资产管理和趋势检测来提升这些数据的价值，尽管面临数据异质性和季节性的挑战。

Method: 系统评估了输电系统中电能质量参数的个体和集成预测方法。分析了德国和爱沙尼亚测量活动中收集的700多个周时间序列数据，在结构化集成框架内评估了各种模型和聚合策略。

Result: 集成预测在准确性和鲁棒性方面持续优于个体模型，相比季节性朴素基准和表现最佳的单模型实现了显著改进。集成预测被证明是输电系统中长期电能质量预测的稳健且可扩展的方法。

Conclusion: 集成预测是输电系统长期电能质量预测的稳健且可扩展方法，能够有效处理数据异质性和季节性挑战，为主动资产管理和趋势检测提供支持。
...
TL;DR: 提出基于正交延迟多普勒分复用调制器的多普勒移位键控调制，解决现有DD域调制高PAPR和高检测复杂度问题，适用于高移动性应用


<details>
  <summary>Details</summary>
Motivation: 现有延迟多普勒域调制在高速移动应用中存在高峰均功率比和不可接受的检测复杂度问题，尤其在上行链路传输中，需要大时宽和带宽保证高DD分辨率

Method: 提出多普勒移位键控调制，基于正交延迟多普勒分复用调制器，利用DD域输入输出特性；采用one-hot映射和低复杂度迭代SIC-MRC检测器；扩展到零自相关序列实现，支持多用户上行链路；使用ZC序列作为基序列，优化ZC根分配最小化用户间干扰

Result: 通过双弥散信道下的广泛仿真验证，DSK调制具有低PAPR和良好的误码率性能，低复杂度检测器有效，特别适用于双弥散信道的上行多址接入

Conclusion: DSK调制是解决DD域调制高PAPR和检测复杂度问题的有效方案，特别适用于高速移动场景的上行多址接入，在双弥散信道中表现优越
...
TL;DR: 提出一种用于虚拟MIMO广播信道的合成孔径安全波束成形方法，通过优化无人机节点部署、传输策略和波束成形来最大化系统信道容量并增强物理层安全性。


<details>
  <summary>Details</summary>
Motivation: 在混合窃听环境下，需要设计安全的虚拟MIMO广播信道系统。传统方法在物理层安全性方面存在局限，特别是在动态环境中对抗窃听者的能力不足。合成孔径波束成形技术能够提供空间增益控制，但如何优化无人机部署和波束成形策略以实现安全通信仍是一个挑战。

Method: 1) 利用合成孔径波束成形在自由空间中沿预定角度提供空间增益，同时在其他方向抑制增益；2) 分析渐近信道特征值表达式以优化无人机飞行节点部署；3) 基于给定虚拟MIMO信道，研究最小化发射功耗的能量高效预编码设计方法，满足基站QoS、窃听者泄漏容忍度和节点功率约束；4) 将非凸功率最小化问题转化为可处理形式；5) 应用线性约束最小方差(LCMV)方法设计接收波束成形以有效抑制干扰。

Result: 数值结果表明，所提出的方法在提升信道容量方面具有优越性。通过优化无人机部署和波束成形策略，系统能够在保证合法用户服务质量的同时，有效抑制窃听者的信息获取，实现安全的无线通信。

Conclusion: 该研究提出了一种有效的合成孔径安全波束成形框架，通过联合优化无人机部署、预编码和波束成形，在混合窃听环境下实现了虚拟MIMO广播信道的安全通信。该方法不仅提升了系统容量，还增强了物理层安全性，为未来无线通信系统的安全设计提供了新思路。
...
TL;DR: 该论文研究利用双跳非视距路径改进双基地快照无线电SLAM，推导了CRB并提出了识别和利用双跳路径的算法，实验证明双跳路径能提高定位精度并增强环境建图能力。


<details>
  <summary>Details</summary>
Motivation: 现有无线电SLAM方法主要利用视距和单跳非视距路径，而将高阶非视距路径视为干扰。本文旨在探索利用双跳非视距路径来解决双基地快照无线电SLAM问题，以提升用户设备和地标位置的联合估计精度。

Method: 1) 推导了存在双跳非视距路径时用户设备状态和地标位置联合估计的克拉美罗下界；2) 提出了识别双跳非视距路径的算法；3) 将双跳路径纳入用户设备和地标的联合估计中；4) 使用模拟数据和实验毫米波测量数据进行验证。

Result: 数值和实验结果表明：与单跳非视距路径共享至少一个入射点的双跳路径能提高用户设备状态和现有单跳路径入射点的估计精度。更重要的是，利用双跳路径增强了环境建图能力，能够揭示仅使用单跳路径无法观测到的地标。

Conclusion: 双跳非视距路径在无线电SLAM中具有重要价值，不仅能提高现有参数的估计精度，还能扩展环境感知能力，为更精确的定位和建图提供了新的可能性。
...
TL;DR: SilentWear是一种完全可穿戴的纺织颈部接口，用于EMG信号采集和处理，结合轻量级CNN架构SpeechNet，实现端到端实时设备端语音识别，适用于有声和无声语音，具有低功耗和长续航特性。


<details>
  <summary>Details</summary>
Motivation: 现有基于生物信号的语音检测方法存在可穿戴性不足和缺乏边缘处理能力的问题，这限制了其在临床和日常应用中的最小侵入性、响应性和隐私保护性。

Method: 开发了SilentWear系统，包括纺织颈部接口和BioGAP-Ultra处理器，采集14个差分通道的EMG信号；设计了轻量级CNN架构SpeechNet（仅15k参数）进行语音解码；提出增量微调策略缓解重新定位带来的性能下降；在商用多核MCU上实现端到端实时设备端处理。

Result: 在8个代表性人机交互命令上，有声和无声语音的平均交叉验证准确率分别为84.8±4.6%和77.5±6.6%；跨会话设置下分别为71.1±8.3%和59.3±2.2%；增量微调策略使用少于10分钟额外数据恢复超过10%准确率；设备端推理能耗63.9μJ，延迟2.47ms，总功耗20.5mW，可连续工作超过27小时。

Conclusion: SilentWear系统展示了完全可穿戴、纺织基的EMG语音接口的可行性，通过轻量级模型和设备端处理实现了高性能、低功耗的实时语音识别，为噪声鲁棒、隐私保护、可扩展的人机交互应用提供了解决方案。
...
TL;DR: 该研究设计了用于29GHz卫星通信的Ka波段移动天线，从单单元扩展到8x8阵列，实现了高达21dB的增益和低至-30dB的回波损耗，通过数学计算和CST软件优化，解决了Ka波段天线设计的挑战。


<details>
  <summary>Details</summary>
Motivation: Ka波段（29GHz）具有更高的数据传输速率，能够满足现代卫星通信系统对高速互联网接入和多媒体流媒体等应用的需求。然而，Ka波段天线设计面临信号衰减、方向精度、圆极化和阻抗匹配等挑战，需要开发高性能的移动天线解决方案。

Method: 采用从单单元到8x8阵列的渐进设计方法，通过数学计算确定贴片尺寸、基板特性和有效介电常数等参数，使用CST软件进行仿真和实现。采用相控阵和电子可调天线等多种配置，并集成机器学习技术进行优化。

Result: 天线实现了高达21dB的增益和低至-30dB的回波损耗，成功解决了Ka波段天线设计中的关键挑战，包括信号衰减、方向精度、圆极化和阻抗匹配等问题。

Conclusion: 该研究推动了高频传输技术的发展，满足了现代卫星通信系统对高速数据传输的需求，为高速互联网接入和多媒体流媒体等应用提供了有效的天线解决方案。
...
TL;DR: 提出基于连续孔径阵列的集成感知与通信框架，在衰落信道下分析三种连续波束成形设计，推导性能闭式表达式，证明优于传统离散阵列方案
...
Method: 建立连续算子信号模型，利用Landau特征值定理分析信道统计特性，提出三种连续波束成形设计：感知中心、通信中心和帕累托最优设计

Result: 推导了感知速率、遍历通信速率和中断概率的闭式表达式，获得高信噪比下的复用和分集增益，表征了SR-CR可达区域，数值结果显示优于传统方案

Conclusion: CAPA-ISAC框架在衰落信道下显著优于传统离散阵列和频分方案，为集成感知与通信系统设计提供了新思路
...
### [12] [Video TokenCom: Textual Intent-Guided Multi-Rate Video Token Communications with UEP-Based Adaptive Source-Channel Coding](https://arxiv.org/abs/2603.02470)
*Jingxuan Men,Mahdi Boloursaz Mashhadi,Ning Wang,Yi Ma,Mike Nilsson,Rahim Tafazolli*

Main category: cs.IT

TL;DR: 提出Video TokenCom框架，通过文本意图引导的多速率视频通信，结合不等错误保护，在带宽受限下提升语义保真度
...
Method: 1) 使用预训练视频tokenizer提取离散视频token，结合文本条件视觉语言建模和光流传播识别用户意图相关的时空token；2) 语义感知多速率比特分配策略，意图相关token用完整码本精度编码，非意图token用降低码本精度的差分编码；3) 源信道编码自适应方案，根据资源和链路条件调整比特分配和信道编码

Result: 在多个视频数据集上的实验表明，该框架在宽SNR范围内，在感知和语义质量方面均优于传统和语义通信基线方法

Conclusion: Video TokenCom框架通过文本意图引导的不等错误保护源信道编码自适应，实现了高效的语义感知视频通信，为未来无线网络中的token通信范式提供了有效解决方案
...
### [13] [RIS-Enabled Wireless Channel Equalization: Adaptive RIS Equalizer and Deep Reinforcement Learning](https://arxiv.org/abs/2603.02489)
*Gal Ben-Itzhak,Ender Ayanoglu*

Main category: cs.IT

TL;DR: 该论文研究了RIS辅助的脉冲响应均衡和信号增强，比较了基于信道估计的经典自适应滤波方法和无模型的深度强化学习方法，发现SAC算法在性能和复杂度之间取得了良好平衡。


<details>
  <summary>Details</summary>
Motivation: 可重构智能表面(RIS)能够重塑无线传播环境，但配置大型无源阵列以实现可靠信号均衡的实用方法有限。均衡对于抵消多径引起的脉冲失真至关重要，否则会降低符号恢复性能。

Method: 提出了两种方法：1) 基于信道估计的自适应RIS均衡(ARISE)，利用最速下降法配置RIS系数；2) 无模型的深度强化学习(DRL)方法(DDPG、TD3、SAC)，直接从接收脉冲响应优化RIS系数，无需显式信道估计。

Result: 通过大量仿真表明，SAC算法实现了快速稳定的收敛，均衡性能与ARISE相当，同时显著降低了实现复杂度。DRL方法在多种信道条件和RIS尺寸下都表现良好。

Conclusion: 深度强化学习(特别是SAC)为实时RIS控制提供了实用且可扩展的解决方案，能够在无需复杂信道估计的情况下实现有效的脉冲响应均衡和信号增强。
...
### [14] [E-variables and tests of randomness for distribution classes](https://arxiv.org/abs/2603.02492)
*Georgii Potapov,Yuri Kalnishkan*

Main category: cs.IT

TL;DR: 本文提出了一种e变量近似性方法，用于为实际应用中重要的分布类构建e变量，并展示了如何将其用于构造针对分布类的随机性检验。


<details>
  <summary>Details</summary>
Motivation: e变量是近年来快速发展的统计假设检验新方法，但为实际应用中常见的分布类构建e变量仍然是一个挑战。本文旨在开发一种通用的近似技术来解决这个问题。

Method: 提出了e变量近似性方法，这是一种通用的近似技术，允许为流行的分布类构造e变量。该方法基于Levin的平均有界随机性检验概念，将e变量构造与算法信息论联系起来。

Result: 开发了一种通用的e变量构造技术，能够为实际应用中重要的分布类构建e变量。同时展示了该方法可以用于为分布类提供显式的随机性检验构造。

Conclusion: e变量近似性方法为解决实际应用中分布类的e变量构造问题提供了有效的通用技术，并将e变量与算法信息论中的随机性检验概念联系起来，为两个领域建立了桥梁。
...
### [15] [Functional Properties of the Focal-Entropy](https://arxiv.org/abs/2603.02533)
*Jaimin Shah,Martina Cardone,Alex Dytso*

Main category: cs.IT

TL;DR: 本文从信息论角度系统研究了focal-loss，建立了focal-entropy的理论框架，揭示了其在类别不平衡问题中的工作机制和局限性。


<details>
  <summary>Details</summary>
Motivation: 尽管focal-loss在类别不平衡分类问题中已被广泛使用并取得经验成功，但缺乏系统性的信息论研究。本文旨在填补这一理论空白，从分布视角深入理解focal-loss的工作原理。

Method: 采用分布视角，研究focal-entropy（focal-loss的交叉熵类似物）。分析其有限性、凸性和连续性条件，提供各种渐近特征描述。证明focal-entropy最小化器的存在性和唯一性，描述其结构。

Result: 证明了focal-loss会放大中范围概率，抑制高概率结果，在极端类别不平衡情况下会引发过度抑制机制，使非常小的概率进一步减小。focal-entropy最小化器可能与数据分布显著不同。实验结果验证了理论分析。

Conclusion: 研究为理解focal-loss提供了理论基础，阐明了在应用于不平衡学习任务时引入的权衡关系，揭示了其工作机制和潜在局限性。
...
### [16] [Semantic Forwarding and Codebook-Enhanced Model Division Multiple Access for Satellite-Terrestrial Networks](https://arxiv.org/abs/2603.02536)
...
TL;DR: 提出了一种面向卫星-地面网络的语义转发语义通信框架，通过联合语义编码调制、卫星语义转发和信道感知重建，在低信噪比下实现鲁棒高效的语义传输。
...
Method: 1) 提出向量量化联合语义编码调制方案，联合优化语义编码器和语义码本以塑造星座符号分布；2) 引入卫星语义转发机制，使中继卫星能在语义层面直接转发信号而无需完全解码和重新编码；3) 设计基于特征线性调制(FiLM)的信道感知语义重建方案，融合接收信噪比与语义特征；4) 提出码本分割增强的模型分割多址接入(CS-MDMA)方法支持多用户接入。

Result: 仿真结果表明，所提出的SFSC框架在低信噪比下比现有基准方法实现了约7.9 dB的峰值信噪比增益，证明了其在卫星-地面网络中实现鲁棒和频谱高效语义传输的有效性。

Conclusion: 该研究提出的语义转发语义通信框架通过创新的语义编码调制、转发机制和信道感知重建技术，显著提升了卫星-地面通信在低信噪比下的性能和频谱效率，为未来卫星通信系统提供了有前景的解决方案。
...
### [17] [Large Language Model Empowered CSI Feedback in Massive MIMO Systems](https://arxiv.org/abs/2603.02686)
*Jie Wu,Wei Xu,Le Liang,Xiaohu You,Mérouane Debbah*

Main category: cs.IT

TL;DR: 提出基于大语言模型的CSI压缩反馈框架，将CSI反馈任务重构为掩码令牌预测任务，并设计基于自信息的信息论掩码选择策略


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在各个领域取得了成功，但它们在FDD大规模MIMO系统中用于高效信道状态信息压缩和反馈的潜力尚未被充分探索，而这一应用越来越重要

Method: 1) 将CSI压缩反馈任务重构为掩码令牌预测任务，使其更符合LLM的功能特性；2) 设计基于自信息的信息论掩码选择策略，在用户设备端识别并选择具有最高自信息的CSI元素进行反馈

Result: 通过这种设计，掩码令牌对应较低自信息的元素，而可见令牌对应较高自信息的元素，从而最大化LLM预测的准确性

Conclusion: 该框架利用LLM的潜力，为FDD大规模MIMO系统中的CSI反馈提供了一种新颖有效的解决方案
...
### [18] [Enhancing User Throughput in Multi-panel mmWave Radio Access Networks for Beam-based MU-MIMO Using a DRL Method](https://arxiv.org/abs/2603.02745)
*Ramin Hashemi,Vismika Ranasinghe,Teemu Veijalainen,Petteri Kela,Risto Wichman*

Main category: cs.IT

TL;DR: 本文提出了一种基于深度强化学习（DRL）的自适应波束管理方法，用于提升多面板毫米波MU-MIMO系统中的用户吞吐量并降低延迟。
...
Method: 采用深度强化学习框架，将通信代理与环境交互建模为马尔可夫决策过程（MDP）。利用不同天线面板间波束的互相关、测量的参考信号接收功率（RSRP）和波束使用统计等空间域特征，动态调整波束成形决策。

Result: 数值结果显示，与基线（传统波束管理）相比，吞吐量提升高达16%，延迟降低3-7倍。

Conclusion: 提出的DRL方法能有效优化毫米波MU-MIMO系统的波束管理，显著提升频谱效率并降低端到端延迟。
...
### [19] [Shatter Throughput Ceilings: Leveraging Reflection Surfaces to Enhance Transmissions for Vehicular Fast Data Exchange](https://arxiv.org/abs/2603.02752)
*Qianyao Ren,Qingxiao Huang,Yiqin Deng,Xianhao Chen,Phone Lin,Yuguang Fang*

Main category: cs.IT

TL;DR: 提出反射增强传输框架，利用道路旁镜面反射面集中多方向信号，突破单车吞吐量上限，提升空间多样性和信道秩


<details>
  <summary>Details</summary>
Motivation: 智能移动性快速发展需要车辆在短时间内通过单个链路传输大量突发数据，但传统通信系统在带宽有限场景下面临单车吞吐量上限，即使分配所有时频资源也无法突破信道空间多样性不足的限制

Method: 提出反射增强传输框架：1) 在道路旁部署专用镜面反射面，主动增强传输环境，类似凹面放大镜的光聚焦效应；2) 引入反射面板组动态虚拟化方法，兼容现有通信系统，灵活管理与其他用户的干扰；3) 引入多反射面板协同旋转机制增强信号集中

Result: 通过3GPP兼容的系统级仿真验证，该方法能显著提升单车吞吐量，突破传统系统的吞吐量上限

Conclusion: 反射增强传输框架通过主动增强传输环境，集中多方向信号，有效提升信道空间多样性和秩，显著提高单车吞吐量上限，为智能移动性提供高效数据传输解决方案
...
### [20] [An Optimization-Based User Scheduling Framework for Multiuser MIMO Systems](https://arxiv.org/abs/2603.02998)
*Victoria Palhares,Christoph Studer*

Main category: cs.IT

TL;DR: 提出一种用于多用户MIMO系统的用户调度框架，通过近似求解非凸优化问题实现全局资源分配，支持多种目标函数和资源控制，在毫米波和sub-6GHz系统中优于现有算法


<details>
  <summary>Details</summary>
Motivation: 现有用户调度方法多采用贪心算法，无法全局处理资源分配问题。在拥塞场景中，用户调度对减轻用户间干扰至关重要，需要更优的全局调度方案

Method: 提出灵活的用户调度框架，通过近似求解非凸优化问题来确定每个资源时隙中应同时传输的用户子集。支持多种目标函数（如均方误差、容量、可实现和速率），并能精确控制用户占用资源的最小和最大数量

Result: 在毫米波大规模多用户MIMO和sub-6GHz无蜂窝大规模多用户MIMO系统中验证了框架的有效性，性能优于现有调度算法，接近穷举搜索的性能

Conclusion: 该用户调度框架为多用户MIMO系统提供了一种灵活且高效的全局资源分配解决方案，能够显著提升系统性能，特别是在高密度用户场景中
...
### [21] [Channel-Adaptive Edge AI: Maximizing Inference Throughput by Adapting Computational Complexity to Channel States](https://arxiv.org/abs/2603.03146)
*Jierui Zhang,Jianhao Huang,Kaibin Huang*

Main category: cs.IT

TL;DR: 提出了一种用于6G网络中边缘推理的集成通信与计算框架，通过建立端到端推理精度分析模型，设计信道自适应AI算法来最大化推理吞吐量。


<details>
  <summary>Details</summary>
Motivation: 集成通信与计算（IC²）是6G网络中实现高效边缘推理的新范式，但缺乏可处理的端到端推理性能理论框架，现有方法难以同时考虑信道失真和AI模型架构与计算复杂度。

Method: 1) 提出端到端推理精度分析模型，使用von Mises混合分布表征高维特征分布；2) 基于该模型设计信道自适应AI算法，联合优化发送端特征压缩和接收端模型复杂度；3) 在延迟和精度约束下最大化边缘处理率。

Result: 实验结果表明，所提算法相比固定复杂度方法具有优越性能，能够根据信道条件自适应调整特征压缩和模型复杂度，实现更高的推理吞吐量。

Conclusion: 成功建立了IC²的端到端推理性能理论框架，提出的信道自适应AI算法实现了完整的IC²集成，为6G边缘推理系统设计提供了有效解决方案。
