...
Motivation: 研究2021年提出的秩度量Reed-Muller码的解码问题，特别是在二进制情况下的高效解码算法。

Method: 采用递归方法，灵感来自汉明度量二进制Reed-Muller码的Plotkin "(u|u+v)"结构解码，开发了针对特定子类的秩度量Reed-Muller码的递归解码算法。

Result: 提出的递归算法在特定子类上具有比基于Dickson矩阵的通用算法更优的渐近复杂度，并提出了通用的类似Plotkin构造的秩度量码构造方法。

Conclusion: 成功开发了高效的秩度量Reed-Muller码解码算法，并建立了秩度量码的Plotkin-like构造框架，为秩度量码的解码和构造提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 可移动天线有潜力进一步提升ISAC系统的性能增益，但在近场球面波传播环境下实现这些增益面临挑战。

Method: 提出了一种交替优化算法，同时优化感知接收组合器、通信预编码矩阵、感知发射波束形成器和用户可移动天线的位置。

Result: 仿真结果表明，在近场ISAC系统中使用可移动天线相比固定天线系统具有显著性能优势；当较大权重分配给靠近基站的用户时获得最高WSR；感知性能比通信性能更易受最小感知SINR阈值影响。

Conclusion: 可移动天线在近场ISAC系统中能提供实质性性能提升，且系统设计需要考虑用户位置权重分配和感知性能对阈值的敏感性。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法需要简化的信道模型来支持快速优化，但这些模型往往是粗略近似，而更准确的描述需要复杂仿真或测量。需要一种能绕过显式信道建模的快速优化方法。

Method: 采用AI辅助退火朗之万动力学(ALD)，通过MAP估计框架设计深度展开ALD算法，利用DNN估计分数梯度，结合零阶梯度和主动学习的训练方法。

Result: 在受丰富散射效应影响的RIS辅助场景中，该方法实现了快速可靠的信道参数调谐，具有有限的延迟。

Conclusion: AI辅助ALD方法能够在复杂动态变化环境中实现快速信道优化，无需显式信道建模。
...
<details>
  <summary>Details</summary>
Motivation: 低地球轨道星座中的太赫兹卫星间链路虽然能提供前所未有的感知精度，但受到硬件损伤、指向误差和网络干扰的基本限制，需要系统性的性能分析框架。

Method: 开发了网络克拉美-罗下界框架，结合动态拓扑、硬件质量因子、相位噪声和协作效应，通过递归费舍尔信息分析来评估性能边界。

Result: 研究发现：硬件和相位噪声导致功率无关的性能上限和下限；干扰系数可实现机会性感知，增益达5.5dB；适当建模的测量相关性不会降低性能，反而提供共模抑制优势。

Conclusion: 实现亚毫米级测距需要硬件、振荡器和3D几何配置的协同优化，具体条件为硬件质量因子小于0.01，相位噪声小于10^{-2}。
...
<details>
  <summary>Details</summary>
Motivation: 虽然侵入式记录已成功解码与抓握类型规划和执行相关的局部神经活动，但非侵入性脑电图捕捉的宏观神经激活模式仍知之甚少。

Method: 引入新型视觉抓握平台，使用FBCSP技术提取各阶段区分性频率特征，SVM分类器进行二元和多元分类，并与传统MRCP方法比较。

Result: 低频振荡在规划和执行阶段均携带抓握相关信息，精度vs力量抓握分类准确率达75.3-77.8%；高频活动在抓握vs无抓握分类中达93.3%准确率。

Conclusion: 这项工作证明了低频振荡在使用非侵入性EEG解码规划阶段抓握类型中的作用。
...
<details>
  <summary>Details</summary>
Motivation: 随着6G通信系统向智能化和高可重构性发展，传统固定天线的局限性日益突出，GMA和ERA成为突破这一瓶颈的关键技术。但它们的集成形成的"几何-电磁双重构"范式给信号处理带来了高维混合优化的严峻挑战。

Method: 首次整合GMA的几何优化和ERA的电磁重构，提出MARA的统一建模框架，研究GMA、ERA和MARA的信道建模和频谱效率优化，并系统回顾基于AI的解决方案。

Result: 填补了现有文献在几何-电磁双重构下AI驱动信号处理范式缺乏全面综述的空白，为具有高频谱效率和灵活性的6G无线系统设计与优化提供了理论支持。

Conclusion: MARA框架成功整合了几何和电磁维度的自由度，AI方法在高维非凸优化计算中优于传统算法，为6G无线通信系统的智能化和高可重构性发展提供了重要支撑。
...
<details>
  <summary>Details</summary>
Motivation: 现有的SAF算法在广义高斯噪声环境下性能下降，在脉冲噪声下存在显著的稳态失配，且未能有效处理异常值的不利影响。

Method: 提出了SAF-GMBZ算法，通过引入广义修正Blake-Zisserman鲁棒函数来增强对噪声的鲁棒性，并基于此进一步开发了FcGMBZ算法用于主动噪声控制。

Result: SAF-GMBZ在广义高斯噪声环境下表现出优越的学习性能，理论稳态均方误差分析得到验证，FcGMBZ在脉冲噪声环境下的主动噪声控制应用中表现出有效性。

Conclusion: SAF-GMBZ算法有效解决了传统SAF算法在噪声环境下的局限性，FcGMBZ算法在主动噪声控制应用中具有良好的收敛精度和噪声消除能力。
...
<details>
  <summary>Details</summary>
Motivation: VANET中不同优先级的数据需要差异化服务，802.11p协议在非关键数据开始传输时，关键数据必须等待，这影响了紧急消息的及时传输。

Method: 通过模拟评估802.11p和FROG-MAC在延迟和吞吐量方面的性能。FROG-MAC将普通数据包分片传输，在分片间隔中允许紧急数据包访问信道。

Result: 模拟结果显示FROG-MAC在高优先级和低优先级数据的延迟和吞吐量方面都优于802.11p，因为其为紧急流量提供了更早的信道访问。

Conclusion: FROG-MAC协议通过分片传输机制有效解决了VANET中紧急数据访问延迟问题，性能优于标准的802.11p协议。
...
<details>
  <summary>Details</summary>
Motivation: 高中教师因工作压力大和多任务期望，经常面临睡眠质量和认知功能问题，这对教学能力产生负面影响，需要研究改善方法。

Method: 使用嵌入脉搏率和血氧传感器的智能手表监测睡眠质量（分为差、一般、好三级），并结合认知评估问卷进行自我评估。

Result: 研究发现大多数教师睡眠质量和认知功能较差，两个变量之间存在显著关联。
...
<details>
  <summary>Details</summary>
Motivation: 激光粉末床熔融增材制造过程中存在激光能量下降等异常，需要开发低功耗的边缘计算框架进行实时异常检测。

Method: 使用光电二极管监测技术捕捉打印过程中的等离子体和红外辐射变化，构建脉冲神经网络模型，并在CPU、FPGA和Intel Loihi神经形态芯片上实现算法。

Result: 通过调整脉冲神经网络的脉冲延迟，有效减少了噪声对监测信号的掩盖，提高了异常检测的准确性。

Conclusion: 证明了在增材制造中使用低功耗神经形态芯片进行边缘异常检测的可行性，并为该过程创建了框架。
...
<details>
  <summary>Details</summary>
Motivation: 解决AI与IoT集成中多边缘协同推理系统的隐私和带宽限制问题，避免固定压缩比传输特征导致的通信资源利用效率低下。

Method: 采用率自适应量化方案动态调整特征提取码率，基于特征对下游推理任务的重要性；使用动态规划方法在有限带宽约束下为每个边缘设备选择码率。

Result: 在多视图数据集上的实验表明，该框架显著优于固定率量化框架，在有限带宽条件下实现了通信效率和推理性能的良好平衡。

Conclusion: 提出的动态码率调整框架有效解决了多边缘协同推理系统中的通信效率问题，为AI驱动的任务提供了高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究5G铁路系统在高速铁路环境中的窄波束信道特性，为5G-R系统的设计和优化提供指导。

Method: 建立了三种典型高速铁路场景，采用射线追踪仿真和设计的波束跟踪方案进行动态窄波束信道模拟，使用Vienna 5G仿真器和硬件在环平台评估系统性能。

Result: 分析了路径损耗、阴影衰落、衰落严重程度、时频空色散和平稳区间等信道特性，评估了误块率、吞吐量、频谱效率等系统性能指标。

Conclusion: 研究结果为高速铁路环境下5G-R系统的设计和优化提供了有价值的指导。
...
TL;DR: 提出了一种用于6G高速移动场景的延迟-多普勒域信道探测方法，包括波形设计、同步和信道扩展函数估计，并通过车对基础设施场景验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 在6G系统中，传统时域或频域信道探测方法无法直接获取高速移动场景下的多普勒信息，而延迟-多普勒域信道探测方法仍未被充分探索。

Method: 设计了探测信号的波形，详细阐述了延迟-多普勒域信道探测的方法论，包括同步和信道扩展函数估计，并提出了提高测量精度的算法。

Result: 建立了适用于6G高速移动场景的延迟-多普勒域信道探测系统，在城市环境下车对基础设施场景进行了信道测量，获得了信道扩展函数、功率延迟分布、多普勒功率谱密度等多路径特性。

Conclusion: 所提出的方法有效，为推进6G高速移动通信研究提供了有益见解。
...
<details>
  <summary>Details</summary>
Motivation: 研究5G-NR网络中无人机定位的安全漏洞，特别是针对现有3GPP定位框架中未充分考虑的安全威胁，如合并峰值欺骗攻击，并提出相应的防护措施。

Method: 通过理论建模和敏感性分析量化欺骗攻击成功概率，提出轻量级UE辅助定位减少开销，设计基于LMF的网络端异常检测框架，结合递归梯度下降的鲁棒定位算法过滤异常数据。

Result: 揭示了当前3GPP定位框架在同步质量和几何因素方面的根本弱点，验证了优化和安全机制在3GPP兼容无人机定位中的有效性。

Conclusion: 提出了一个统一的框架，同时提供鲁棒的受害者定位和欺骗者定位能力，填补了现有文献中的空白，并通过广泛仿真验证了其有效性。
...
<details>
  <summary>Details</summary>
Motivation: 研究AFDM和OTFS波形对被动窃听者的鲁棒性，假设窃听者不知道啁啾参数或时延多普勒网格配置，必须穷举搜索解调矩阵。

Method: 分析窃听者暴力解调的复杂度，并通过误码率仿真验证分析结果。

Result: 分析显示暴力解调复杂度OTFS为O(√N)，AFDM为O(N²)，误码率仿真表明AFDM信号几乎无法被窃听者解码，而OTFS在同等条件下允许部分信号恢复。

Conclusion: AFDM在对抗被动窃听方面优于OTFS，具有更强的安全性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 局部场电位的响应调谐通常比尖峰调谐更弱和更宽，因此需要选择优化的调谐方法来准确评估LFP响应并与邻近的尖峰活动进行比较。

Method: 首先分析灵长类初级视觉皮层记录的亮度对比度诱发的LFP响应，使用单调性指数区分超饱和、线性和饱和CRF，然后通过MLP神经网络、RBF神经网络、模糊模型、神经模糊模型和LOLIMOT算法等静态识别方法识别超饱和记录数据。

Result: MLP神经网络相比传统和改进的双曲Naka-Rushton函数，在调谐局部场电位对亮度对比度刺激的响应方面表现出更优越的性能，成功调谐了所有三种类型的显著更多神经记录。

Conclusion: MLP神经网络模型可以作为测量神经元群体对比敏感度调谐曲线的新方法，比其他当前使用的模型提供更好的拟合效果。
...
Method: 集成STAP目标检测跟踪、MUSIC高分辨率角度估计和自适应频谱滤波技术，建立微多普勒能量变化与生理活动的数学关系

Result: 可在5米范围内准确检测和分离最多4个目标的生命体征，呼吸率和心率平均绝对误差分别为1.2次/分钟和2.3次/分钟

Conclusion: 该方法在具有环境噪声和运动的多目标场景中表现优于传统相位方法，提供固有的相位噪声和运动伪影鲁棒性
...
<details>
  <summary>Details</summary>
Motivation: 研究心电图和阻抗心电图的生物识别潜力，特别关注这些特征在情感变化下的区分能力和鲁棒性。

Method: 使用随机森林模型结合多种可解释性方法评估29个ECG和ICG特征，包括时间、幅度、斜率和形态四个域，并采用特征重要性分析、相关性分析和特征选择方法。

Result: ECG特征特别是QRS相关描述符在重要性排名中占据最高位置，特征选择将特征集从29个减少到14个，准确率仅下降1%（达到99%），证实了QRS特征对情感变化的鲁棒性。

Conclusion: 可靠的生物识别个体性主要编码在QRS相关的ECG特征中，这些特征对情感变化具有弹性，可能源于心室质量、传导通路和胸腔几何的个体差异。
