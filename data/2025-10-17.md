...
Motivation: 传统固定天线系统无法实现辐射元件的动态定位和激活，限制了无线网络的灵活性和覆盖能力。夹持天线系统通过创建可按需定位的局部辐射点，解决了这一限制。

Method: 提出了广义夹持天线系统概念，可在多种物理实现中应用，包括介质波导、漏泄同轴电缆、表面波导结构等，采用机械、电子或混合激活机制。
...
Method: 提出基于混合消息传递的框架，通过多个近似来降低计算复杂度，处理用户信道与目标位置之间的复杂依赖关系。
...
<details>
  <summary>Details</summary>
Motivation: 随着数据流量需求增长放缓，移动行业不能仅依赖通信服务来维持发展，需要新的解决方案来扩展网络功能。

Method: 通过在通信网络中嵌入感知能力，实现集成感知与通信（ISAC），将网络从单纯的数据传输平台转变为支持多样化应用的多功能平台。

Result: ISAC技术为无线网络提供了新的发展方向，能够支持多种应用场景，扩展网络的功能范围。

Conclusion: ISAC代表了无线网络设计的范式转变，为研究人员提供了新的研究方向和机遇，需要关注关键挑战和应用场景来推动该领域的未来发展。
...
<details>
  <summary>Details</summary>
Motivation: 利用ZP辅助的时域信道矩阵下三角结构，设计低复杂度的接收机检测器，在保持性能的同时降低计算复杂度。

Method: 提出基于矩阵下三角结构的MMSE检测器和MRC-TD检测器，并分析MMSE和最大似然检测器的理论BER性能。

Result: 仿真结果表明，所提出的检测器能够达到与传统基于矩阵求逆的MMSE检测器相同的BER性能，同时显著降低复杂度。

Conclusion: ZP-AFDM系统可以通过利用信道矩阵结构实现低复杂度高性能的接收机设计。
...
Method: 设计三混合波束成形优化框架，结合数字、模拟和电磁波束成形，使用分数规划和流形优化方法将问题转化为可处理的子问题。

Result: 仿真结果显示，相比传统全向天线混合波束成形系统，提出的ERA-ISAC系统实现了近10dB的感知与通信性能增益。

Conclusion: 电磁可重构天线能够有效增强ISAC系统的自由度，显著提升感知和通信性能。
...
<details>
  <summary>Details</summary>
Motivation: 为光学显微镜、电子显微镜、RADAR、天文成像、生物医学成像、遥感等不同计算传感应用领域的研究人员创建交流论坛，促进跨领域知识分享。

Method: 在欧洲中心举办为期3天的研讨会，邀请6位主题演讲者，并征集科学报告和展示演示的扩展摘要。

Result: 建立了一个跨学科的计算传感交流平台，让来自看似不相关应用领域的研究人员能够相互学习和发现新发现。

Conclusion: ISCS成功地为计算传感领域的研究人员提供了有价值的交流机会，促进了不同应用领域之间的知识交换与合作。
...
<details>
  <summary>Details</summary>
Motivation: 当前流体天线的理论分析往往基于不切实际的假设（如瞬时重配置、完美信道知识等），这会导致在实际应用中高估性能增益，因此需要重新审视这些假设并考虑现实约束。

Method: 通过解构常见假设，对比理论假设与现实约束（有限响应时间、不完美CSI、快速衰落、电磁耦合等），使用示例和仿真展示忽略这些因素的影响，并提出建模改进、实验验证方法和新兴控制算法。

Result: 研究显示忽略实际约束会高估容量、覆盖等性能增益，而考虑现实因素后的系统设计能更准确地评估流体天线的真实潜力。

Conclusion: 流体天线在B5G/6G和物联网应用中仍极具前景，但只有将实际约束纳入系统设计和性能评估，才能充分发挥其全部潜力。
...
Method: 利用Sherman-Morrison公式、低秩SVD近似和代数操作，专门针对信号低于噪声底的场景进行优化。
...
<details>
  <summary>Details</summary>
Motivation: 未来智能网络需要感知所有传输信号而不仅是最强信号，但强同信道干扰会淹没目标信号，导致信号无法使用。

Method: 基于最大似然的跨前导码估计框架，利用波束扫描同步信号中载波频率偏移的恒定性，相干聚合多个观测信息来增强目标信号。

Result: Cramer-Rao下界分析和仿真表明，即使信号比干扰弱千倍以上，仍能实现可靠估计。低空无线电地图案例研究验证了框架的实际有效性。

Conclusion: 该框架能够有效解决强干扰环境下的弱信号估计问题，为智能网络的无线环境感知提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 符号间干扰(ISI)在多种信道中普遍存在，会导致时间色散和噪声着色。传统的均衡方法会带来有色噪声问题，需要有效的解码方案来处理这种有色噪声。

Method: 开发了ORBGRAND-AI解码器，基于统计物理学中的近似独立性原理。通过放弃交织，该方法能够处理ISI信道中的有色噪声，并评估了延迟抽头模型和RFView信道在不同信道状态信息假设下的性能。

Result: ORBGRAND-AI在相同信息比特能量下，能够提供与最先进的软输入解码器(如CA-SCL)相同或更低的误块率，特别是在双抽头dicode ISI信道和RFView信道中表现良好。

Conclusion: ORBGRAND-AI是一种有效的解码方法，能够在不使用交织的情况下处理ISI信道中的有色噪声，二阶自回归模型能够充分表示RFView信道效应。
...
<details>
  <summary>Details</summary>
Motivation: 分析多层感知机各层间信息流动对于理解神经网络工作机制至关重要，特别是在监督学习背景下需要理解信息是如何被处理和优化的。

Method: 从信息论角度构建问题框架，提出信息矩阵概念作为形式化工具，用于分析优化策略的病因学和研究信息流动。

Result: 提出了参数化优化策略，发现信息瓶颈框架中的优化策略与信息矩阵推导的策略具有强相似性，揭示了多层感知机作为"适配器"处理输入以达成给定目标的功能。

Conclusion: 多层感知机在监督学习中起到信息适配器的作用，信息矩阵为理解神经网络优化和信息流动提供了有效的理论框架，且与信息瓶颈方法存在重要联系。
...
<details>
  <summary>Details</summary>
Motivation: 传统防御机制与当前低延迟流水线不匹配：检测重传增加RTT，字节流ECC忽略代数结构并强制格式转换。

Method: 构建两种互补方案：针对奇数长度使用Hensel提升BCH理想和幂等编码器；针对2的幂次长度使用重复根负循环码和导数式解码。通过环自同构提供原地交织以分散突发错误。

Result: 在四个帧大小上，在符号错误率10^-6-10^-5时达到每帧失败目标10^-9，仅产生0.20%-1.56%开销，容忍约32-72字节未知错误突发（标记为擦除时约翻倍）。
...
<details>
  <summary>Details</summary>
Motivation: 现有工作忽视了RIS辅助系统中信道估计的固有困难以及实际部署中的离散相位约束，特别是对于具有功率收集功能的零功率有源RIS系统。

Method: 设计新型单元式RIS架构提供动态单元选择能力，基于菲涅尔衍射理论构建空间域位置到相位域波分布的映射，提出具有确定-对齐相位的分布式波束成形设计。

Result: 渐近分析表明当RIS较大时，该方案能以固定比例的反射单元实现最优增益，仿真验证了其相对于其他协议的优越性。

Conclusion: 所提出的分布式位置辅助传输方案有效解决了零功率有源RIS系统的信道估计难题，在降低估计开销的同时实现了优异的性能增益。
...
<details>
  <summary>Details</summary>
Motivation: 多用户交互式VR应用对延迟、能效和计算资源有严格要求，特别是在分布式移动边缘计算网络中面临挑战。

Method: 使用概率模型联合表示物理空间和虚拟空间，将资源部署建模为多目标组合优化问题，提出MO-CMPO算法结合监督学习和强化学习，利用稀疏图神经网络生成帕累托最优解。

Result: 在真实基站数据集上的仿真显示，MO-CMPO在超体积性能上表现优异，推理延迟显著低于基线方法，并揭示了延迟导向和能耗导向的不同部署模式。
...
<details>
  <summary>Details</summary>
Motivation: 为可重构智能表面开发轻量级索引调制方案，避免复杂的RIS侧波束成形，实现简单的信道估计和可扩展的频谱效率。

Method: 将N单元RIS划分为不相交组，每组采用预定义二进制反射模式生成不同的信道签名，信息嵌入在签名索引中传输。

Result: 推导了误码概率闭式上界和容量分析，揭示了分集阶数为n_R，编码增益与N成正比。瑞利衰落下的仿真验证了理论分析，且空间相关性在低频谱效率下能提升性能。

Conclusion: RIS-CSM方案实现了轻量级索引调制，具有简单的信道估计和可扩展的频谱效率，空间相关性在特定条件下能改善系统性能。
...
<details>
  <summary>Details</summary>
Motivation: 虽然给定长度的不等价码总数已有研究，但维度随长度变化的情况尚未被考虑，本文旨在填补这一空白。

Method: 通过推导q-二项式系数和的精确渐近表达式，分析线性码在三种标准等价关系下的渐近数量。

Result: 获得了固定字母表大小和增长长度下线性码等价类数量的显式渐近公式，并建立了与离散高斯分布的概率联系。

Conclusion: 研究不仅解决了该领域的开放问题，还为线性码的渐近计数提供了概率解释，揭示了与布朗运动的深刻联系。
...
Result: 仿真结果表明，所提出的可旋转天线方案在阵列增益方面显著优于传统的固定方向天线和全向天线方案。

Conclusion: 通过利用天线旋转带来的新自由度，可旋转天线技术能够有效提升波束形成性能，突破传统天线阵列的限制。
...
<details>
  <summary>Details</summary>
Motivation: 大天线阵列和宽信号带宽下，高分辨率ADC成本高、功耗大，因此需要设计低分辨率ADC的毫米波系统

Method: 开发两种信道估计器：纯无源RIS的级联信道估计器，以及利用半无源RIS射频链信息的分离信道估计器

Result: 数值结果表明，所提方法优于纯数字方法，能接近无限分辨率ADC系统性能，且训练开销小

Conclusion: 基于任务量化的信道估计设计在有限比特分辨率约束下能有效提升RIS毫米波系统性能
...
<details>
  <summary>Details</summary>
Motivation: 研究自适应速率码集合的渐近性能，特别是探索空间耦合技术如何帮助接近信道容量极限。

Method: 使用密度进化方法分析原型图空间耦合LDPC码作为内码的MacKay-Neal码集合，通过定义的并行信道模型计算置信传播解码阈值。

Result: 空间耦合MN码集合在整个[0,1]速率范围内性能优异，距离二进制输入AWGN信道容量仅0.15dB。

Conclusion: 空间耦合技术显著提升了MN码集合的性能，使其能够在全速率范围内接近信道容量极限。
...
<details>
  <summary>Details</summary>
Motivation: 为高速无线/光通信链路提供固定块长度下的速率自适应编码方案，解决传统编码方案在速率灵活性方面的限制。

Method: 采用外部分布匹配器与内层原图LDPC码结合的非线性结构，通过等效通信模型分析性能，进行密度演进和错误平层分析。

Result: 设计示例显示，使用单一LDPC码集合可在宽速率范围内实现距离香农极限1dB以内的性能，通过调整DM参数选择码率。

Conclusion: 该构造为采用二进制输入调制的高速无线/光通信链路提供了具有恒定块长度和固定内码的速率自适应解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 部分信息分解(PID)自2010年提出以来在神经科学和隐私等领域有广泛应用，但缺乏统一的理论框架，存在关键概念和技术挑战。本文旨在解决PID违反整体等于部分之和原则这一根本问题。

Method: 通过三变量系统的反例证明PID违反WESP原则，提出新的公理化框架SID来重新定义信息原子的求和规则，并分析四变量及以上系统中格结构分解的局限性。

Result: 发现PID在格结构下必然违反WESP原则，SID框架能解决三变量系统的WESP问题，但证明在四变量及以上系统中无法完全消除WESP不一致性。

Conclusion: 基于(反链)格结构的分解方法对于一般多变量系统存在固有的不充分性，需要新的理论框架来替代当前的方法。
