...
TL;DR: 本文研究量子比特态空间上的Bures-Helstrom度量的Ricci流，发现该爱因斯坦度量在归一化流下是固定点，在未归一化流下呈自相似收缩，最终在时间T=1/4时坍缩至退化度量。


<details>
  <summary>Details</summary>
Motivation: 理解Bures-Helstrom度量这一最小单调黎曼度量在Ricci流下的演化行为，探索量子信息几何与微分几何之间的深层联系。

Method: 采用旋转对称规范分析径向lapse和warping因子的耦合系统；通过Hamilton-DeTurck规范选择将问题简化为单个标量方程；求解线性受迫热方程；分析特征值谱。

Result: 发现Bures-Helstrom度量在未归一化Ricci流下呈自相似收缩g(t)=(1-4t)g_BH，标量曲率6/(1-4t)，灭绝时间T=1/4；在体积归一化流下为固定点，线性化算子为Δ+3，谱σ_ℓ=-(ℓ-1)(ℓ+3)，谱隙为5。

Conclusion: Bures-Helstrom度量在Ricci流下保持单调性直到灭绝时刻，最终通过坍缩极限离开非退化黎曼度量锥；该工作揭示了量子态几何与Ricci流理论的深刻联系。
...
TL;DR: 本文针对BLE方向定位在室内多径环境下精度下降的问题，提出了基于机器学习的高效LOS/NLOS信道分类方法。通过构建数据采集装置获取带标签的IQ特征数据，采用分位数标准化、PCA和AKDE分析特征，并利用Nyström核近似（NKA）构建低秩非线性特征映射，结合轻量级SVC分类器实现信道分类。实验表明NKA相比原始基线提升约7-14%准确率，在训练复杂度、推理成本和内存占用之间取得良好平衡。


<details>
  <summary>Details</summary>
Motivation: BLE方向定位在室内工业场景中具有应用潜力，但多径环境下反射和散射会导致角度估计偏差。虽然宽带无线电的LOS/NLOS检测已有较好研究，但BLE方向定位领域仍缺乏窄带信道特征表示、可扩展的核方法特征变换以及专用数据集。

Method: 1) 构建受控BLE测量平台，在两种不同传播环境中采集带标签的LOS/NLOS数据；2) 采用基于分位数的鲁棒标准化处理IQ特征；3) 使用PCA和自适应核密度估计（AKDE）分析特征的可分离性；4) 应用Nyström核近似（NKA）构建低秩非线性特征映射，配合轻量级SVC分类器；5) 与随机森林（RF）和多层感知机（MLP）进行对比；6) 利用校准后验概率进行成本感知的阈值选择。

Result: NKA方法相比原始基线提升约7-14%的准确率。MLP在绝对准确率上略高，但Nyström-SVC方法在训练复杂度、推理成本和内存占用方面具有更优的权衡。

Conclusion: 所提出的Nyström-SVC方案是一种资源友好的BLE信道分类方法，适合部署于资源受限的实时定位系统，为工业室内定位提供了一种实用的LOS/NLOS检测方案。
...
TL;DR: 本文发现旋转驱动线性非平衡稳态中，对称信息与熵产生之间的紧等式被宇称选择规则禁止。当弛豫和扩散矩阵对易时，快照互信息在驱动反转下精确为偶函数，宇称破坏与对易子范数线性相关。完全各向同性条件下互信息为约0.145 nats的封闭值，熵产生严格为驱动的二次型。该规则仅依赖于漂移的旋转对称性，并在重尾稳定噪声下仍然成立。


<details>
  <summary>Details</summary>
Motivation: 探索非平衡稳态中对称信息与熵产生之间的紧等式关系，解决该领域长期存在的理论空白。

Method: 通过宇称选择规则分析旋转驱动线性非平衡稳态，考虑弛豫矩阵和扩散矩阵的对易关系，推导互信息和熵产生的解析表达式，并用各向同性和各向异性情况验证。

Result: 发现宇称选择规则禁止对称信息与熵产生之间的紧等式；互信息在驱动反转下精确偶对称；完全各向同性时互信息为0.145 nats的封闭值；熵产生严格为驱动二次型；规则在重尾稳定噪声下仍成立。

Conclusion: 提出了基于旋转对称性的宇称选择规则，揭示了非平衡稳态中对称信息与熵产生的基本限制，为热力学不确定性关系提供了新的边界理解，并通过电路实验提出了可验证的检验方案。
...
TL;DR: 本文提出了Doeblin曲线（Doeblin curve）这一概念，作为Doeblin系数的细粒度推广，能够在Doeblin系数为零时仍提供非平凡的信息收缩保证，并应用于噪声迭代优化、噪声电路可靠计算和差分隐私等多个领域。


<details>
  <summary>Details</summary>
Motivation: 传统Doeblin系数在多路信息收缩分析中需要严格条件（如远离0），当其为零时无法提供有效收缩保证。现有研究缺乏一种更精细的刻画方法来捕捉Doeblin系数为零时的收缩行为。

Method: 提出Doeblin曲线的概念——一种非线性函数，用于量化马尔可夫核在不同散度水平和功率下对输入分布集合的收缩行为；给出了Doeblin系数的新变分刻画；定义了功率约束Doeblin曲线的多种版本；推导了其上下界。

Result: 建立了Doeblin曲线的多种性质，推导了基于变分刻画的上下界，并在噪声迭代优化的泛化界、噪声电路的误差界和在线迭代算法的差分隐私保证等领域展示了应用，将相关结果推广到了更广泛的域或群体设置中。

Conclusion: Doeblin曲线能揭示比Doeblin系数更精细的收缩现象，在多个领域中扩展了现有结果的应用范围。
...
TL;DR: 本文利用条件扩散模型(cDDIM)和条件流匹配模型(cFMM)生成高质量、位置条件化的MIMO信道数据，以解决真实信道测量成本高昂的问题。实验表明，cFMM在保持与cDDIM相当质量的同时，推理速度快约一个数量级，且合成数据能显著提升下游物理层任务性能。


<details>
  <summary>Details</summary>
Motivation: 真实MIMO信道数据采集需要大量实地测量，成本高昂，限制了AI原生无线网络的发展。因此需要低成本、高质量的合成信道数据生成方法。

Method: 对比两种位置条件化的生成范式：条件去噪扩散隐式模型(cDDIM)和条件流匹配模型(cFMM)，均以用户坐标为条件生成MIMO信道矩阵。在28 GHz和3.5 GHz的视距与非视距场景下评估。

Result: 两种模型均能准确捕捉站点特定信道特征，即使仅用少量真实数据训练。cFMM质量与cDDIM相当，但推理时间少一个数量级。合成数据增强可显著提升信道压缩和波束对齐等下游任务性能。

Conclusion: 位置条件化的生成模型能有效替代昂贵的信道测量，其中cFMM在质量与效率之间实现更优平衡，为AI原生无线网络中的数据稀缺问题提供了可行解决方案。
...
TL;DR: 本文提出基于离散天线选择的服务区域覆盖优化框架，解决近场捏合波导天线在无线信息与能量同时传输中对位置误差敏感的鲁棒性问题，实现速率-能量区域稳健性提升。


<details>
  <summary>Details</summary>
Motivation: 现有优化框架多为点式（单一坐标增益最大化），对定位误差和移动性高度敏感，近场信号在小位移内波动剧烈，亟需空间鲁棒设计。

Method: 提出基于离散天线选择的服务区域覆盖优化框架，将问题建模为非凸二元二次规划，以最大化EH服务区内收集能量为目标，同时满足ID服务区稳健速率约束。上界采用半定松弛（SDR），下界采用低复杂度交换式局部搜索算法。

Result: 数值结果表明，所提覆盖导向设计实现了稳健的速率-能量（R-E）权衡，在服务区域内保持稳定性能，优于点式近场优化方法。

Conclusion: 离散天线激活的覆盖优化框架有效克服了点式方法对位置误差的脆弱性，为近场SWIPT提供了空间稳健的解决方案。
...
TL;DR: 本文提出了一款工作在J波段（292 GHz）的多基地雷达芯片组，包含发射和接收MMIC，采用×16倍频链实现低频本振分配。发射机集成四路功率合成放大器，输出功率11.2 dBm，带透镜时EIRP达41 dBm；接收机集成三级低噪声放大器，转换增益43.3 dB。芯片采用130 nm SiGe BiCMOS工艺，实现了150米远距离雷达探测。


<details>
  <summary>Details</summary>
Motivation: 传统雷达架构在兼顾高发射功率和高接收灵敏度方面存在局限。多基地雷达架构可同时实现高发射功率与高接收灵敏度，因此需要设计相应的芯片组来充分发挥这一优势。

Method: 设计一款多基地J波段雷达芯片组，包含发射和接收MMIC。发射机采用四路功率合成放大器链、×16倍频链、多级滤波实现谐波抑制；接收机采用三级低噪声放大器。芯片采用130 nm SiGe BiCMOS工艺，集成贴片天线并配合PTFE透镜实现高定向性。

Result: 发射机输出功率11.2 dBm，带透镜EIRP为41 dBm（292 GHz），不带透镜为8.8 dBm。片内谐波抑制>24 dBc，辐射谐波抑制约50 dBc。接收机转换增益43.3 dB。成功实现150米远距离雷达测量。

Conclusion: 该芯片组证明了多基地J波段雷达架构在远距离探测中的可行性，通过×16倍频链、功率合成和集成天线设计，实现了高性能、可扩展的毫米波雷达系统，适用于远距离传感应用。
...
TL;DR: 本文通过互阴影面积闭合表达式估计大型智能表面（LIS）的空间自由度（DoF），验证了DoF与可形成的独立波束数量近似相等，并发现极化分量对DoF贡献不均，总场DoF是单一极化分量的两倍。


<details>
  <summary>Details</summary>
Motivation: 多用户大型智能表面系统中，电磁场需要在多个近场位置进行成形、分辨和聚焦，但缺乏对空间自由度、采样和波束成形的系统理解，尤其是闭合形式的DoF估计方法。

Method: 利用互阴影面积推导闭合DoF表达式；对线源配置开发基于单位DoF间隔的解析采样方案；对表面配置采用离散经验插值法数值确定采样点；通过最大比传输和迫零波束成形验证DoF限制；进行极化感知分析。

Result: DoF预测与数值奇异值谱的膝点高度吻合；约DoF数量的独立波束可成功形成，超出则干扰增加；极化成分对DoF贡献不均，总场DoF为单一极化分量的两倍。

Conclusion: 空间DoF是LIS系统性能的关键限制因素，波束成形数量不应超过DoF预测值，极化多样性可有效增加DoF。
...
TL;DR: 本文提出基于Radio Stripes（RS）的广义pinching天线（RS-GPA）框架，通过沿共享电缆部署有源天线处理单元（APU）实现位置灵活的无线接入，并开发了稀疏APU激活与波束成形的优化算法，显著降低总功耗。


<details>
  <summary>Details</summary>
Motivation: 传统基于介质波导的无源pinching天线依赖导波到自由空间的被动耦合，缺乏灵活性和可控性。本文旨在通过有源APU架构实现更灵活、可控且广泛频率适用的pinching天线系统。

Method: 提出RS-GPA框架，建立考虑距离相关APU-用户信道的系统模型；下行链路提出重加权群稀疏波束成形算法，并推导单用户场景下APU激活阈值；上行链路提出联合APU激活与用户功率控制问题，并设计几何引导的低复杂度算法。

Result: 相比基准方案，RS-GPA框架显著降低总功耗；几何引导算法在几乎不损失功耗性能的前提下，运行时间远低于群稀疏优化算法。

Conclusion: RS-GPA框架通过有源APU架构和智能激活策略，有效平衡了传输功耗与电路功耗，为pinching天线在实际通信系统中的应用提供了可行方案。
...
TL;DR: 本文针对实时应用中的参数化约束优化问题，提出了一种基于离线学习和在线处理的两阶段策略。通过利用参数可分离结构，设计了基于ADMM的求解流程，并借助神经网络协处理器，在资源有限的设备上实现了轻量级在线计算。


<details>
  <summary>Details</summary>
Motivation: 实时应用中的参数化约束优化问题需要在资源有限的设备上高效求解，传统方法难以同时满足低延迟和低计算复杂度的要求。

Method: 提出两阶段策略：离线阶段利用神经网络学习函数表示；在线阶段借助ADMM迭代算法和神经网络协处理器，利用参数可分离结构实现轻量级在线计算。

Result: 即使在参数变化的情况下，通过精心设计的ADMM流程，结合神经网络协处理器的辅助，可以在设备上通过轻量级在线计算高效求解参数化优化问题。

Conclusion: 所提出的离线学习+在线ADMM策略能够显著降低在线计算复杂度，适用于资源受限设备上的实时优化问题。
...
TL;DR: 本文提出了一种基于CP-OFDM的分布式声学传感（DAS）系统，利用频域信道重建方法消除传统匹配滤波脉冲压缩中存在的空间码间干扰（ISI），并在同一波形上同时实现前向通信数据恢复，首次展示了共享波形光纤集成感知与通信（ISAC）的可行性。


<details>
  <summary>Details</summary>
Motivation: 传统匹配滤波脉冲压缩DAS存在非零压缩旁瓣，导致确定性跨距离单元泄漏（空间ISI）和重建瑞利背向散射迹中的虚假响应，亟需一种无空间ISI的波形设计方法。

Method: 将分布式瑞利背向散射建模为有限记忆传感多径信道；采用数据承载的CP-OFDM波形作为探测信号；通过CP去除、单抽头频域均衡和IDFT重建每个距离单元系数，避免波形引入的确定性空间ISI。

Result: 仿真5.2 km链路中，相比匹配滤波脉冲压缩，相位迹均方误差改善达29.55 dB；外差相干实验在5.2 km光纤上以111.984 MHz带宽实现500 Hz PZT振动盲定位（相关系数0.990和0.962）；同时以零误码率和-23.14 dB中位误差矢量幅度恢复图像数据。

Conclusion: CP-OFDM频域信道重建可实现无空间ISI的DAS，并展示了共享波形光纤ISAC的潜力。
...
TL;DR: 本文提出ConsisFormer，一种基于无线信道短时一致性的计算高效Transformer设计，通过自适应令牌聚合（ATA）和特征序列插值（FSI）将WFM计算复杂度降低83%以上，同时性能几乎无损。


<details>
  <summary>Details</summary>
Motivation: 现有无线基础模型（WFM）基于Transformer架构，其计算复杂度随输入序列长度平方增长，在推理延迟约束下难以部署。亟需一种计算高效的Transformer设计来降低计算成本。

Method: 1) 提出自适应令牌聚合（ATA）模块，利用相邻时频实例共享相似散射体簇的特性，动态合并相邻CSI令牌，减少自注意力计算的序列长度；2) 提出特征序列插值（FSI）方法，从Transformer输出的稀疏特征序列恢复完整CSI表示；3) 提出聚合自编码器（AAE）预训练范式，从稀疏化CSI令牌中学习鲁棒的通道表示。

Result: 在信道预测、LoS/NLOS分类、波束预测和定位等多种任务上，将WFM的计算复杂度降低超过83%，且性能损失可忽略不计。

Conclusion: ConsisFormer通过利用无线信道的短时一致性，以令牌聚合和特征插值的方式实现了计算高效性，为WFM在实际6G网络中的低延迟部署提供了可行方案。
...
TL;DR: 本文提出了一种基于OFDM的幅度-相位-频率块调制（APFBM）方案，通过将信息符号映射到Stokes球上的Jones向量，利用公共相位自由度实现无旁瓣信息的PAPR降低和无需专用导频的信道感知，在统一波形中同时支持通信、低PAPR和感知功能。


<details>
  <summary>Details</summary>
Motivation: 现有OFDM通感一体化系统要么在时域或频域上分离通信和感知资源，要么依赖专用导频进行信道估计，导致系统灵活性受限且开销增加。亟需一种无需资源划分、无需旁侧信息即能同时实现可靠通信、低PAPR和精确感知的统一波形方案。

Method: 提出APFBM方案：将信息符号映射到Stokes球上的能量归一化Jones向量，建立每块的确定性相位参考；利用公共相位自由度设计分组相位优化算法降低PAPR（无需旁侧信息）；接收端采用基于Viterbi的最大似然序列检测算法联合恢复优化相位并估计信道。

Result: 通过数值仿真和软件无线电实测验证：有效降低PAPR，实现准确的信道感知和相位恢复，稳定的信道状态信息重构。推导了闭式的误码率和感知精度表达式。

Conclusion: 该方案以适度降低频谱效率为代价，实现了无需旁侧信息的PAPR降低和无需导频的感知，是一种统一的通感一体化波形设计。
...
TL;DR: 本文首次评估了脑电图基础模型（EEG FMs）在ICU场景下无需患者特定校准的爆发抑制（BS）检测任务中的表现。REVE-base模型在事件级F1-score达到0.868，相比EEGNet和自适应阈值方法分别减少了52.1%和36.2%的每分钟爆发误差，全微调策略效果最优，且在标注数据有限时预训练表示优势显著。


<details>
  <summary>Details</summary>
Motivation: BS模式在患者间差异大，人工标注数据集稀缺，自动检测具有挑战性。虽然EEG基础模型在下游任务中表现良好，但尚未被应用于BS检测，因此本研究旨在填补这一空白。

Method: 比较REVE-base、LUNA-large、LuMamba-Tiny三种EEG基础模型，以自适应阈值方法和EEGNet作为基线。采用传统窗口分类和事件级爆发检测两种评估方式，并进行消融实验对比全微调、冻结骨干训练、两步微调和LoRA等适应策略。

Result: REVE-base在事件级F1-score达到0.868±0.167，爆发每分钟误差比EEGNet降低52.1%，比自适应阈值降低36.2%。全微调比冻结骨干训练提升最多+0.102（LUNA-large）。在仅用25%标注数据时，预训练REVE-base比随机初始化提升+0.723 F1分数。

Conclusion: EEG基础模型能够有效应用于ICU中的爆发检测，支持可扩展的EEG监测，全微调是最佳适应策略，预训练表示在小样本场景下具有显著优势。
...
TL;DR: 本文研究了光可重构智能表面（ORIS）辅助的自由空间光（FSO）链路，并引入混合自动重传请求（HARQ）方案来提升性能。推导了端到端反射信道的统计模型，给出了HARQ-CC的闭合中断概率表达式和HARQ-IR的中断上界，分析了分集增益和时延特性，数值结果表明HARQ显著提升ORIS-FSO可靠性，HARQ-IR优于HARQ-CC。


<details>
  <summary>Details</summary>
Motivation: ORIS可以在障碍物周围创建虚拟视距路径，但实际FSO链路仍受大气湍流、指向抖动和几何损耗影响，现有研究缺乏对ORIS辅助FSO系统与HARQ联合性能的理论分析。

Method: 联合建模大气湍流、ORIS指向误差和几何衰减得到端到端信道统计模型；基于此推导HARQ-CC的闭合中断概率和HARQ-IR的中断上界；进行高信噪比分析获得分集阶数；分析截断HARQ的时延特性（平均传输轮数及给定成功解码下的条件均值）。

Result: 推导出HARQ-CC的闭合中断概率表达式和HARQ-IR的中断上界；揭示了两种方案的分集阶数；给出了平均传输轮数和条件均值的解析结果；蒙特卡洛仿真验证了理论分析的准确性。

Conclusion: HARQ能显著提升ORIS辅助FSO链路的可靠性，且HARQ-IR在中断概率和时延两方面均优于HARQ-CC，即使在少量重传轮数下效果也很明显。
...
TL;DR: 本文提出了一种混合TRP-UE感知机制，通过结合基站和用户设备的协同感知能力，在3GPP标准的ISAC信道模型下显著提升了复杂环境（如室内工厂）中被动移动目标的定位精度。


<details>
  <summary>Details</summary>
Motivation: 为了实现网络在提供通信服务的同时可扩展地感知环境，针对ISAC中被动移动目标的精确定位这一关键功能，现有基于网络（TRP）的感知性能在挑战性传播环境（如室内工厂）下存在不足。

Method: 提出了一种新型混合TRP-UE感知机制，在传统基站（TRP）感知基础上引入用户设备（UE）辅助感知，并使用3GPP兼容的ISAC信道模型进行评估。

Result: 在3GPP兼容的ISAC信道模型下，该混合机制在室内工厂等挑战性传播环境中，显著提升了网络感知性能。

Conclusion: TRP感知与UE辅助感知的互补结合，能有效改善复杂环境下的ISAC感知性能，为网络在恶劣传播条件下实现精准定位提供了可行方案。
...
TL;DR: 本文研究了一种基于捏合天线系统（PAS）的多用户下行NOMA系统，通过联合优化PA位置和基站发射预编码来最大化最小用户速率，并提出了一种两阶段结构化优化框架，显著优于启发式基准和传统MIMO-NOMA系统。


<details>
  <summary>Details</summary>
Motivation: PAS通过重新定位电介质辐射元件（PA）可克服信号阻挡，但由于每个波导由单个射频链驱动，NOMA非常适合PAS下的多用户通信，因此需要研究如何联合优化PA位置和预编码以提升系统性能。

Method: 提出两阶段结构化优化框架：第一阶段忽略PA信道相位，使用内点法进行粗PA位置和功率分配优化；第二阶段考虑相位，先通过相位归零局部调整PA位置以对齐信道相位，再使用交替前向后向PA位置细化与基于连续凸近似的复发射预编码优化迭代收敛。

Result: 仿真表明所提框架显著优于启发式优化基准，计算时间更低；相比可比MIMO-NOMA系统有较大增益；揭示了PA数量、用户数量和发射功率对系统性能的影响。

Conclusion: 该两阶段优化方法能有效解决PAS-NOMA系统中因PA间干扰导致的非光滑非凸优化问题，实现用户速率最大化。
