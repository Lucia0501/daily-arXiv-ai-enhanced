...
TL;DR: 提出IPFDDP算法，通过非参数贝叶斯方法联合估计认知雷达的感知和感知-行动策略，优于现有方法并量化不确定性


<details>
  <summary>Details</summary>
Motivation: 现有电子对抗方法假设已知认知雷达的感知或感知-行动策略之一，这在对抗环境中不现实。需要同时推断两者以进行有效的逆学习

Method: 提出IPFDDP算法：使用依赖狄利克雷过程建模感知依赖的行动策略，将核基DDP推断嵌入贝叶斯逆粒子滤波框架，联合估计感知和感知-行动策略

Result: IPFDDP在均方误差、KL散度和动作偏好识别准确率上优于现有方法。主动探测结合IPFDDP相比随机探测平均减少40%的KL散度收敛时间

Conclusion: IPFDDP能有效联合估计认知雷达的感知和感知-行动策略，量化不确定性，结合主动探测可实现样本高效的逆学习电子对抗
...
TL;DR: 基于AMD RFSoC和Sivers半导体模拟前端的毫米波硬件测试平台，支持29.8GHz的模拟波束成形通信


<details>
  <summary>Details</summary>
Motivation: 毫米波频段具有大带宽通信潜力，但由于大气衰减高，需要窄定向波束进行通信，需要开发相应的硬件测试平台

Method: 使用AMD RFSoC和Sivers半导体模拟前端构建硬件测试平台，包含基于OFDM的基带物理层和数字前端（ARM处理器和FPGA），集成高速数据转换器，并通过模拟波束成形在29.8GHz实现空口通信

Result: 实现了端到端的空口通信，并在真实无线信道中分析了射频损伤和波束失准对误码率的影响

Conclusion: 成功开发了毫米波模拟波束成形硬件测试平台，能够评估实际通信场景中的性能，为毫米波通信系统研究提供了实验基础
...
<details>
  <summary>Details</summary>
Motivation: 现有ISAC研究主要利用雷达感知进行波束识别，但调制方案选择仍依赖传统耗时反馈机制。在6G高速车载通信等应用中，需要更高效的调制方案选择方法。

Method: 提出雷达使能的调制方案选择框架，利用雷达感知信息直接选择最佳调制方案，避免传统下行-上行反馈过程。通过分析四种不同轨迹验证方法有效性。

Result: 相比现有最先进的ISAC方法，吞吐量提升54-209%，具体取决于移动用户的轨迹类型。

Conclusion: 雷达感知信息可用于高效调制方案选择，显著提升ISAC系统吞吐量，为6G高速车载通信等应用提供更优解决方案。
...
<details>
  <summary>Details</summary>
Motivation: IEEE 802.11ad标准使用模拟波束成形进行毫米波通信，但波束对准过程引入大量开销，降低整体吞吐量。需要消除这一瓶颈。

Method: 设计端到端IEEE 802.11ad ISAC基站收发器原型，将数字基带硬件前端与Simulink毫米波前端集成，雷达和通信功能共享相同波形、频谱和前端。

Result: 即使存在硬件损伤，基于802.11ad的ISAC系统比理想毫米波前端的标准系统提供34%更高的吞吐量，并分析了不同信道、目标运动、信噪比和硬件配置下的性能。

Conclusion: 提出的ISAC原型有效消除了波束对准开销，显著提升系统吞吐量，证明了集成感知与通信在毫米波系统中的实用价值。
...
TL;DR: 提出一种双层自聚焦策略，将极坐标格式算法（PFA）与后向投影算法（BPA）结合，用于汽车合成孔径雷达（SAR）成像，在保持低复杂度的同时实现高质量图像聚焦。


<details>
  <summary>Details</summary>
Motivation: 现有自聚焦算法主要针对频域SAR技术设计，而汽车应用中常使用后向投影算法（BPA）以获得更少失真的图像。目前缺乏针对时域算法的高效自聚焦解决方案。

Method: 提出双层自聚焦策略：第一层在PFA域中使用新型定位误差补偿自聚焦（LECA）处理流程估计和校正定位误差；第二层将这些校正无缝转移到BPA。同时将相位梯度自聚焦（PGA）技术扩展到BPA中以提高定位误差补偿效率。

Result: 通过真实汽车实验验证，该管道实现了最先进的图像聚焦和分辨率，为计算高效的SAR成像设定了新基准。

Conclusion: 该研究填补了时域SAR算法高效自聚焦解决方案的空白，通过PFA与BPA的结合，在保持计算效率的同时显著提升了汽车SAR成像质量。
...
TL;DR: 本文提出使用卷积Tsetlin机（CTM）在5G同步信号块（SSB）特征上进行轻量级、可解释的干扰检测，相比CNN训练更快、内存需求更少，适合边缘部署。


<details>
  <summary>Details</summary>
Motivation: 5G网络中的关键应用依赖稳定的射频环境，但面临有意干扰或低功率干扰的威胁，这些攻击可能低于链路层可观测性，需要轻量级、可解释的检测方案。

Method: 使用卷积Tsetlin机（CTM）直接在5G SSB特征上进行干扰检测。CTM基于布尔逻辑子句处理量化输入，支持比特级推理和确定性FPGA部署。在真实5G测试床上使用空中SSB数据进行实验验证，并与CNN基准进行比较。

Result: CTM在真实数据集上达到91.53±1.01%的准确率（CNN为96.83±1.19%），训练速度快9.5倍，内存需求减少14倍（45MB vs 624MB）。同时为Zybo Z7 FPGA设计了紧凑架构，并提供了延迟、功耗和准确性的权衡部署方案。

Conclusion: CTM为RF域干扰检测提供了实用、可解释且资源高效的替代方案，是边缘部署、低延迟、安全关键5G应用的强有力候选方案，并为B5G系统奠定了基础。
...
TL;DR: 该论文研究了网络化低轨卫星系统中考虑切换的功率高效下行传输问题，提出了两段式帧结构分离切换操作与用户平面传输，并开发了联合协作波束成形与隐式调度的功率优化算法。


<details>
  <summary>Details</summary>
Motivation: 低轨卫星星座通过星间链路提供无处不在的宽带非地面服务，但快速轨道运动导致频繁的调度更新和切换，同时严格的星上约束（如有限的射频链）使得用户调度与协作波束成形紧密耦合，需要解决切换感知的功率高效传输问题。

Method: 提出两段式帧结构分离切换相关操作与用户平面传输；建立包含新建立卫星-用户链路切换成本和切换期间有效传输窗口减少的功率消耗模型；使用硬化界遍历速率度量，在分段服务质量约束、每卫星功率预算和服务基数限制下，制定每帧全网功率最小化问题；开发结合重加权ℓ2代理、基于惩罚的松弛和分数规划内循环的迭代算法，产生一系列凸二阶锥规划。

Result: 基于时变轨道动力学和逐帧服务集演化的仿真表明，该方法在功率-切换权衡方面表现优异，相比非协作和预调度协作基线，实现了持续的功率节省和可行性改进。

Conclusion: 该研究为网络化低轨卫星系统提供了切换感知的功率高效传输解决方案，通过创新的帧结构和优化算法，有效解决了快速轨道运动带来的调度和切换挑战，在功率消耗和切换性能之间取得了良好平衡。
...
TL;DR: 本文针对流体MIMO系统中的发射端和接收端天线位置联合优化问题，提出基于交替优化的两种算法，以最大化遍历容量并应对贝塞尔函数带来的高度非凸优化挑战。


<details>
  <summary>Details</summary>
Motivation: 流体天线系统应用于MIMO时产生流体MIMO，在丰富散射环境下，空间相关矩阵由可调天线位置通过零阶贝塞尔函数控制，形成高度非凸的"贝塞尔景观"，存在波动局部最优，需要有效的优化方法。

Method: 首先通过Kronecker信道分解开发理论分析结果，包括高信噪比容量近似、闭式容量损失界和全局最优天线间距。然后提出两种交替优化算法：1) 结合粒子群优化的AO-PSO算法，无需梯度信息全局探索；2) 使用逐次凸近似的AO-SCA算法，通过一阶贝塞尔函数获得闭式梯度，构建凸代理函数实现快速收敛。

Result: 理论分析揭示了问题的内在结构：高信噪比容量可分解为发射和接收相关矩阵的对数行列式项；建立了与理想i.i.d. MIMO信道性能差距相关的容量损失界；确定了N=2时的全局最优天线间距。提出的AO算法能有效优化流体MIMO系统性能。

Conclusion: 本文通过理论分析和算法设计解决了流体MIMO中的天线位置优化问题，提出的交替优化框架结合粒子群优化和逐次凸近似方法，能够有效应对贝塞尔景观的非凸挑战，为流体MIMO系统性能优化提供了系统解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 6G ELAA引入显著近场效应，需要鲁棒的近场波束训练策略。由于硬件损伤导致相位信息不可靠，仅幅度信道恢复成为关键替代方案。现有近场幅度方法假设简单的视距条件，而远场相位恢复方法受限于平面波模型，不适合近场传播。

Method: 提出两阶段稀疏相位恢复框架：第一阶段在标准2D DFT波束空间上执行自适应支撑集发现，利用近场波束模式引导的物理先验；第二阶段将感知和稀疏相位恢复限制在学习的子空间内，细化信道估计。

Result: 数值结果显示，提出的自适应流水线持续优于非自适应基线，在低信噪比下将波束成形增益提高超过70%。

Conclusion: 该两阶段稀疏相位恢复框架为多径环境中的仅幅度近场波束训练提供了有效解决方案，通过自适应子空间学习显著提升了性能。
...
TL;DR: MetaSort是一种新颖的尖峰排序算法，同时解决尖峰分类和压缩问题，通过自适应电平交叉算法进行压缩，利用潜在特征表示进行分类，并通过元迁移学习利用数据几何信息。


<details>
  <summary>Details</summary>
Motivation: 许多先前的研究将尖峰分类和压缩作为独立问题处理，但实际应用中需要同时解决这两个问题以实现高效、低功耗的尖峰排序系统。

Method: 提出MetaSort算法：1) 使用自适应电平交叉算法进行高保真尖峰形状压缩；2) 利用潜在特征表示处理分类问题；3) 通过元迁移学习框架同时利用数据的几何信息，确保算法的鲁棒性和判别性。

Result: 使用体内尖峰数据的实证实验表明，MetaSort表现出有前景的性能，突显了其潜力，并激励继续开发超低功耗的片上实现。

Conclusion: MetaSort成功地将尖峰分类和压缩问题整合到一个统一框架中，通过自适应压缩、潜在特征分类和元迁移学习实现了高效、鲁棒的尖峰排序，为超低功耗的片上实现铺平了道路。
...
TL;DR: 该论文研究多卫星下行链路ISAC网络，提出集中式和分布式两种协作感知框架，通过波束成形和功率分配优化，在满足通信需求的同时显著提升感知性能。


<details>
  <summary>Details</summary>
Motivation: 研究多卫星ISAC网络，旨在同时为地面用户提供通信服务，并通过多个网关实现对空中目标的协作感知，解决通信与感知双重功能的需求。

Method: 提出基于均匀平面阵列的通信和感知波束成形设计，优化功率分配。构建集中式和分布式两种协作感知框架：集中式将感知数据发送到中央单元进行联合估计；分布式各网关独立估计后仅传输估计结果，使用匈牙利算法解决数据关联问题。

Result: 仿真结果表明，提出的集中式和分布式框架在满足通信性能要求的同时，显著优于现有感知方案。同时评估了感知精度与通信功耗之间的权衡关系。

Conclusion: 论文成功设计了多卫星ISAC网络的协作感知框架，实现了通信与感知的有效协同，提出的两种框架在不同场景下都能提供优越的性能，为ISAC系统设计提供了重要参考。
...
TL;DR: 本文提出了一种基于STAR-RIS的鲁棒安全下行传输框架，用于多用户MIMO系统，通过联合优化基站主动波束成形和STAR-RIS被动反射/传输系数，在满足多种约束条件下最大化保密速率。


<details>
  <summary>Details</summary>
Motivation: STAR-RIS技术为集成感知与通信系统提供了变革性方法，特别是在增强物理层安全方面。传统反射型RIS在安全传输方面存在局限性，而STAR-RIS能够同时反射和传输信号，为多用户MIMO系统提供更灵活的安全增强方案。

Method: 提出鲁棒波束成形优化问题，联合设计基站主动波束成形和STAR-RIS被动反射/传输系数。采用交替优化算法，结合S-procedure处理信道不确定性约束，使用半定松弛和惩罚凸凹规划获得可处理的次优解。

Result: 仿真结果表明，所提框架在严格感知条件下相比传统反射型RIS系统显著提高了频谱效率，验证了STAR-RIS在增强物理层安全方面的有效性。

Conclusion: STAR-RIS赋能的ISAC系统能够有效增强物理层安全，提出的鲁棒优化框架在存在恶意窃听者的情况下实现了安全通信和感知任务的协同优化，为未来无线网络提供了有前景的安全增强方案。
...
TL;DR: 该论文系统研究了堆叠智能超表面(SIM)中的层间误差校准问题，提出了一个集成校准协议和解决方案策略的通用框架，并探讨了多阶段校准方法在减轻几何偏差和提高校准精度方面的有效性。


<details>
  <summary>Details</summary>
Motivation: 实际SIM实现中，由于制造和组装缺陷，理论模型与实测响应之间存在明显不匹配，这影响了SIM在下一代无线网络中实现电磁波域信号处理的性能。

Method: 首先对代表性的建模和硬件引起的缺陷进行分类，然后概述SIM校准的主要挑战，开发一个集成校准协议与相关解决方案策略的通用框架，并研究多阶段校准方法在减轻几何偏差和提高传播系数对齐方面的有效性。

Result: 提出了一个系统性的SIM校准框架，多阶段校准方法能有效减轻几何偏差，提高校准传播系数与实际传播系数之间的对齐精度。

Conclusion: 该研究为未来实现物理一致和硬件兼容的SIM实现提供了关键研究机会和实际挑战的详细阐述，推动了SIM技术在下一代无线网络中的实际应用。
...
TL;DR: 提出了一种用于零能耗设备的统一框架，通过多出口点自适应推理和能量感知电路，在保持检测精度的同时显著降低能耗。


<details>
  <summary>Details</summary>
Motivation: 随着智能和自主系统的普及，边缘设备需要直接执行智能计算。这对于零能耗设备尤其具有挑战性，因为需要同时解决内存、能量可用性和推理精度的严格约束。

Method: 设计、训练和部署了一个用于人员检测的微型机器学习模型，采用多出口点架构实现自适应推理，并引入辅助能量感知电路动态调节系统运行。

Result: 相比最先进的能量感知单出口TinyML方法，该方法实现了约29.6%的能耗降低，同时保持了检测精度。

Conclusion: 该框架为ZED平台提供了准确且能量高效的智能解决方案，通过自适应推理和能量感知电路的结合，在严格约束下实现了性能优化。
...
TL;DR: 提出一种基于OFDM和AFDM波形正交性的共生无线电系统，实现主通信信号与反向散射通信信号在同一频谱内无干扰共存，同时支持高分辨率传感。


<details>
  <summary>Details</summary>
Motivation: 传统集成传感与反向散射通信系统存在直接链路干扰和反向散射设备间干扰问题，需要设计一种能够在同一频谱内实现信号域无干扰共存的系统。

Method: 利用OFDM和AFDM信号的波形域正交性，采用基于啁啾的AFDM波形，将反向散射传输嵌入仿射域，同时保持可靠的OFDM主通信。通过为每个反向散射设备分配不同的仿射域偏移来抑制设备间干扰。

Result: 仿真结果表明，该方案能有效抑制干扰而不影响主链路的误码率，并改善了反向散射通信的漏检概率性能，适用于未来低功耗和抗干扰系统。

Conclusion: 提出的共生无线电系统通过波形域正交性实现了主通信与反向散射通信的无干扰共存，同时支持高分辨率传感，为未来低功耗抗干扰系统提供了有前景的解决方案。
...
TL;DR: 提出一种基于时域传输系数分析的线缆通信网络软故障检测、估计与定位方法，使用OFDM方案估计传输系数，通过故障严重度比估计故障强度，残差函数确定故障位置。


<details>
  <summary>Details</summary>
Motivation: 有线通信网络中软故障的准确检测、强度估计和位置定位对于网络维护和故障排除至关重要，需要一种有效的诊断方法。

Method: 基于时域传输系数分析，在无故障和有故障情况下对比传输系数；使用OFDM方案估计传输系数；推导故障严重度比来估计故障强度；提出基于残差的函数来确定故障位置。

Result: 在Y形测试装置上进行实验验证，证明了所提方法的有效性。

Conclusion: 该方法能够有效检测、估计和定位有线通信网络中的软故障，为网络故障诊断提供了实用解决方案。
...
TL;DR: 提出一种基于样条近似的瞬时频率估计新方法，用于处理含噪声多分量信号中紧密模态的干扰问题


<details>
  <summary>Details</summary>
Motivation: 处理含噪声多分量信号中紧密模态的瞬时频率估计问题，Prony方法会产生振荡干扰，需要消除这些干扰以获得更准确的估计

Method: 结合Prony技术和样条近似算法，先分析Prony方法产生的振荡特性，然后设计样条近似算法来消除这些振荡干扰

Result: 在各种情况下进行数值应用验证，表明Prony技术与样条近似相结合能有效改善含噪声多分量信号中紧密模态的瞬时频率估计

Conclusion: Prony技术与样条近似相结合的方法能有效处理含噪声多分量信号中紧密模态的瞬时频率估计问题，消除振荡干扰，提高估计精度
...
TL;DR: 本文设计了一种适用于低轨卫星连接的5G用户设备，重点解决时变传播延迟和多普勒频移补偿问题，并开发了基于无人机软件定义无线电的实验平台来模拟透明和再生卫星载荷。
...
Method: 设计了专门用于低轨卫星连接的5G用户设备，重点关注可变延迟和多普勒补偿策略。开发了基于无人机软件定义无线电的自定义实验平台，能够模拟透明和再生卫星载荷两种架构。

Result: 虽然完整的端到端系统验证尚未完成，但初步实验室测试证实了架构的可行性，为未来的实验活动奠定了基础。

Conclusion: 本文提出的5G用户设备设计和实验平台为低轨卫星连接提供了可行的技术方案，特别是在处理时变延迟和多普勒效应方面，为后续的非地面网络实验研究打下了基础。
...
TL;DR: 该论文研究了在硬件损伤下，流体天线系统与可重构智能表面结合的NOMA系统安全性能，建立了考虑硬件失真的安全BLER分析框架，揭示了硬件质量对性能上限的决定性影响。


<details>
  <summary>Details</summary>
Motivation: 流体天线系统与可重构智能表面的结合为安全无线通信提供了新机遇，但硬件损伤对这种组合的实际影响尚未充分探索。论文旨在研究在现实硬件约束下，FAS-RIS架构对NOMA系统安全性能的影响。

Method: 开发了综合分析框架，捕捉自适应空间分集、智能信号反射和硬件引起失真之间的复杂相互作用。采用分段线性近似和块相关模型，推导了平均安全块错误率的可处理表达式。

Result: 分析表明，虽然FAS与智能表面的协同为安全增强提供了显著自由度，但实际硬件缺陷会形成性能上限，无论空间分集增益如何。硬件质量成为实现FAS-RIS架构全部潜力的决定性因素。

Conclusion: 理论框架揭示了系统复杂性与可实现安全性能之间的关键设计权衡，硬件质量是实现FAS-RIS架构全部潜力的关键因素。研究为平衡流体智能协作优势与硬件限制提供了实用设计指导。
...
TL;DR: 本文研究基于Koopman算子和小波变换特征的transformer心电图分类，发现小波特征在二分类任务中表现优异，而Koopman特征与transformer结合在四分类任务中表现更好，适当的EDMD字典选择能显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 心电图分析对检测心脏异常至关重要，但由于生理信号的复杂性和变异性，稳健的自动分类具有挑战性。本研究旨在探索基于Koopman算子和小波变换特征的transformer模型在心电图分类中的应用。

Method: 使用扩展动态模式分解（EDMD）近似Koopman算子，从小波变换和Koopman算子提取特征，构建transformer分类模型。研究两个任务：二分类（正常vs非正常）和四分类（正常、房颤、室性心律失常、传导阻滞）。

Result: 小波特征在二分类任务中表现优异，而Koopman特征与transformer结合在四分类任务中表现更好。简单的混合特征并未提升准确率，但选择合适的EDMD字典（特别是调参后的径向基函数字典）能显著提升性能，超越小波基线和小波-Koopman混合系统。

Conclusion: 研究证明了基于Koopman的特征学习与transformer结合的有效性，为将动力系统理论整合到时序分类中提供了有前景的方向，并通过Koopman重构分析提供了可解释的动态学习洞察。
...
TL;DR: 提出基于天线选择的传输框架，抑制混合近远场系统中的干扰，通过联合设计天线选择和功率分配来最大化系统和速率


<details>
  <summary>Details</summary>
Motivation: 混合近远场系统中，近场和远场信道的非正交性会导致严重的用户间干扰，降低系统性能，特别是当模拟波束赋形采用低复杂度的全阵列最大比传输时

Method: 1. 提出天线选择传输框架，无需机械改变天线结构；2. 针对典型两用户场景分析，揭示干扰抑制与阵列增益增强之间的权衡；3. 针对多用户场景，开发基于惩罚对偶分解的两层框架，结合块坐标下降和逐次凸逼近技术；4. 提出基于干扰抑制准则的低复杂度天线去激活策略

Result: 分析表明仅去激活少量天线即可显著抑制混合场干扰，同时不过度损失阵列增益；数值结果显示所提方案在干扰抑制和阵列增益损失之间取得良好平衡，性能优于多种基线方案

Conclusion: 所提出的天线选择框架能有效抑制混合近远场系统中的干扰，在干扰抑制和阵列增益增强之间实现有利权衡，显著提升系统性能
...
TL;DR: 提出一种跨接收机的射频指纹识别框架，通过去噪谱商序列和可训练校准神经网络，同时解决信道变化和接收机差异问题，在WiFi设备上实现90%以上的跨接收机识别准确率。


<details>
  <summary>Details</summary>
Motivation: 射频指纹识别(RFFI)利用设备特定的硬件损伤进行发射机识别，但其性能在跨接收机部署时容易受到接收机差异和无线信道变化的影响。现有方法难以同时解决这两个挑战。

Method: 提出TDSQCNN框架：1) 注册阶段：开发信道鲁棒的预处理方法构建去噪谱商(DSQ)序列，训练DSQCNN；2) 跨接收机部署阶段：构建源和目标接收机的校准数据集，设计可训练校准神经网络(TCNN)学习非线性映射；3) 级联TCNN-DSQCNN实现目标接收机上的鲁棒分类。

Result: 在12个WiFi发射机和3个接收机的仿真实验中，该方法在24dB信噪比下达到超过90%的跨接收机分类准确率，首次通过信道抑制和非线性接收机校准联合解决了信道和接收机可移植性问题。

Conclusion: 该研究提出的跨接收机RFFI框架通过信道鲁棒预处理和接收机校准，有效解决了信道变化和接收机差异的双重挑战，为实现实用的射频指纹识别系统提供了重要技术路径。
...
TL;DR: 提出了一种用于协作集成感知与通信的子空间融合感知算法，通过张量展开构建等效虚拟天线阵列，实现无数据关联的分布式AP融合感知


<details>
  <summary>Details</summary>
Motivation: 在协作集成感知与通信系统中，需要有效融合分布式接入点的感知信息，传统方法存在数据关联复杂、性能受限等问题

Method: 将来自多个AP的接收信号堆叠为三阶张量，通过张量展开构建等效虚拟天线阵列，开发基于子空间的无数据关联融合感知算法

Result: 仿真结果表明所提算法相比传统技术具有更好的性能，同时推导了Cramer-Rao下界作为性能基准

Conclusion: 提出的子空间融合感知算法能够有效解决分布式AP协作感知问题，无需复杂的数据关联过程，在集成感知与通信系统中具有良好应用前景
...
TL;DR: 提出基于图结构的语义编码解码器（GBSED）用于车联网任务导向通信，通过场景图捕捉道路实体关系，实现高达99%的压缩率，在10dB以上SNR时语义保真度超过0.9。


<details>
  <summary>Details</summary>
Motivation: 传统通信范式传输原始比特消耗过多带宽且无法保持语义相关性，而车联网中的安全关键应用（如碰撞避免、协同感知）需要高效可靠的语义通信框架。

Method: 提出图基语义编码解码器架构：编码器利用场景图捕捉道路实体的空间和语义关系，结合语义压缩算法将提取的图表示压缩至原始图像的1%；解码器重建任务相关表示而非原始数据。

Result: 在自动驾驶场景仿真中，集成到MIMO OFDM物理层系统后，风险评估预测成功率高，在3GPP CDL信道下鲁棒性强，通信开销显著降低，语义保真度在10dB以上SNR时超过0.9。

Conclusion: 该语义通信框架是未来6G系统的有前景解决方案，能在保持高语义保真度的同时大幅降低通信开销，提升车联网任务性能。
...
TL;DR: 该论文研究了多模式夹持天线系统，通过模式选择与模式组合协议实现模式域复用，使用PSO-KPBF算法优化波束成形、天线位置和传播常数，显著提升了多用户通信性能。


<details>
  <summary>Details</summary>
Motivation: 传统单模式夹持天线系统在频谱效率和多用户通信方面存在限制，需要开发能够利用多个正交模式进行高效通信的多模式系统。

Method: 提出两种操作协议：模式选择（每个天线主要辐射单一模式）和模式组合（每个天线灵活辐射多个模式）。采用PSO-KPBF算法联合优化发射波束成形、天线位置和传播常数。

Result: 1) 即使使用简单的均匀模式组合设计，多模式PASS也显著优于传统单模式PASS和混合波束成形系统；2) 模式组合实现高频谱效率，模式选择以较低硬件复杂度接近其性能。

Conclusion: 多模式夹持天线系统通过模式域复用有效提升了多用户通信性能，提出的两种协议和优化算法在实际应用中具有良好平衡性能与复杂度的潜力。
...
TL;DR: 研究模式相关损耗对多芯光纤系统信噪比统计特性的影响，分析其对ASE噪声和非线性干扰的影响差异


<details>
  <summary>Details</summary>
Motivation: 模式相关损耗是耦合芯多芯光纤系统中的重要损伤机制，但其对信噪比统计特性的具体影响尚未被充分研究，特别是对ASE噪声和非线性干扰的不同影响

Method: 通过数值模拟和理论分析，深入研究了MDL对接收端ASE噪声和NLI的影响，以及它们对信噪比的联合贡献，并与单模偏振相关损耗进行了对比

Result: MDL对两种噪声产生不同的统计特性，与单模偏振相关损耗存在显著差异，同时空间模式色散会影响MDL引起的损伤

Conclusion: MDL对多芯光纤系统性能有重要影响，需要综合考虑其对ASE噪声和NLI的不同统计特性，空间模式色散会进一步影响MDL损伤
...
TL;DR: 本文对部分信息分解(PID)框架进行了全面综述，系统分析了现有PID度量方法及其数学性质，建立了性质间的依赖关系网络，为PID的理论完善和实际应用提供了统一视角。


<details>
  <summary>Details</summary>
Motivation: PID已成为描述复杂系统中信息结构和质量的重要信息论框架，但由于缺乏唯一解，出现了多种不同数学承诺的形式化方法，需要对这些方法进行系统梳理和比较。

Method: 通过将现有PID度量整合到共同语言中，系统检查所有主要PID方法，确定每个度量是否满足已知性质，推导性质间的定理网络，并揭示新的依赖关系结果。

Result: 建立了PID数学性质的完整图谱，揭示了不同性质间的兼容性和冲突关系，为PID框架提供了统一的理论视角和系统化理解。

Conclusion: 本文通过全面分析PID的数学景观，为该框架的讨论提供了统一视角，并为未来的理论完善和知情实证应用指明了路径。
...
TL;DR: 该论文研究在N个并行高斯信道上（如OFDM子载波）的功率分配问题，每个信道有目标频谱效率。通过最小化总平方速率偏差，证明了最优分配从不超过任何目标，且当所有目标可行时可能留用功率，这与经典注水算法结构不同。提出了基于Lambert W函数的闭式解和高效算法。


<details>
  <summary>Details</summary>
Motivation: 在OFDM等多信道系统中，每个信道通常有特定的速率目标要求。经典注水算法旨在最大化总速率，但无法精确控制各信道的速率偏差。需要一种能最小化各信道实际速率与目标速率偏差的功率分配方案。

Method: 建立优化问题：最小化总平方速率偏差∑(log₂(1+aᵢPᵢ)-Tᵢ)²，受限于总功率约束∑Pᵢ≤P_tot和非负约束Pᵢ≥0。利用KKT条件推导出闭式解，使用Lambert W函数表示激活集上的解，并通过一维单调二分法求解对偶变量。

Result: 证明了最优分配从不超过任何目标，且当所有目标可行时可能留用功率。算法时间复杂度为O(Nlog(1/ε))，在N=1024时比通用数值求解器快1890倍。数值实验显示闭式解达到机器精度，且在瑞利衰落信道中比注水、均匀分配和比例公平算法有更好的目标跟踪性能。

Conclusion: 该研究提出了一种高效的最优功率分配方法，能精确控制各信道速率与目标的偏差，适用于需要精确速率控制的通信系统。算法结构简单高效，性能优于传统方法。
...
Method: 1. 针对奇偶校验矩阵列具有单位欧几里得范数的模拟纠错码，提出了高度剖面的上界；2. 设计了简单的单错误纠正解码器；3. 构造了冗余为3、适用于任意码长的单错误纠正模拟纠错码族。

Result: 1. 建立了高度剖面的理论界限；2. 提出的单错误纠正解码器有效工作；3. 构造的模拟纠错码族相比已知MDS构造具有更小的高度剖面。

Conclusion: 本文为具有单位范数列的模拟纠错码提供了理论分析和实用构造，所提出的码族在高度剖面方面优于现有MDS构造，为模拟计算中的错误纠正提供了更优方案。
...
TL;DR: 提出一种基于可移动天线（MA）的新方法，通过联合优化波束成形矩阵、用户分簇、公共速率分配和天线位置向量，解决两层速率分割多址接入（RSMA）系统中的用户公平性问题，实现最大最小公平性。


<details>
  <summary>Details</summary>
Motivation: 在先进的多用户系统（如两层RSMA）中，提升用户公平性是一个关键但具有挑战性的任务。现有方法在保证用户间公平性方面存在不足，需要新的解决方案。

Method: 提出一种可移动天线（MA）方法，将问题建模为最大最小公平性问题，最大化最小用户速率。采用双层迭代算法：外层循环使用动态邻域剪枝粒子群优化方法寻找高质量的天线位置向量（APV），内层循环针对给定的APV优化波束成形矩阵、用户分簇和公共速率分配。

Result: 仿真结果表明，所提出的方案相比各种基准方案，在公平性方面取得了显著提升，验证了方法的有效性。

Conclusion: 可移动天线方法能有效解决两层RSMA系统中的用户公平性问题，通过联合优化天线位置和其他系统参数，实现了显著的用户公平性提升。
...
TL;DR: 研究二进制马尔可夫源在汉明失真下的d-tilted信息，发现其中心化块和是马尔可夫链占据计数的仿射映射，导致所有中心化累积量与失真水平D无关。


<details>
  <summary>Details</summary>
Motivation: d-tilted信息在有限块长率失真理论中已被证明是有用的量。本文研究平稳二进制马尔可夫源在汉明失真下，由单字母Blahut-Arimoto操作点诱导的源侧d-tilted和，这是一个与n字母操作d-tilted信息不同的源侧量。

Method: 研究二进制马尔可夫源在汉明失真下的源侧d-tilted和，证明中心化块和$J_n(D) - nμ_D$恰好是马尔可夫链占据计数$N_n = \sum_{t=1}^n \mathbf{1}\{X_t = 1\}$的仿射映射。使用2×2转移矩阵分析有限n分布和极限累积量生成函数。

Result: 所有中心化累积量与失真水平D无关；有限n方差有闭式解；精确的有限n分布和极限累积量生成函数由2×2转移矩阵给出。

Conclusion: 对于二进制马尔可夫源在汉明失真下，源侧d-tilted和的中心化块和具有简洁的数学结构，表现为马尔可夫链占据计数的仿射映射，这简化了其统计特性的分析。
...
TL;DR: 提出一种利用信道响应幅度从失真信道比特征中恢复理想信道特征的方法，解决无线通信中时钟异步导致的相位偏移问题，显著提升亚波长位移感知精度


<details>
  <summary>Details</summary>
Motivation: 无线通信信号的无接触感知因其非侵入性和普适性基础设施而备受关注，但双基地部署引入的时钟异步导致信道响应中存在未知相位偏移，阻碍了细粒度感知。现有系统广泛采用交叉天线信道比来消除这些有害相位偏移，但信道比仅在整数波长目标位移时保持感知特征精度，失去了亚波长保真度。

Method: 首先推导失真信道比特征与理想信道特征之间的定量映射关系，在此基础上开发一个鲁棒框架，利用信道响应幅度从失真信道比中恢复理想信道特征。

Result: 在Wi-Fi和LoRa上的真实世界实验表明，该方法能有效重建亚波长位移细节，实现精度提升近一个数量级。

Conclusion: 通过建立信道比特征与理想信道特征之间的定量映射，并利用信道响应幅度进行恢复，成功解决了无线通信感知中的亚波长精度问题，显著提升了无接触感知系统的性能。
...
TL;DR: 该论文对ORBGRAND解码器进行了有限块长分析，推导了ORBGRAND特定的随机编码联合界，并得到了二阶可达率展开和正态近似。


<details>
  <summary>Details</summary>
Motivation: 现有ORBGRAND的信息论结果都是渐近的（块长趋于无穷），无法量化其在短到中等块长下的性能。需要开发有限块长分析来评估实际应用中的性能。

Method: 1. 推导ORBGRAND特定的随机编码联合界（ORB-RCU）；2. 分析两个关键解码度量：传输码字度量作为U统计量处理，竞争码字度量简化为加权独立同分布伯努利随机变量和；3. 结合Berry-Esseen论证得到二阶可达率展开和正态近似。

Result: 得到了ORBGRAND的二阶可达率展开，其一阶项等于ORBGRAND广义互信息，二阶项定义了具有单字母方差表示的ORBGRAND色散。数值结果表明ORB-RCU相对于基于最大似然的RCU基准很紧，正态近似在实际操作区域内准确。

Conclusion: 该论文成功开发了ORBGRAND的有限块长分析框架，解决了秩诱导解码度量的非加性和符号间耦合的关键挑战，为实际短到中等块长通信系统提供了理论性能评估工具。
...
TL;DR: 本文研究了有限域上长度为n=λ(q^m+1)的BCH码和LCD循环码，分析了陪集结构，确定了码的维数，改进了最小距离下界，构造了部分最优码，并给出了对偶BCH码和LCD循环码的充要条件。


<details>
  <summary>Details</summary>
Motivation: BCH码是一类重要的循环码，许多BCH码是最优的并在通信系统中有广泛应用，但确定其参数仍然是一个具有挑战性的问题。本文旨在研究特定长度n=λ(q^m+1)下的BCH码和LCD循环码的性质。

Method: 首先分析模n的分圆陪集，建立γ是陪集首的充要条件并确定两个最大的陪集首。基于此确定几类BCH码的维数，改进其最小距离的下界。当m为奇数时，建立BCH码为对偶BCH码的充要条件。最后枚举所有LCD循环码。

Result: 确定了BCH码的维数，改进了最小距离下界，构造了部分最优码。当m为奇数时，给出了BCH码为对偶BCH码的充要条件。枚举了所有LCD循环码。

Conclusion: 本文系统研究了特定长度下的BCH码和LCD循环码，解决了参数确定问题，构造了最优码，为相关研究提供了理论基础，并提出了若干开放问题供进一步研究。
...
TL;DR: TBM是一种基于ℤₘ上非二进制线性分组码的编码调制方案，通过M-PSK映射形成几何均匀信号空间码，具有张量可识别性和多用户分离能力。
...
Method: 将TBM建模为ℤₘ上的非二进制线性分组码，推导其生成矩阵，分析秩缺陷特性，通过参考符号实现张量可识别性（对应码字缩短），形成准系统或系统码。

Result: 在单用户AWGN和多用户非相干多天线衰落信道中的仿真表明，TBM具有强鲁棒性和干扰抑制能力，证明其作为可扩展的代数结构调制编码方案的有效性。

Conclusion: TBM成功建立了张量表示与现代编码理论之间的桥梁，为无源随机接入提供了一种可扩展、代数结构清晰的调制编码方案。
...
TL;DR: 提出Hadamard-NOMA方法，通过在调制前引入Hadamard变换来缓解衰落和CSI不完美的影响，显著降低误码率，相比现有方案获得10-15dB的性能增益。


<details>
  <summary>Details</summary>
Motivation: 非正交多址(NOMA)系统虽然能提高频谱效率，但对功率分配系数、衰落效应和不完美信道状态信息(CSI)非常敏感，这些限制影响了系统可靠性。

Method: 提出Hadamard-NOMA方法，在调制前对源信号应用Hadamard变换(HT)，通过变换处理来减轻衰落和CSI不完美的影响，增强系统鲁棒性。

Result: 相比T-NOMA和Usman-NOMA方案，近用户获得15dB增益(在BER=10^-2)，远用户获得10dB增益(在BER=10^-1)。在非完美SIC的两用户场景中，用户1比用户2低14dB SNR即可达到BER=10^-3。

Conclusion: 在源级应用Hadamard变换能有效缓解CSI误差，使NOMA系统对下一代无线网络更具弹性，提供更可靠的性能。
...
TL;DR: 提出了一种基于组合数据增强的非视距检测算法(CDA-ND)，通过单次测距测量生成多个初步定位估计，利用NLoS证据向量进行NLoS检测，显著提高了6G定位精度。
...
Method: 提出CDA-ND算法：1) 通过多个gNB组合的单次测距测量生成大量初步定位估计(PELs)；2) 当目标gNB处于NLoS时，PELs会分裂成两个簇，通过NLoS证据向量(NEV)计算NLoS似然分数；3) 开发硬决策(HD)和软决策(SD)两种检测模式；4) 基于检测结果设计定位算法，排除NLoS gNB或重新加权。

Result: 在室内工厂环境FR1频段下，当NLoS gNB比例约18%和56%时，NLoS检测准确率分别达到96.6%和91.1%。集成CDA-ND后，在LoS主导和NLoS主导环境中，平均绝对误差分别减少20.04%和65.99%。

Conclusion: CDA-ND算法能有效检测NLoS条件，显著提升6G定位性能，特别是在NLoS主导环境中效果更为显著，为6G高精度定位提供了实用解决方案。
...
TL;DR: 本文建立了关于微分Rényi熵、Rényi散度和Rényi交叉熵的新锐不等式，当其中一个概率密度函数是另一个的伴随密度时达到等式。该不等式与相互倒易变换的一般框架结合，推导出涉及经典和新信息泛函的进一步不等式。


<details>
  <summary>Details</summary>
Motivation: 研究Rényi信息度量之间的精确关系，建立概率密度函数之间信息泛函的尖锐不等式，为信息理论提供新的数学工具和关系。

Method: 建立微分Rényi熵、Rényi散度和Rényi交叉熵之间的新锐不等式，结合相互倒易变换的一般框架，推导涉及绝对矩、Fisher信息度量等相对和交叉版本的信息泛函不等式。

Result: 证明了Rényi散度可以被交叉型和单型信息泛函的商尖锐地界定，推导出涉及两个和三个概率密度函数的精确不等式，包括相对和交叉版本的绝对矩、Fisher信息度量等。

Conclusion: 本文建立了Rényi信息度量的新锐不等式框架，当概率密度函数是伴随密度关系时达到等式，为信息理论提供了新的数学关系和界限工具，这些不等式在多个信息泛函之间建立了精确的定量关系。
...
TL;DR: 该论文提出了一种考虑互耦效应的宽带可重构全息表面模型和联合波束赋形框架，通过耦合分解和雅可比辅助优化方法提升多用户和速率。


<details>
  <summary>Details</summary>
Motivation: 密集亚波长辐射元件之间的互耦效应限制了宽带可重构全息表面的多用户传输性能，现有模型未能充分考虑这种耦合影响。

Method: 1) 建立基于磁偶极子单元的电磁等效模型，将耦合分解为自由空间近场耦合和导波表面波耦合；2) 采用加权最小均方误差块坐标法，结合闭式数字预编码更新；3) 提出雅可比辅助的耦合一致性全息图更新方法，保持耦合敏感性同时保持凸性。

Result: Meep实验验证了耦合模型的正确性，28GHz、1GHz带宽的RHS下行链路仿真表明，雅可比辅助的WMMSE方法在馈电功率和RHS激励功率约束下能有效提升多用户和速率。

Conclusion: 该研究提出的耦合感知宽带RHS模型和优化框架能有效处理互耦效应，为实际RHS系统设计提供了理论和方法基础。
...
TL;DR: 本文研究了受限对手存在时通信网络中的单源多播问题，发现经典割集界不再紧致，需要内外码联合设计，与无限制对手情况形成对比。


<details>
  <summary>Details</summary>
Motivation: 当对手被限制在特定边集上操作时，经典网络编码理论中的割集界不再紧致，这需要新的编码策略来达到容量，与无限制对手情况下的线性网络编码加秩度量外码的方法形成鲜明对比。

Method: 基于网络解码框架，研究了[4]中引入的基本2级网络家族，确定了其中一个家族的确切单次容量，改进了另一个家族的最佳已知下界。引入了一个新的网络家族来推广已知示例，并推导了部分容量结果。

Result: 确定了基本2级网络家族中一个的确切单次容量，改进了另一个家族的最佳已知下界。新引入的网络家族展示了受限对手设置下特有的多种现象，并研究了网络在秩度量和汉明度量下的可分离性概念。

Conclusion: 受限对手设置下的网络编码需要内外码联合设计，与无限制情况不同。研究揭示了受限对手环境下的独特现象，为理解网络在对抗性攻击下的容量特性提供了新见解。
...
TL;DR: 本文提出了一种针对具有记忆效应的线性高斯ISI信道的GRAND解码算法变体，通过引入错误突发和序列可靠性概念，开发了SGRAND-ISI及其简化版本ORB-GRAND算法，显著提升了信道解码性能。


<details>
  <summary>Details</summary>
Motivation: 传统GRAND解码算法在处理具有记忆效应的信道（如线性高斯ISI信道）时性能受限，需要开发能够有效利用信道记忆特性的解码方法。

Method: 引入错误突发概念来描述信道记忆效应，定义序列可靠性来表征错误模式的似然性。基于此提出SGRAND-ISI算法作为软GRAND在ISI信道上的推广，然后开发了便于实现的ORB-GRAND算法。

Result: 提出的算法相比忽略信道记忆的GRAND算法获得了多个dB的性能提升，通常能达到ML下界的0.1-0.2dB范围内。与现有ORBGRAND-AI算法相比，在10^-3误块率下至少有0.5dB增益，且计算复杂度显著降低。

Conclusion: 通过考虑信道记忆效应并引入序列可靠性概念，本文提出的GRAND算法变体在线性高斯ISI信道上实现了接近ML性能的解码，同时保持了较低的计算复杂度。
...
TL;DR: 该论文研究了受激光相对强度噪声影响的强度调制直接检测光通信系统，推导了离散时间信道模型，发现噪声具有信号依赖性和记忆效应，不同于文献中常用模型。研究显示当接收机忽略信道记忆时，广义互信息随星座尺寸增加而饱和，密集星座无法带来增益。


<details>
  <summary>Details</summary>
Motivation: 研究激光相对强度噪声对强度调制直接检测光通信系统的影响，特别是噪声的信号依赖性和记忆效应，这些特性在现有文献模型中常被忽略，需要更准确的模型来评估系统性能。

Method: 从连续时间波形模型出发，推导等效离散时间信道模型；采用失配解码方法，使用无记忆解码度量计算广义互信息；在实用相关系统参数下研究可达信息率。

Result: 推导的信道模型显示噪声具有信号依赖性和记忆效应，噪声条件方差与感兴趣符号呈多项式依赖关系；数值结果表明当接收机忽略信道记忆时，广义互信息随星座尺寸增加而饱和，密集星座无法提供增益；这种饱和源于符号对广义互信息的非对称非消失贡献。

Conclusion: 激光相对强度噪声导致光通信信道具有信号依赖噪声和记忆效应，忽略这些特性会限制系统性能，密集星座无法带来预期增益，需要开发考虑信道记忆的接收机设计来提升性能。
...
TL;DR: 该论文针对光学卫星上行链路中的高功率光纤非线性效应问题，提出了基于查找表的星座整形和简单非线性相位旋转的低复杂度数字信号处理技术，可将最大可接受链路损耗提高6dB。


<details>
  <summary>Details</summary>
Motivation: 光学卫星上行链路依赖高功率光学放大器来克服自由空间衰减，但在高功率水平下，光纤克尔非线性效应变得显著并降低系统性能。需要开发能够补偿非线性效应的技术来提升系统性能。

Method: 1. 开发了考虑非线性效应的光学上行链路实际模型；2. 提出了基于查找表的星座整形技术；3. 设计了在发射端和/或接收端应用的简单非线性相位旋转补偿技术；4. 通过查找表实现根据信道条件的自适应速率调整。

Result: 仿真结果表明，所提技术可将最大可接受链路损耗提高高达6dB，且复杂度可忽略不计。系统层面分析显示，高功率光学放大器中的传播可建模为简单的非线性相位旋转，等效于在零色散无噪声光纤链路中传播，完全由单一参数——特征非线性功率来表征。

Conclusion: 该研究为光学卫星上行链路中的非线性补偿提供了有效的低复杂度解决方案，通过星座整形和相位旋转技术显著提升了系统性能，同时建立了简化的系统级非线性传播模型。
...
TL;DR: 该论文研究了双基地集成感知与通信系统中OFDM波形的优化设计，通过联合优化子载波分配和功率分配，在满足感知精度约束下最大化通信数据速率。


<details>
  <summary>Details</summary>
Motivation: 在集成感知与通信系统中，需要设计能够同时支持通信和感知功能的波形。现有方法通常将这两个功能分开处理，未能充分利用系统资源实现联合优化，限制了系统性能。

Method: 提出联合路径系数和延迟估计方案，揭示了通信数据速率由通信子载波数量决定，而延迟感知精度由感知子载波的索引分布决定。采用二次变换和拉格朗日对偶分解方法，迭代更新子载波分配和功率分配变量。

Result: 研究发现：当且仅当子载波的Fisher信息增益超过相应的通信速率损失时，该子载波才被分配用于感知；通信子载波的功率分配呈现有界注水结构。仿真结果表明，所提框架在延迟估计精度和通信数据速率方面显著优于现有基线方法。

Conclusion: 该研究为双基地ISAC系统中的OFDM波形设计提供了有效的优化框架，通过联合优化子载波分配和功率分配，在满足感知精度要求的同时最大化通信性能，实现了通信与感知功能的协同优化。
...
TL;DR: 本文构建了随机几何框架来分析5G多连接网络中的下行电磁场暴露和效率，使用EN-DC配置作为代表性用例。采用PPP和beta-GPP建模基站分布，beta-GPP能更好捕捉实际部署中的排斥效应。推导了EMF暴露分布的解析表达式，并通过蒙特卡洛模拟和巴黎真实基站数据验证。引入REBT-DL新指标综合考虑吞吐量和接收功率。结果显示网络配置显著影响暴露和REBT-DL，beta-GPP比PPP更准确拟合实际部署。


<details>
  <summary>Details</summary>
Motivation: 随着5G网络部署，电磁场暴露问题日益受到关注。现有研究缺乏综合考虑网络效率与电磁暴露的分析框架，特别是针对5G多连接网络场景。需要开发能够准确建模基站空间分布、评估暴露水平与网络性能之间权衡的工具。

Method: 1. 使用随机几何框架，采用泊松点过程(PPP)和beta-Ginibre点过程(beta-GPP)建模基站空间分布，其中beta-GPP能捕捉实际部署中的排斥效应；2. 以5G EN-DC配置为代表性用例；3. 推导电磁场暴露分布的解析表达式；4. 引入新指标REBT-DL（下行链路每比特传输辐射能量），综合考虑吞吐量和接收功率；5. 通过蒙特卡洛模拟和巴黎真实基站数据进行验证。

Result: 1. 网络配置对电磁场暴露和REBT-DL有显著影响；2. beta-GPP比PPP能更准确地拟合实际基站部署；3. 提出的框架为可持续网络评估提供了全面工具；4. 结果强调了能源感知部署策略的重要性；5. 框架验证了随机几何方法在分析5G网络暴露与效率权衡方面的有效性。

Conclusion: 本文开发的随机几何框架能够有效分析5G多连接网络中的电磁场暴露与网络效率。beta-GPP比PPP更适合建模实际基站部署。REBT-DL指标为评估网络可持续性提供了新视角。该框架可作为网络规划与优化的工具，帮助实现能源效率与暴露控制的平衡。
...
TL;DR: 首个面向ISAC的贝叶斯多径环境建图框架，融合单站和双站测量，考虑非理想表面传播，提升建图精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 低空经济发展推动ISAC技术需求，现有研究局限于双站非视距链路和理想镜面反射假设，忽略了ISAC系统的单站感知能力和非理想建筑表面引起的漫散射效应。

Method: 提出首个贝叶斯多径环境建图框架，建立单站和双站测量与共同反射表面的几何关系，实现数据级融合；设计两个互补的贝叶斯框架及因子图表示，适应不同场景需求。

Result: 通过合成RF数据验证，融合单站和双站链路相比单链路基线，能获得更高精度、更强鲁棒性和更快收敛的环境地图。

Conclusion: 该框架有效解决了现有ISAC环境建图研究的局限性，为低空经济应用提供了更可靠的感知通信一体化解决方案。
...
TL;DR: FedPASS：一种基于可调天线系统的低延迟无线联邦学习框架，通过动态调整天线位置优化信道质量，显著减少训练延迟


<details>
  <summary>Details</summary>
Motivation: 无线联邦学习面临不可靠通信链路的根本限制，特别是在上行信道存在阻塞、衰落或弱视距条件时。可调天线系统提供了动态重新定位辐射点的物理层能力，但如何将其有效集成到联邦学习中仍待探索。

Method: 提出FedPASS框架，将多目标优化问题形式化为混合整数非线性规划，开发双层迭代算法：外层通过块坐标下降更新调度、通信时间分配和功率控制，内层在间距约束下使用基于高斯-赛德尔的坐标更新和网格搜索优化天线位置。

Result: 在MNIST和CIFAR-10数据集上的数值结果表明，FedPASS在实现与理想化联邦学习基线相当的准确性的同时，相比传统无线联邦学习大幅减少了总训练延迟。

Conclusion: FedPASS通过利用可调天线系统的物理层能力，为无线联邦学习提供了有效的低延迟解决方案，在保持模型性能的同时显著改善了训练效率。
...
<details>
  <summary>Details</summary>
Motivation: 无线网络中的分布式机器学习依赖于准确的信道状态信息和高效的高维模型更新交换。由于设备移动性和散射环境的异构性，不同设备的信道相干时间和带宽各不相同，导致下行链路传输质量下降和上行链路空中聚合失真。

Method: 1. 下行链路：采用产品叠加复用技术，将长相干时间（静态）设备的全局模型符号复用到短相干时间（动态）设备所需的导频上；2. 将OFDM超块划分为与最小相干时间和带宽对齐的子块；3. 通过先前本地模型填充（PLMF）缓解动态设备的部分模型接收问题；4. 建立异构链路损伤、不完美CSI和聚合噪声下的收敛保证。

Result: 该框架能够在相干异构性下实现高效调度，分析和实验表明，与传统联邦学习基线相比，在通信效率、延迟和学习准确性方面取得了显著提升。

Conclusion: 提出的相干感知联邦学习框架通过创新的通信策略，有效解决了异构无线网络中下行和上行链路的损伤问题，为分布式机器学习在无线环境中的高效部署提供了可行方案。
