...
TL;DR: 本文研究了捏合天线系统(PASS)的能量效率，开发了功率分布的一致性公式，并提出混合动态-静态算法来最大化能量效率。


<details>
  <summary>Details</summary>
Motivation: 研究PASS系统的能量效率，因为其每天线功率分布不是通过功率分配策略显式控制，而是通过调节捏合耦合和位置隐式控制。

Method: 开发了一类混合动态-静态算法，通过以不同速率更新系统参数来最大化能量效率。其中捏合位置可通过移动元件动态调节，而耦合则通过改变捏合元件的有效耦合长度来调节。

Result: 实验结果表明，动态调节捏合位置能显著提升PASS系统的能量效率。

Conclusion: 动态调节捏合位置是提升PASS系统能量效率的有效方法，特别是在低用户移动性场景下。
...
<details>
  <summary>Details</summary>
Motivation: 开发更高效的雷达检测器，解决现有基于transformer和卷积的雷达检测器参数多、计算量大、速度慢的问题。

Method: 使用两个状态空间模型：一个在单个啁啾内顺序处理所有接收通道的样本学习啁啾级特征，另一个顺序处理啁啾级特征学习帧级表示，然后解码进行分割和检测任务。

Result: 在RADIal数据集上，SSMRadNet比最先进的transformer和卷积雷达检测器参数量减少10-33倍，计算量减少60-88倍，速度快3.7倍，分割任务性能相当。

Conclusion: SSMRadNet证明了状态空间模型在雷达检测中的高效性，为实时雷达处理提供了轻量级解决方案。
...
TL;DR: 本文研究了支持集成感知与通信的蜂窝自由大规模MIMO系统中的功率控制设计，通过用户中心架构和目标准心方法同时服务移动用户和监测环境，提出了最大化数据速率和感知信噪比的QoS方案。


<details>
  <summary>Details</summary>
Motivation: 解决在相同时间-频率资源下，蜂窝自由大规模MIMO系统如何同时实现高效通信和环境感知的问题，探索集成感知与通信技术在下一代网络中的潜力。

Method: 采用用户中心架构处理数据服务，引入目标准心方法执行检测任务，推导接收感知信噪比，并基于此制定最大化数据速率和有效感知信噪比的QoS方案。

Result: 仿真结果表明，所提方案优于正交资源分配算法，验证了集成感知与通信在蜂窝自由大规模MIMO网络中的有效性。

Conclusion: 本文提出的集成感知与通信框架在蜂窝自由大规模MIMO系统中具有显著优势，能够同时优化通信和感知性能，为未来网络发展提供了有前景的方向。
...
TL;DR: 提出了一种基于强化学习的波束加权框架，结合策略网络和增强加权最小二乘估计器，用于多波束LEO星座中的精确低复杂度定位。


<details>
  <summary>Details</summary>
Motivation: 传统基于几何或CSI的方法在动态波束几何下定位精度不足且复杂度高，需要开发无需显式CSI估计的鲁棒定位方法。

Method: 使用策略网络直接从上行链路导频响应和几何特征学习，结合增强WLS联合估计位置和接收机时钟偏差。

Result: 在代表性场景中，相比几何基线方法，平均定位误差减少99.3%，达到0.395米RMSE，且具有近实时推理能力。

Conclusion: 该方法在无需显式CSI估计的情况下实现了高精度、低复杂度的定位，在动态波束几何下具有优异的数值稳定性。
...
TL;DR: 提出OG-PCL网络处理毫米波雷达生成的稀疏3D点云，用于人体活动识别。该网络参数仅0.83M，在RadHAR数据集上达到91.75%准确率，优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 毫米波雷达的人体活动识别提供了一种保护隐私且鲁棒的替代方案，相比基于摄像头和可穿戴设备的方法。需要处理稀疏3D雷达点云的轻量级解决方案。

Method: 采用Occupancy-Gated Parallel-CNN Bi-LSTM (OG-PCL)网络，包含三视图并行结构以保持三维空间信息，引入Occupancy-Gated Convolution (OGConv)块处理稀疏点云。
...
<details>
  <summary>Details</summary>
Motivation: 传统IF-TEM存在过度过采样问题，导致对信息平滑分布信号的编码效率低下。

Method: 引入线性偏置IF-TEM，使偏置动态跟踪输入信号，保持积分器输入近似恒定，从而集中触发间隔分布。

Result: 理论分析建立了可实现的过采样范围界限，实验结果显示在显著降低比特率的情况下达到可比的重建精度。
...
TL;DR: 提出RadHARSimulator V2，一种直接从视频生成多普勒频谱的雷达人体活动识别模拟器，包含计算机视觉和雷达模块，并提出了混合并行-串行神经网络架构。


<details>
  <summary>Details</summary>
Motivation: 现有的雷达人体活动识别模拟方法基于模型或运动捕捉数据，灵活性有限，需要一种更灵活的模拟方法。

Method: 计算机视觉模块使用目标检测和姿态估计技术获取3D姿态，雷达模块使用Savitzky-Golay方法进行姿态插值和平滑，通过延迟模型和镜像方法模拟回波，生成距离-时间图和多普勒-时间图，并提取脊特征。

Result: 通过数值实验验证了模拟器和网络模型的有效性。

Conclusion: 提出的模拟器能够直接从视频生成多普勒频谱，为雷达人体活动识别提供了更灵活的模拟方法。
...
TL;DR: 提出了一种在结构化潜在空间中合成组合运动模式的EMG信号生成方法，通过VAE编码和混合一致性损失来改进组合运动的分类性能


<details>
  <summary>Details</summary>
Motivation: 传统方法假设组合运动是基本运动的线性组合，但由于肌肉共收缩等复杂神经肌肉现象，这种假设往往不成立，导致合成信号保真度低和分类性能下降

Method: 使用变分自编码器将EMG信号编码到低维表示，引入混合一致性损失来结构化潜在空间，使组合运动嵌入在其组成基本运动之间，然后在此结构化潜在空间中生成合成模式用于训练分类器

Result: 在上肢运动分类实验中，该方法比输入空间合成方法表现更好，准确率提高了约30%

Conclusion: 在结构化潜在空间中合成组合运动模式的方法能够有效提高EMG信号分类性能，特别是在识别未见过的组合运动方面
...
TL;DR: 提出基于跨模态注意力的深度学习架构xHAP，通过融合操作者历史力反馈和控制动作序列来估计触觉反馈，显著提升触觉互联网的可靠性和覆盖范围。


<details>
  <summary>Details</summary>
Motivation: 触觉互联网需要超低延迟和高保真触觉反馈，但在信道变化和网络中断下确保触觉数据包的可靠传输具有挑战性。现有本地估计方法难以准确再现真实触觉力。

Method: 使用跨模态注意力架构，通过模态特定编码器学习时间表示，然后在跨注意力层中让远程操作者触觉数据关注操作者输入，选择性聚焦最相关的操作者感官数据。

Result: 相比现有方法，均方误差降低两个数量级，在3GPP UMa场景下，误差阈值为0.1时可靠传输的SNR要求降低10dB，覆盖范围增加138%，在SNR降低10dB时仍能支持59.6%更多触觉用户。

Conclusion: xHAP架构通过跨模态注意力有效融合多源信息，显著提升了触觉反馈估计的准确性和系统可靠性，为触觉互联网提供了可行的解决方案。
...
TL;DR: 本文研究了在双弥散信道中OFDM系统的最优导频模式设计，基于LMMSE估计器。通过将信道协方差矩阵分解为时延域和多普勒域的Kronecker积，并利用Szegö极限定理，推导出了LMMSE信道估计误差的紧凑解析形式和闭式下界。


<details>
  <summary>Details</summary>
Motivation: 在双弥散信道中，OFDM系统的信道估计性能受到导频模式设计的显著影响。现有方法在分析复杂度和性能优化方面存在挑战，需要一种更有效的最优导频模式设计方法。

Method: 将信道协方差矩阵分解为时延域和多普勒域的Hermitian Toeplitz矩阵的Kronecker积，利用Szegö极限定理证明这些矩阵可由DFT矩阵近似对角化，从而推导出LMMSE信道估计误差的紧凑解析形式。

Result: 获得了LMMSE信道估计误差的闭式下界，并建立了该下界可通过基于格点的导频模式实现的条件。数值结果表明所提出的矩阵近似引入的误差可忽略不计。

Conclusion: 提出的方法为双弥散信道中OFDM系统的最优导频模式设计提供了理论依据和实用方案，能够有效提升信道估计性能。
...
TL;DR: Mip-NeWRF是一种基于物理信息的神经框架，通过圆锥截头体采样、尺度一致混合位置编码和课程学习，显著提高了室内信道预测的精度和收敛速度。
...
Method: 采用基于射线的粗到细重要性采样流程，使用圆锥截头体采样和尺度一致混合位置编码，结合共享MLP和课程学习策略，在信道合成中整合路径损耗和表面交互衰减。

Result: 在典型场景中，归一化均方误差比最先进基线方法降低了14.3 dB。

Conclusion: Mip-NeWRF框架通过改进的位置编码和物理建模，显著提升了室内信道预测的性能和收敛稳定性。
...
<details>
  <summary>Details</summary>
Motivation: 针对集成感知与通信系统中双弥散信道带来的干扰问题，需要系统分析不同干扰处理方法的性能差异。

Method: 采用广义OFDM框架，通过连续时延-多普勒信道表征，推导LMMSE信道估计器和估计误差，量化通信性能。

Result: 理论分析和数值结果揭示了四种ISCI处理方法在性能和复杂度之间的基本权衡关系。

Conclusion: 为双弥散信道中实用ISAC波形和接收机设计提供了重要见解。
...
TL;DR: 本文提出了一种基于高阶非线性波束形成和相干因子加权的声学成像方法，相比传统DAS方法显著提高了图像对比度，并在嵌入式平台上实现了GPU加速的实时性能。


<details>
  <summary>Details</summary>
Motivation: 传统DAS波束形成方法存在高旁瓣、宽主瓣和低对比度的问题，而自适应方法又因计算成本和实时性限制不适用于实际应用，需要一种兼顾高性能和实时性的解决方案。

Method: 采用延迟-乘加和(DMAS)技术结合相干因子加权的非线性波束形成方法，专门针对空中超声麦克风阵列进行优化，并实现GPU加速的嵌入式实时处理。

Result: 通过仿真和实际声学数据验证，相比DAS基线方法，所提方法在图像对比度方面有显著提升。

Conclusion: 高阶非线性波束形成是一种实用且高性能的空中声学成像解决方案，能够在嵌入式平台上实现实时处理。
...
### [14] [Equivalence of Several 6G Modulation Schemes for Doubly-Selective Channels](https://arxiv.org/abs/2511.09418)
*Nishant Mehrotra,Sandesh Rao Mattu,Robert Calderbank*

Main category: eess.SP

TL;DR: 该论文提出了一个分析双选择性信道调制方案的框架，重点关注非选择性和可预测性两个特性，这些特性与调制方案实现的多样性和频谱效率直接相关。
...
Method: 开发了一个分析框架，使用非选择性和可预测性两个特性来评估调制方案，比较了延迟-多普勒、啁啾和时序域调制与时频调制的性能差异。

Result: 研究表明延迟-多普勒、啁啾和时序域调制具有非选择性和可预测性，并且彼此等价，而时频调制具有选择性和不可预测性。

Conclusion: 延迟-多普勒、啁啾和时序域调制在双选择性信道中比传统时频调制表现更好，因为它们具有非选择性和可预测性的优势特性。
...
TL;DR: 提出了一种新型二维夹持天线系统（2D-PASS），将传统的线形结构扩展为连续介质波导平面，形成可重构辐射平面，能够在二维空间域实现动态波束适配。
...
Method: 开发了优化框架，通过自适应调整夹持天线的空间配置来最大化用户设备的最小接收信噪比，作为动态空间控制的模拟波束成形机制。针对连续位置场景提出了基于粒子群优化的算法，并为实际硬件约束引入了离散变体。

Result: 仿真结果表明，与传统的线形PASS和固定位置天线基准相比，所提出的2D-PASS显著提高了最小信噪比，同时在变化的用户分布和距离下保持鲁棒性。

Conclusion: 2D-PASS通过扩展为二维结构，显著提升了空间灵活性和性能，为多用户和室内场景提供了更有效的解决方案。
...
TL;DR: 提出了一种结合数字孪生的通道图表定位方法，将自监督定位结果转换为真实空间坐标，无需标注数据即可实现29%的相对平均距离误差降低。


<details>
  <summary>Details</summary>
Motivation: 传统通道图表定位技术估计的位置处于任意坐标系，与真实空间坐标不对齐，这限制了其实际应用价值。

Method: 提出新框架：(i)从估计CSI和数字孪生中提取大尺度信道状态信息特征；(ii)使用余弦相似度损失函数匹配这些特征，并与传统CC损失结合学习定位函数。

Result: 在模拟室内场景中，相比现有技术，相对平均距离误差降低了29%，且对数字孪生建模不匹配和测试数据分布偏移具有鲁棒性。

Conclusion: 该方法成功将自监督定位结果转换为真实空间坐标，显著提升了定位精度和实用性。
...
TL;DR: 提出了一种最大似然检测框架PAD-D，在极坐标域中联合缓解幅度和相位硬件损伤，通过失真感知加权实现可靠检测，相比传统检测器显著降低错误平层。


<details>
  <summary>Details</summary>
Motivation: 现有检测器主要处理相位失真，缺乏对幅度和相位硬件损伤的联合缓解方法，需要开发统一框架来应对实际收发机损伤。

Method: 将收发机失真建模为残余幅度和相位噪声，在极坐标域设计近似相位-幅度失真检测器，采用失真感知加权策略。

Result: PAD-D相比传统欧几里得检测器和基于高斯假设的相位噪声检测器，在高阶QAM和SAPSK星座下实现高达一个数量级的错误平层降低。

Conclusion: 该框架为实际收发机损伤下的检测提供了统一实用的解决方案，并进一步开发了针对PAD-D优化的星座设计，验证了分析SEP近似的准确性。
...
TL;DR: 提出了一种新型柔性连续孔径阵列(FCAPA)电磁结构，通过将表面柔性融入典型连续孔径阵列系统，增强了MIMO系统的自由度。


<details>
  <summary>Details</summary>
Motivation: 增强连续孔径阵列系统的自由度，提升MIMO系统性能。

Method: 提出柔性连续孔径阵列结构，并制定和求解下行链路多用户波束成形优化问题以最大化加权和速率。

Result: FCAPA结构在性能上大幅优于典型CAPA系统，且性能随变形能力增强而提升。

Conclusion: 柔性连续孔径阵列通过增加表面柔性有效提升了MIMO系统性能。
...
TL;DR: 本文提出了一个物理一致的双基地感知框架，采用二维波导馈电超表面天线阵列实现超大规模MIMO孔径。通过耦合偶极子模型捕捉阵列互耦效应，并提出了磁极化率的无源性约束。在双基地感知设置下，利用诺伊曼级数近似推导多目标参数估计的克拉美-罗界，并将其融入超表面单元共振强度配置的感知优化中。


<details>
  <summary>Details</summary>
Motivation: 开发能够实现超大规模MIMO孔径的双基地感知系统，解决波导和自由空间互耦效应带来的挑战，优化超表面阵列在强耦合环境下的感知性能。

Method: 提出耦合偶极子模型捕捉阵列互耦效应，引入磁极化率的无源性约束，利用诺伊曼级数近似阵列响应模型，推导多目标参数估计的克拉美-罗界，并将其融入超表面单元共振强度配置的优化框架。

Result: 仿真结果表明，在辐射近场中提出的设计方案能够量化位置误差界限，揭示了超材料布局在强耦合超表面XL MIMO双基地感知系统中的关键作用。

Conclusion: 该物理一致框架成功解决了超大规模MIMO双基地感知中的互耦问题，证明了超材料布局对系统感知性能的重要影响，为强耦合环境下的感知系统设计提供了理论基础和优化方法。
...
TL;DR: 本文开发了基于KLM模型的完整系统级灵敏度优化分析模型，建立了包含换能器、电缆和接收器的端到端等效电路，揭示了关键参数间的耦合效应，并通过实验验证了模型准确性。


<details>
  <summary>Details</summary>
Motivation: 光声成像中声学检测子系统的灵敏度严重影响图像质量，但以往研究只关注前端声学组件或后端电子组件，忽略了换能器、电缆和接收器之间的端到端耦合效应。
...
Conclusion: 这项工作为优化光声成像系统检测灵敏度和提高图像保真度提供了全面框架。
...
TL;DR: 提出增强型DAS框架，结合STFT时频特征提取和视频到DAS跨模态预训练，显著提升分布式光纤声学传感的性能和泛化能力


<details>
  <summary>Details</summary>
Motivation: 解决DAS-MAE在时域主导数据中频率分析不足的问题，并缓解Transformer架构对大量训练数据的需求

Method: 使用短时傅里叶变换进行显式时频特征提取，并开创性地采用视频到DAS的跨模态预训练方法

Result: 在少样本分类中达到0.1%错误率（相对DAS-MAE提升90.9%），外部损伤预防应用中识别错误率为4.7%（相对从头训练提升75.4%）

Conclusion: 该框架通过连接计算机视觉和分布式传感领域，扩展了可用训练资源，促进了DAS在工业物联网中的部署
...
TL;DR: 本文提出了一种降低复杂度的多模型编码方法，通过联合优化第一阶段参数模型和比特率分配以及第二阶段残差压缩方法，在满足重构信号质量约束的同时减少比特率。


<details>
  <summary>Details</summary>
Motivation: 传统多模型编码方法产生恒定速率的比特流但质量变化，需要一种能够满足特定失真约束的优化方法。

Method: 提出了三种方法：穷举搜索作为基准；黄金分割搜索降低第一阶段速率选择的复杂度；利用率失真模型筛选有前景的模型并缩小搜索区间。

Result: 仿真结果表明，所提出的降低复杂度MMC方法在相同复杂度下，相比现有VCS解决方案，在给定失真约束下减少了比特率。

Conclusion: 该方法有效降低了多模型编码的复杂度，同时保持了信号重构质量，在电压电流信号压缩方面优于现有技术。
...
TL;DR: 提出XG-ZEDs——一种基于反向散射的无电池、频谱无关、跨代兼容的物联网设备，利用环境无线信号实现通信、感知和定位，解决6G物联网的能源和可持续性挑战。


<details>
  <summary>Details</summary>
Motivation: 6G网络中大规模物联网连接带来的能源消耗、电池浪费和生命周期可持续性挑战，当前蜂窝物联网解决方案受限于网络代际寿命并依赖数十亿一次性电池，造成不可持续的经济和环境成本。

Method: 提出XG-ZEDs（跨代零能耗设备），基于反向散射技术，利用现有环境无线信号进行通信、感知和定位，无需电池且频谱无关。

Result: XG-ZEDs将基础设施和用户设备转变为超低功耗连接的通用使能器，支持传感、定位和无线电SLAM等应用。

Conclusion: XG-ZEDs为6G物联网提供了可持续的解决方案，但仍需解决架构分类、通信协议、网络集成等方面的挑战。
...
TL;DR: 提出基于大语言模型的波束训练框架，用于多用户MIMO通信中的夹持天线系统，通过上下文感知和环境自适应探测来减少开销，在单用户和多用户场景下均显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决传统波束训练开销大的问题，利用大语言模型实现智能化的波束训练，使夹持天线系统能够进行上下文感知和环境自适应的探测。

Method: 1) 单用户场景：构建LLM夹持波束成形码本生成问题，采用最大比传输获得最优发射波束成形；2) 多用户场景：基于系统总速率构建联合码本生成和波束选择问题，使用MMSE波束成形，通过端到端训练GPT模型最小化交叉熵损失。

Result: 1) 单用户场景：波束选择Top-1准确率超过95%，波束成形增益比传统方法提升51.92%；2) 多用户场景：总速率比基于LLM的大规模MIMO和传统PASS波束训练分别提升57.14%和33.33%。

Conclusion: 基于大语言模型的波束训练框架在夹持天线系统中表现出色，显著提升了波束训练的性能和效率，为智能通信系统提供了有效解决方案。
...
Method: 结合从空间协方差估计中寻找低秩投影与快速多项式矩阵逆的方法

Result: 射线追踪仿真显示，相对于完整的瞬时波束成形，该方法性能损失极小，同时提供显著的开销和计算增益

Conclusion: 该方法能够有效解决大规模MIMO系统的可扩展性问题，在保持性能的同时大幅降低计算复杂度和开销
...
<details>
  <summary>Details</summary>
Motivation: 研究流体天线系统中多端口激活和最大比合并技术对系统性能的提升，特别是在Rician衰落信道环境下。

Method: 使用拉普拉斯变换方法推导合并后信噪比的统计特性，分析中断概率，并给出了中断概率的下界和在高信噪比下的渐近表达式。

Result: 数值结果验证了分析框架的有效性，并展示了关键系统参数对基于MRC的流体天线系统性能的影响。

Conclusion: 提出的分析框架能够有效评估基于MRC的流体天线系统性能，为系统设计提供了理论依据。
...
<details>
  <summary>Details</summary>
Motivation: 在6G无线网络背景下，RIS能够智能控制电磁波传播并增强回程通信性能，需要研究其在混合网络架构中的应用。

Method: 推导了中断概率和误码率等关键性能指标的闭式表达式，进行了渐近分析确定系统分集阶数，并通过蒙特卡洛仿真验证结果。

Result: 提出的混合模型比传统RF-FSO框架性能提升52.54%，空中RIS集成使系统性能提升41.39%，将RIS放置在较低高度且与通信端点等距较短距离可显著改善性能。

Conclusion: 该混合网络架构结合FSO/THz和空中RIS-RF链路，通过优化RIS部署位置和切换策略，能够显著提升6G无线网络的回程通信性能。
...
TL;DR: 本文通过将代数几何码的迹映射到二进制域，探索能否保持AG码超过Gilbert-Varshamov界的优势，提出了一种新的迹-AG码构造方法。


<details>
  <summary>Details</summary>
Motivation: 解决编码理论中最古老的问题之一：用显式二进制码匹配Gilbert-Varshamov界。传统方法使用级联进行字母表缩减，而本文希望利用AG码超过GV界的优势，通过迹映射来保持这种优势。

Method: 采用代数几何码的迹映射方法（TAG码），从常数域扩展到二进制域。开发了适用于TAG码分析的Hasse-Weil类型定理，比经典定理更适合该场景。

Result: 虽然未获得改进的构造，但证明了常数因子加强的界就足够。在高距离机制下，TAG码不如码级联方法有效。新定理在更广泛场景下适用，包括推导新的指数和估计。

Conclusion: TAG码方法提供了一种新的字母表缩减策略，其分析完全利用AG码的代数结构。虽然在高距离机制下不如级联，但为匹配GV界提供了新的研究方向。
...
TL;DR: 提出了一种基于策略引导的蒙特卡洛树搜索解码器，用于短块码的近最大似然解码，无需高斯消元，搜索复杂度降低95%。
...
Method: 使用蒙特卡洛树搜索在接收信息位中搜索测试错误模式，通过神经网络策略引导搜索过程，采用基于MCTS的学习训练策略。

Result: 在短块码上实现近最大似然解码性能，搜索复杂度降低95%，在高信噪比下解码延迟低于OSD和非GE OSD。

Conclusion: 该方法为短块码提供了一种高效的低复杂度近最大似然解码方案，具有实际应用价值。
...
TL;DR: 该论文提出了一种使用多重集而非有序序列的编码方案，适用于符号顺序无法维持或观察的场景，如传感器网络中的移动物体跟踪问题。


<details>
  <summary>Details</summary>
Motivation: 研究动机源于传感器网络中移动物体跟踪问题，以及在符号顺序无法维持或观察的场景下的编码需求。

Method: 采用sunmao构造方法，将原始源数据网格分解为子网格，在子网格上解决多重集编码问题，然后将子网格解拼接形成最终解。提出了braid码作为具体解决方案。

Result: braid码易于在多维网格上定义，对于给定码集大小和多重集基数的码，在编码效率方面具有与最优码相同的渐近阶。

Conclusion: braid码不仅编码效率高，还具有有趣的内在纠错特性，适用于多重集编码场景。
...
Method: 采用两阶段训练：连续预训练阶段，然后直接二值化并进行微调，不使用梯度近似技术。

Result: 在(7,4)块配置和二进制对称信道下，学习的编码器-解码器对学习到了最优汉明码的旋转版本（陪集码），自然恢复了其线性和距离特性，在最大似然解码下实现了相同的块错误率。

Conclusion: 紧凑的自动编码器架构可以通过稳定且直接的训练有效学习结构化的、代数最优的二进制码。
...
<details>
  <summary>Details</summary>
Motivation: 二进制MDS阵列码的修复问题，特别是单节点故障的修复，在分布式存储系统中具有重要意义。现有方法在子分组化程度和修复带宽方面存在优化空间。

Method: 通用构造I：基于任意二进制MDS阵列码，构造具有最优访问带宽的(k+r,k,ms^⌈(k+r)/s⌉)码；通用构造II：针对偶数r≥4且s=r/2的情况，构造具有最优修复带宽的(k+r,k,ms^(k+r)/(s+1))码。

Result: 通用构造I生成的码具有更小的子分组化和更大的系数矩阵选择灵活性；通用构造II生成的码在现有二进制MDS阵列码中具有最小的子分组化程度。

Conclusion: 两种通用构造方法有效降低了二进制MDS阵列码的子分组化程度，同时保持了最优的修复性能，为分布式存储系统提供了更高效的纠删码方案。
...
TL;DR: 提出在频域实现智能无线电环境的新方法——可移动信号，通过动态调整信号频谱在频轴上的位置来增强无线通信性能，无需电子可重构组件。


<details>
  <summary>Details</summary>
Motivation: 现有智能无线电环境技术（如可重构智能表面和柔性天线）依赖电子可重构或可移动组件，存在实现挑战并可能阻碍商业化。需要寻找替代方案。

Method: 在频域引入可移动信号概念，通过动态移动信号频谱来优化通信性能。分析在LOS和NLOS条件下MISO系统中的表现，并利用固定智能表面（FIS）的反射效应。

Result: 在LOS条件下，可移动信号比量化等增益传输获得更高平均接收功率；在NLOS条件下，FIS辅助系统使用可移动信号可实现比RIS辅助系统使用固定频率信号高4倍的接收功率。

Conclusion: 频域可移动信号是实现智能无线电环境的有前景的替代方案，能够克服现有技术的实现挑战，在LOS和NLOS条件下均表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 研究率失真理论中编码源的最小码本大小问题，同时允许重建中的一定误差，探讨优化测试信道的可计算性在理论和实践中的重要性。

Method: 通过分析率失真函数的凸优化问题，研究涉及互信息等信息论量的优化器的可计算性。

Result: 发现率失真函数通常可计算，但优化器在一般情况下不可计算，即使对于简单的失真度量也是如此。

Conclusion: 与信息论中其他问题的优化器类似，率失真函数的优化器在可计算性方面也表现出相似的结果，即通常不可计算。
