...
TL;DR: 提出首个基于Set Transformer的无蜂窝集成感知与通信波束赋形框架STCIB，通过注意力机制建模网络实体间的全局关系，在无监督下实现三种优化模式，相比现有方法显著降低计算复杂度并提升性能。
...
Method: 提出基于Set Transformer的STCIB框架，利用注意力机制显式建模网络实体间的全局关系，自然处理无序输入集并保持置换不变性，以无监督方式运行，支持感知中心、通信中心和联合ISAC三种优化模式。

Result: STCIB在ISAC性能上显著优于CNN基线，在η=0.4时，感知和通信总速率分别提升14.8%和31.6%，而运行时间仅增加0.26%。相比CCPA和ALM-MO优化算法，STCIB计算成本显著降低，在通信阈值为3.0 bps/Hz时，运行时间仅为CCPA的0.1%和ALM-MO的0.3%，同时感知总速率分别提升4.45%和5.9%。

Conclusion: STCIB是首个基于Set Transformer的无蜂窝ISAC波束赋形框架，能够有效建模全局关系，在保持高性能的同时显著降低计算复杂度，为大规模分布式ISAC系统提供了一种高效解决方案。
...
TL;DR: 提出一种基于光学无线通信的水下传感器到无人机数据中继方案，通过蒙特卡洛光线追踪模拟水-空气信道，分析气泡、海面波动和无人机不稳定性对通信性能的影响。


<details>
  <summary>Details</summary>
Motivation: 水下观测站需要高效、低成本的数据传输方案。传统方法效率低、成本高，因此研究利用光学无线通信实现水下传感器到无人机的水-空气直接通信链路。

Method: 1) 建立LED发射器与SiPM接收器的水-空气光学通信链路；2) 开发基于蒙特卡洛方法的光线追踪算法，结合Mie散射理论模拟气泡影响；3) 使用JONSWAP谱生成真实海面模型；4) 考虑风力导致的无人机不稳定性信道损耗；5) 分析风速、发射配置、接收特性等关键参数对信道的影响。

Result: 通过综合分析无线信道特性，评估了不同深度和数据速率下的端到端系统性能，分析了平均误码率，为所提方法的可行性和效率提供了有价值的见解。

Conclusion: 提出的水-空气光学通信方案能够有效实现水下传感器到无人机的数据传输，蒙特卡洛光线追踪模型能够准确表征复杂信道特性，为水下观测站数据中继提供了可行的解决方案。
...
TL;DR: 该研究比较了三种接收阵列配置对主动非相干毫米波成像性能的影响，发现优化的阵列设计能提高空间采样效率并改善重建质量。


<details>
  <summary>Details</summary>
Motivation: 在主动非相干毫米波成像系统中，阵列几何结构决定了视场和图像质量。本研究旨在探究阵列冗余和空间采样多样性对图像重建性能的影响，以优化阵列设计。

Method: 比较了三种接收阵列配置：一种简单圆形设计和两种通过优化策略获得的阵列。优化策略旨在最大化独特空间样本，同时保持系统分辨率和视场。使用38-GHz AIM成像系统进行实验验证，在48位置可重构孔径中实现24单元接收阵列。

Result: 实验结果表明，优化的阵列配置提高了空间采样效率，与传统的圆形阵列相比，在重建质量上获得了可测量的提升。通过结构相似性指数和峰值旁瓣水平等图像域指标评估，显示出更好的重建保真度和伪影抑制能力。

Conclusion: 阵列设计对主动非相干毫米波成像系统至关重要，优化的阵列配置能显著改善空间采样效率和图像重建质量，为AIM成像系统的阵列设计提供了重要指导。
...
TL;DR: 提出一种六维可移动天线架构，通过联合优化天线位置、旋转角度和波束赋形来缓解太赫兹通信中的波束斜视效应，实现宽带宽波束覆盖。


<details>
  <summary>Details</summary>
Motivation: 传统模拟波束赋形在宽带系统中因波束斜视效应性能下降，而使用真时延线成本高昂。需要一种成本效益高的解决方案来实现太赫兹通信的宽带传输。

Method: 提出6DMA架构，联合优化模拟波束赋形向量、天线在二维区域内的位置以及天线阵列的三维旋转角度。针对特殊情况推导最优解，对一般情况提出交替优化算法，结合连续凸逼近、吉布斯采样序列更新和粗细粒度搜索。

Result: 仿真结果表明，所提方案显著优于传统无移动或旋转的天线阵列，为太赫兹频段的宽带传输提供了成本效益高的解决方案。

Conclusion: 六维可移动天线架构能有效缓解波束斜视效应，通过天线位置和旋转的优化实现宽带宽波束覆盖，为太赫兹通信提供了有前景的低成本解决方案。
...
TL;DR: 该论文建立了复杂介质不确定性对FDA-MIMO探地雷达杂波统计特性的理论分析框架，填补了现有研究中缺乏显式统计建模的空白。


<details>
  <summary>Details</summary>
Motivation: 现有FDA-MIMO杂波研究很少在显式统计框架中考虑地下介质的色散、耗散和随机非均匀性，存在建模空白。

Method: 采用连续弛豫谱描述复杂介质，建立从随机弛豫谱扰动到复杂介电常数、复波数、导向矢量扰动、介质诱导附加杂波协方差和总杂波协方差的统计传播链。

Result: 数值结果验证了理论与蒙特卡洛样本统计的一致性，表明介质不确定性不仅改变协方差矩阵元素，还重塑其特征谱和有效子空间，影响目标与杂波的几何分离。

Conclusion: 该研究为将复杂介质不确定性嵌入FDA-MIMO-GPR杂波统计分析提供了显式且可解释的理论接口。
...
TL;DR: 提出一种基于多模态感知辅助的自适应波束探测策略，通过感知信息预测波束质量，使用深度Q集成估计不确定性，自适应控制探测开销，显著减少毫米波/太赫兹链路的波束对准训练开销。


<details>
  <summary>Details</summary>
Motivation: 毫米波/太赫兹链路需要快速波束对准，但传统的码本扫描方法训练开销过大。需要利用多模态感知信息来减少探测开销，同时保证链路可靠性。

Method: 1) 将多模态感知（雷达/LiDAR/相机）映射到波束先验分布；2) 使用深度Q集成预测每个波束的奖励，以集成分歧作为认知不确定性代理；3) 基于Prior-Q上置信分数调度小规模探测集；4) 根据先验熵自适应调整探测预算，将感知置信度与通信开销耦合；5) 使用基于裕度的安全规则防止低SNR锁定。

Result: 在DeepSense-6G数据集（训练：场景42和44；测试：场景43）上，使用21波束DFT码本，Top-1/Top-3准确率达到0.81/0.99，每次扫描平均仅需探测2个波束，在θ=0 dB、裕度Δ=3 dB条件下实现零中断。相比消融实验，多模态先验与集成不确定性匹配了链路质量并提高了可靠性。

Conclusion: 提出的多模态感知辅助自适应探测策略能够有效减少毫米波/太赫兹链路的波束对准训练开销，同时保持高可靠性和链路质量，通过更好的预测模型实现了开销削减。
...
TL;DR: 该论文分析了亚太赫兹频段MIMO系统中相位噪声对混合预编码的影响，推导了频谱效率下界和误码率闭式解，验证了单导频符号可有效抑制相位噪声，并比较了传统QAM与针对相位噪声优化的极坐标QAM性能。


<details>
  <summary>Details</summary>
Motivation: 亚太赫兹频段虽然能提供大规模MIMO技术和高频谱效率，但硬件实现面临相位噪声挑战。混合预编码作为全数字预编码的低成本替代方案，需要研究相位噪声对其性能的影响，以设计有效的抑制方案。

Method: 采用高斯相位噪声模型，基于奇异值分解和公共本地振荡器架构分析MIMO系统。推导了频谱效率下界，提供了4-QAM和16-QAM在高信噪比和强相位噪声条件下的误码率闭式表达式，并通过蒙特卡洛仿真验证。

Result: 分析表明，在单用户MIMO系统中，仅需单个导频符号即可有效抑制高斯相位噪声，而SISO系统则难以实现有效抑制。仿真结果显示，针对相位噪声优化的极坐标QAM相比传统QAM具有更好的性能。

Conclusion: 亚太赫兹频段MIMO系统可通过混合预编码和简单的导频方案有效应对相位噪声挑战。针对相位噪声优化的调制方案能进一步提升性能，为未来系统提供了性能提升和能效改进的方向。
...
TL;DR: 提出一个统一的增量开放集学习框架，用于基于RF的无人机识别，支持新类别发现和增量适应，解决实际部署中未知无人机类型的问题。


<details>
  <summary>Details</summary>
Motivation: 民用和商用无人机的普及增加了对可靠RF识别系统的需求。现有方法大多采用封闭集假设，假设训练时已知所有无人机类型，这在实践中不现实，因为新无人机类型不断出现，导致过度自信的错误分类和低效的重新训练。

Method: 提出一个三阶段框架：1) 开放集识别，在语义特征空间中将未知信号与已知类别分离；2) 无监督聚类模块，基于综合有效性分数选择K-Means或高斯混合模型来发现新无人机类别；3) 轻量级增量学习模块，通过内存受限的重放机制整合新发现的类别，减轻灾难性遗忘。

Result: 在包含24个类别（18个已知，6个未知）的真实世界无人机RF数据集上进行实验，结果显示有效的开放集检测、在评估的噪声设置下有前景的聚类性能，以及稳定的增量适应且存储成本最小。

Conclusion: 该框架支持开放世界无人机识别，能够处理动态和不确定空域条件下新出现的未知无人机类型，具有实际部署潜力。
...
<details>
  <summary>Details</summary>
Motivation: ITU将"AI与通信"列为6G六大关键应用场景之一，智能体AI和多模态感知、复杂任务协调、持续自优化能力将推动基于智能体的通信网络发展。语义通信作为变革性范式，提供任务导向效率、复杂环境可靠性增强和动态资源分配适应能力。然而，在智能体通信背景下，系统梳理两者技术演进关系的综述仍然缺乏。
...
Result: 建立了完整的语义增强智能体通信网络理论框架，系统梳理了该领域的研究现状，识别了关键挑战并提出了潜在解决方案，为这一新兴领域的未来研究提供了方向性指导。

Conclusion: 语义通信与智能体AI的结合为6G通信网络带来了变革性潜力，提出的三层四实体四阶段架构为理解和设计语义增强的智能体通信网络提供了系统框架，未来研究需要解决语义理解、资源分配、安全隐私等关键挑战。
...
TL;DR: 提出精确紧凑的表达式描述交错ADC中偏移、增益和时序偏差对输出频谱的影响，推导杂散功率分布，并提供基于目标良率的校准步长设计实例


<details>
  <summary>Details</summary>
Motivation: 交错ADC在需要多GS/s速率的应用中至关重要，但其性能常受子ADC间的偏移、增益和时序偏差不匹配的限制，需要系统分析这些非理想性对频谱的影响

Method: 提出精确但紧凑的数学表达式来描述每种非理想性对输出频谱的影响，推导诱导杂散和谐波功率的分布，建立基于良率的子ADC规格推导方法

Result: 开发了分析交错ADC非理想性的理论框架，能够量化偏移、增益和时序偏差对频谱的影响，并推导出杂散功率的统计分布，为良率导向的设计提供基础

Conclusion: 该研究为交错ADC设计提供了系统分析工具，能够基于目标生产良率推导子ADC规格和校准步长，有助于提高交错ADC的实际性能和制造良率
...
TL;DR: 本文提出了一种理论指导的注意力网络(JSSAnet)，用于解决6G通信中ELAA系统的近场信道估计问题，通过分段傅里叶表示和联合子信道-空间注意力机制，显著提升了估计性能。


<details>
  <summary>Details</summary>
Motivation: 6G通信系统中部署的超大规模天线阵列(ELAA)给近场信道估计带来了独特挑战，传统方法难以有效处理近场信道的非线性特性，需要新的理论指导的估计框架。

Method: 提出分段傅里叶表示将非线性近场信道分解为多个子信道，每个子信道通过DFT映射到角度域；设计联合子信道-空间注意力网络(JSSAnet)，包含JSSA层和FFN，前者基于理论推导的上下界分配自适应空间注意力权重，后者捕获子信道间的非线性依赖关系；采用大核卷积(DLKC)线性计算JSSA映射。

Result: 数值结果表明，将稀疏信息嵌入注意力网络是有效的，JSSAnet在估计性能上优于现有方法，验证了理论指导设计的优越性。

Conclusion: 提出的理论指导方法成功解决了ELAA近场信道估计的挑战，分段傅里叶表示和联合注意力机制为6G通信系统的信道估计提供了有效解决方案，具有实际应用价值。
...
TL;DR: DeepOFW是一个基于深度学习的OFDM灵活波形调制框架，通过端到端优化降低PAPR并提升BER性能，同时保持传统收发器的硬件结构。


<details>
  <summary>Details</summary>
Motivation: 多载波调制（如OFDM）中的峰均功率比（PAPR）问题限制了功率放大器效率和实际发射功率，需要一种既能降低PAPR又能保持硬件效率的解决方案。

Method: 提出完全可微分的DeepOFW框架，在训练中联合优化波形表示和检测参数，并显式加入PAPR约束。学习阶段集中在离线或集中单元，部署时无需额外计算开销。
...
TL;DR: 本文解决了长期存在的样本压缩猜想，证明了对于VC维度为d的概念类，存在大小为d的标记样本压缩方案


<details>
  <summary>Details</summary>
Motivation: 样本压缩猜想是学习理论中的一个长期开放问题，该猜想提出：每个VC维度为d的概念类都存在大小为d的压缩方案。这个猜想自提出以来一直未得到完全解决，对理解概念类的可压缩性和学习理论具有重要意义。

Method: 对于任意有限函数的概念类，作者提出了一种标记样本压缩方案。该方案的核心是构建一个压缩函数，能够将任意标记样本集压缩到大小为VC维度d的压缩集，同时保证能够从压缩集恢复原始概念。

Result: 成功证明了对于任何概念类，都存在大小为VC维度d的标记样本压缩方案，从而完全解决了长期存在的样本压缩猜想。

Conclusion: 本文彻底解决了样本压缩猜想，为学习理论中的压缩方案研究提供了完整的理论保证，证明了VC维度不仅刻画了概念类的学习复杂度，也完全刻画了其可压缩性。
...
TL;DR: 提出并评估了一种带软信息的反向协调方案在超低信噪比下的性能


<details>
  <summary>Details</summary>
Motivation: 需要解决反向协调问题，特别是在超低信噪比条件下，寻找一种通用的解决方案

Method: 使用带软信息的反向协调方案，并在超低信噪比条件下进行性能评估

Result: 证明了该方案在超低信噪比下表现良好，是一种通用的反向协调解决方案

Conclusion: 带软信息的反向协调方案是解决反向协调问题的通用有效方法，特别适用于超低信噪比环境
...
TL;DR: 提出一种混合信道预测框架，结合未校准数字孪生与高斯过程，利用几何先验信息减少测量开销，提高预测精度。


<details>
  <summary>Details</summary>
Motivation: 传统高斯过程预测依赖密集测量且未利用环境几何信息，数字孪生需要昂贵校准。需要一种能结合两者优势、降低测量成本的方法。

Method: 使用开源地图构建未校准数字孪生提取几何先验信息，与少量信道测量融合到高斯过程中，利用GP不确定性量化指导测量点选择。

Result: 该方法减少了测量开销，提高了信道统计量预测精度，建立了资源高效的无线信道预测实用方法。

Conclusion: 通过将不完美的数字孪生与统计学习结合，实现了测量成本降低和预测性能提升，为资源受限的无线信道预测提供了实用框架。
...
TL;DR: 开发了基于测量校准的AI辅助数字孪生框架，用于太赫兹无线数据中心，通过整合信道测量、射线追踪和隐式神经场建模，实现对复杂室内环境的准确信道表征和系统级性能评估。
...
Method: 1. 在300 GHz频段进行LOS和NLOS场景下的信道测量；2. 在Sionna平台上进行射线追踪仿真；3. 基于测量和RT数据开发RT条件化的隐式神经场模型，构建连续射频场表示；4. 建立数字孪生框架整合所有组件。

Result: 射线追踪仿真与测量结果具有良好一致性；RT条件化的INF模型能够在RT缺失的NLOS区域进行准确预测；数字孪生生成的综合射频地图可为无线数据中心提供系统级分析和决策支持。

Conclusion: 提出的测量校准AI辅助数字孪生框架能够有效解决太赫兹无线数据中心在复杂室内环境中的信道表征挑战，为系统级性能评估和优化提供了可靠工具。
...
<details>
  <summary>Details</summary>
Motivation: MoE架构虽然通过稀疏激活提高了大语言模型的可扩展性，适合边缘网络部署，但专家数量常超过单个边缘节点的内存容量，需要将专家分布在多个节点上进行无线分布式推理。专家选择直接影响通信成本，因此需要优化选择策略。

Method: 首先建立专家替换或跳过导致的精度下降理论界限，基于此界限在延迟和精度约束下制定WIDE推理中的专家选择能量最小化问题。针对慢衰落信道，推导出单令牌解码和多令牌预填充的最优专家选择策略；针对快衰落信道，扩展方案以适应快速变化的信道条件。

Result: 仿真结果表明，与传统WIDE系统中的Top-K路由相比，SiftMoE在保持推理精度的同时显著降低了能耗。

Conclusion: SiftMoE通过理论分析指导的专家选择策略，有效解决了无线分布式MoE推理中的通信成本与精度权衡问题，为边缘网络部署提供了高效解决方案。
...
TL;DR: 提出一种基于三色激光激发的里德堡原子量子接收器架构，解决传统二色架构的成本、灵敏度限制和低频检测问题，实现更好的灵敏度和频谱接入范围。


<details>
  <summary>Details</summary>
Motivation: 传统二色四能级里德堡原子量子接收器面临三个主要问题：1) 依赖不稳定蓝色激光导致高成本和工程挑战；2) 热原子中残余多普勒展宽造成的基本灵敏度限制；3) 双光子共振能级约束导致无法检测低频波段。

Method: 提出三色五能级RAQR架构，采用全红/红外激光，实现三光子共振。结合原子物理和通信理论，推导端到端等效基带信号模型，采用Liouvillian超算符形式为实际RAQR提供精确数值解。

Result: 数值结果表明，三色五能级RAQR相比传统二色四能级RAQR和基于导体天线的经典接收器，实现了更优的灵敏度。同时揭示了固有的灵敏度-容量权衡关系。

Conclusion: 三色五能级RAQR架构不仅解决了工程成本问题，还能有效消除多普勒效应并实现低频检测，更适合在功率受限且需要广泛频谱接入的通信场景中部署。
...
TL;DR: 提出一种基于电磁焦散效应的物理信息鲁棒波束赋形策略，通过仅使用相位偏移实现近场物理层安全，显著提升对窃听者定位误差的鲁棒性


<details>
  <summary>Details</summary>
Motivation: 在近场超大孔径阵列中，即使窃听者位置有微小估计误差也会导致严重的焦点偏移和保密率下降，需要设计对定位误差鲁棒的波束赋形方案

Method: 将发射阵列划分为焦散子阵列和聚焦子阵列，利用电磁焦散效应绕过潜在窃听区域同时照射合法用户，通过相位梯度与电磁波出射角的关系推导分段闭式阵列相位分布

Result: 仿真结果表明，在0.25米定位误差下，所提方案能将最坏情况窃听率降低高达80%，显著优于传统方法

Conclusion: 基于电磁焦散效应的物理信息鲁棒波束赋形策略能有效提升近场物理层安全，仅需相位偏移即可实现，对窃听者定位误差具有强鲁棒性
...
TL;DR: 该论文挑战了语义通信系统易受对抗攻击的普遍观点，发现语义通信实际上比传统分离编码系统具有更强的对抗鲁棒性，需要14-16倍攻击能量才能达到相同失真。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为基于深度联合信源信道编码的语义通信会继承深度学习的对抗脆弱性，但本文质疑这一观点，旨在揭示语义通信系统可能具有意想不到的对抗鲁棒性。

Method: 1) 理论分析：利用Lipschitz平滑性建立最小攻击能量与目标失真的基本界限；2) 提出两种新颖攻击方法：结构感知脆弱集攻击（利用LDPC码图论脆弱性）和渐进梯度上升攻击（利用DeepJSCC可微性）；3) 通过噪声训练证明解码器平滑性提供内在保护。

Result: 实验表明语义通信需要比传统系统多14-16倍攻击能量才能达到相同失真，证实了其优越的对抗鲁棒性。理论证明噪声训练产生的隐式正则化强制解码器平滑性，提供内置保护。

Conclusion: 语义通信系统具有超出预期的对抗鲁棒性，挑战了传统观点。这种鲁棒性源于深度联合信源信道编码的内在特性，特别是噪声训练产生的解码器平滑性，为设计更安全的通信系统提供了新视角。
...
TL;DR: 提出了一种交替优化框架，用于在RIS辅助的分布式MIMO系统中最大化能量效率，同时考虑了相干和非相干接收模式。


<details>
  <summary>Details</summary>
Motivation: 在未来的6G网络中，RIS辅助的分布式MIMO系统需要提高能量效率，但现有的优化方法在处理非凸问题时存在挑战，特别是在联合优化AP功率分配和RIS相位配置方面。

Method: 采用交替优化框架，结合majorization-minimization方法进行功率分配，以及逐元素RIS自适应来联合优化AP功率分配和RIS相位配置，解决非凸优化问题。

Result: 仿真结果表明，在室内部署场景下，该方案在能量效率方面优于等功率分配和随机散射基线，并量化了RIS相位优化、控制器架构（集中式vs分布式）和RIS尺寸对性能的影响。

Conclusion: 该框架为实际RIS辅助的分布式MIMO部署提供了有效的设计见解，特别是在6G网络中，通过联合优化功率分配和RIS配置可以显著提高能量效率。
...
TL;DR: 本文全面研究了加权射影Reed-Muller码的基本结构性质，包括递归构造、广义汉明重量界、子域子码和其对偶码的构造，以及对偶码在低次情况下的描述和Schur积分析。


<details>
  <summary>Details</summary>
Motivation: 研究加权射影Reed-Muller码的结构性质，为这类代数几何码提供系统性的理论框架，包括构造方法、参数分析和代数性质。

Method: 1. 在权重满足某些条件下给出递归构造方法；2. 利用递归构造推导广义汉明重量界；3. 递归构造子域子码和对偶码；4. 对偶码在一般情况下的研究，特别在低次时给出评估码描述；5. 分析非退化情况下的Schur积。

Result: 建立了加权射射影Reed-Muller码的递归构造框架，获得了广义汉明重量的界，提供了子域子码和对偶码的构造方法，给出了对偶码在低次时的具体描述，并分析了Schur积的性质。

Conclusion: 本文系统性地研究了加权射影Reed-Muller码的结构性质，提供了多种构造方法和分析工具，为这类代数几何码的理论和应用奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 传统MDP卷积码需要较大的域尺寸，且当码参数较小时无法纠正较大的突发擦除。需要一种能在较小域上构造且能处理较大突发擦除的卷积码

Method: 提出伪MDP卷积码，放宽对列距离的要求，只要求部分而非全部列距离最优，从而能在较小域上构造

Result: 伪MDP卷积码能在较小域上构造，同时保持MDP码的部分优点，能够纠正较大突发擦除

Conclusion: 伪MDP卷积码提供了一种在域尺寸和纠错能力之间的折衷方案，适用于需要处理突发擦除且域尺寸受限的场景
...
TL;DR: 本文提出了基于超正则矩阵构造m维MDS卷积码的新方法，特别是针对码率1/n的情况，显著扩展了已知的MDS mD卷积码构造集合。


<details>
  <summary>Details</summary>
Motivation: 研究m维卷积码作为一维卷积码的自然推广，探索能够达到m维广义Singleton界的最大距离可分(MDS)卷积码的构造方法。

Method: 利用特定矩阵的超正则性(superregularity)来构造MDS mD卷积码，特别是针对码率1/n的情况，通过生成矩阵满足特定行度条件来实现。

Result: 开发了新的MDS mD卷积码构造方法，建立了新的码率1/n的MDS mD卷积码族，显著扩展了已知的MDS mD卷积码构造集合。

Conclusion: 基于超正则矩阵的构造方法为MDS mD卷积码提供了新的有效途径，特别是在码率1/n的情况下，这些构造丰富了m维卷积码的理论体系。
