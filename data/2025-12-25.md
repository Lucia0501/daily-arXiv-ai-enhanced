...
### [1] [Uplink RSMA Performance Analysis with Rate Adaptation: A Stochastic Geometry Approach](https://arxiv.org/abs/2512.20883)
*Xinyi Guo,Li You,Qiong Liu,Xiqi Gao,Xiang-Gen Xia*

Main category: cs.IT

TL;DR: 提出一个基于随机几何的统一分析框架，用于大规模上行链路速率分割多址接入网络，结合有限调制编码方案速率适配，量化干扰耦合和离散速率行为。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注下行链路和单小区设计，大规模部署下的上行链路RSMA建模和分析尚未充分探索，需要建立理论可处理性和实际现实性之间的桥梁。

Method: 基于随机几何建立统一分析框架，集成有限MCS速率适配，联合捕捉空间干扰耦合和离散速率行为，推导条件接收速率、空间平均值和元分布等高阶统计量。

Result: 该框架不仅推广了现有NOMA和OMA分析，还揭示了离散速率适配如何重塑密集RSMA网络的干扰动态和公平性，提供了新的洞察。

Conclusion: 提出的统一框架为大规模上行链路RSMA网络提供了理论分析和实用洞察，为下一代无线网络的高效干扰管理提供了分析工具。
...
### [2] [Knowledge-Driven 3D Semantic Spectrum Map: KE-VQ-Transformer Based UAV Semantic Communication and Map Completion](https://arxiv.org/abs/2512.20984)
...
<details>
  <summary>Details</summary>
Motivation: 在智能通信网络中，3D频谱地图对于频谱监测至关重要，但在复杂通信环境和稀疏采样数据下，以频谱高效、计算高效和AI驱动的方式获取和传输3D频谱地图具有挑战性

Method: 提出知识增强的语义频谱地图补全框架，包含物理信号传播模型驱动的两个专家知识约束；开发KE-VQ-Transformer多尺度低复杂度智能补全方法，采用稀疏窗口避免超大3D注意力计算；引入KMSE和RKMSE作为新评估指标，开发联合离线在线训练方法

Result: 仿真结果表明，所提方案在RKMSE指标上优于现有基准方案

Conclusion: 通过结合物理知识和深度学习，提出的知识增强框架能够更好地捕捉真实世界物理特性，避免陷入表面数据分布的思维定式，实现更准确和物理一致的频谱地图补全
...
### [3] [Coding-Logic Correspondence: Turning Information and Communication Networks into Logical Formulae via Hypergraph Heyting Algebra](https://arxiv.org/abs/2512.21112)
*Cheuk Ting Li*

Main category: cs.IT

TL;DR: 提出使用混淆超图（超混淆）作为信息模型，替代传统随机变量方法，建立Heyting代数结构，将通信网络需求表达为逻辑公式，直接计算最优编码方案


<details>
  <summary>Details</summary>
Motivation: 传统基于随机变量的信息模型无法直接处理信息的合取、析取和蕴含运算，需要一种新的代数结构来统一表达通信网络的各种编码需求

Method: 使用混淆超图作为信息模型，建立Heyting代数结构，将网络编码、索引编码、Slepian-Wolf编码等通信需求表达为直觉主义逻辑公式，通过超图Heyting代数直接计算最优编码方案

Result: 最优通信成本由超图的熵给出（在对数差距内），建立了编码设置与逻辑公式之间的对应关系，类似于Curry-Howard对应中证明与计算机程序的关系

Conclusion: 混淆超图模型为信息理论提供了新的代数框架，统一了多种编码问题，揭示了编码理论与逻辑之间的深刻联系
...
### [4] [Learning-Enabled Elastic Network Topology for Distributed ISAC Service Provisioning](https://arxiv.org/abs/2512.20722)
*Jie Chen,Xianbin Wang*

Main category: eess.SP

TL;DR: 提出弹性网络拓扑(ENT)用于分布式ISAC服务，通过动态聚合CCN为CFN来平衡信令开销和资源利用，采用两阶段协议和MADRL框架优化
...
Method: 提出弹性网络拓扑(ENT)，允许多个CCN动态聚合为CFN进行联邦操作；设计两阶段协议：阶段I自主分类ISAC服务并划分资源，阶段II分别使用专用和共享资源；提出USR指标量化效用与信令开销权衡，采用MADRL框架进行集中训练分散执行优化

Result: 通过弹性拓扑实现CCN和CFN边界的弹性编排，平衡信令开销和分布式资源利用，实现高效ISAC服务提供

Conclusion: ENT架构能够灵活支持分布式ISAC服务，通过动态网络拓扑调整和MADRL优化，有效解决了传统网络拓扑刚性与异构服务需求不匹配的问题
...
### [5] [Balancing Decentralized Trust and Physical Evidence: A Blockchain-Physical Layer Co-Design for Real-Time 3D Prioritization in Disaster Zones](https://arxiv.org/abs/2512.20824)
*Mohammad Taghi Dabiri,Mazen Hasna,Aiman Erbad,Rula Ammuri,Khalid Qaraqe*

Main category: eess.SP

TL;DR: 论文提出基于区块链和无人机验证的双层架构，用于灾难响应中的实时可信协调，通过数字投票和物理层验证结合，解决虚假信息和通信中断问题。


<details>
  <summary>Details</summary>
Motivation: 当前灾难响应协调模型面临不可靠数据、故意虚假信息和关键通信基础设施中断的问题，需要一种能够实现实时可信协调的解决方案。

Method: 采用区块链驱动的双层架构：第一层是数字投票系统，节点提交时间戳和地理标记的投票到公共账本；第二层是物理验证层，移动无人机验证器执行物理层认证，并向区块链发出独立位置标志，形成双签名机制。

Result: 提出了一种融合不可变数字记录和感官基础信任的解决方案，分析了欺骗、投票模糊性、验证器受损和连接丢失等技术挑战，并提出了分层缓解策略。具体实例化包括利用调制后向反射器传感器和3D感知视距放置的无人机映射方案。

Conclusion: 区块链和无人机物理验证相结合的双层架构为灾难响应提供了弹性、信任锚定的协调路径，能够提高危机地图的可信度和响应效率。
...
### [6] [Towards City-Scale Quantum Timing: Wireless Synchronization via Quantum Hubs](https://arxiv.org/abs/2512.20827)
*Mohammad Taghi Dabiri,Mazen Hasna,Rula Ammuri,Saif Al-Kuwari,Khalid Qaraqe*

Main category: eess.SP

TL;DR: 提出一种基于纠缠光子对和被动角立方体反射器阵列的城市规模无线量子同步框架，实现亚纳秒级同步和定位
...
Method: 采用集中式量子中心发射纠缠光子对，一个射向目标设备，另一个射向本地参考单元；目标设备配备平面CCR阵列被动反射光子，通过量子测量实现同步；建立包含高斯光束扩展、空间失准、大气湍流等物理层现象的完整分析模型

Result: 推导出Gamma-Gamma衰落下的单光子检测概率闭式表达式，建立光子到达事件和同步误差模型；仿真验证了分析模型的准确性，揭示了光束腰、CCR阵列尺寸和背景光之间的关键权衡关系
...
### [7] [The Area Signal-to-Noise Ratio: A Robust Alternative to Peak-Based SNR in Spectroscopic Analysis](https://arxiv.org/abs/2512.20830)
*Alex Yu,Huaqing Zhao,Lin Z. Li*

Main category: eess.SP

TL;DR: 提出基于面积的信号噪声比(aSNR)作为传统基于峰值的pSNR的改进方案，通过积分区域信号提高检测性能，特别适用于宽峰


<details>
  <summary>Details</summary>
Motivation: 传统基于峰值的信噪比(pSNR)存在对噪声尖峰敏感、对宽峰效果差等局限性，需要更鲁棒的替代方案

Method: 引入基于面积的信号噪声比(aSNR)，在感兴趣区域内积分信号；使用蒙特卡洛模拟(每种条件2000次试验)测试高斯、洛伦兹和Voigt线型

Result: aSNR比pSNR需要更低的振幅就能达到50%检测概率；ROC曲线显示在低振幅下aSNR性能更好；特别适用于宽峰检测

Conclusion: aSNR是比pSNR更鲁棒的替代方案，可扩展到多维光谱的体积基SNR，在宽峰检测方面具有显著优势
...
### [8] [QoS- and Physics-Aware Routing in Optical LEO Satellite Networks via Deep Reinforcement Learning](https://arxiv.org/abs/2512.20835)
*Mohammad Taghi Dabiri,Rula Ammuri,Mazen Hasna,Khalid Qaraqe*

Main category: eess.SP

TL;DR: 提出一种轻量级几何与QoS感知路由框架，用于光学LEO卫星网络，通过DRL结合物理约束实现动态路由优化


<details>
  <summary>Details</summary>
Motivation: 光学星间链路(ISL)在现代大规模LEO星座中成为主要通信骨干，但光学波束对卫星相对运动、指向抖动和快速变化的几何结构极为敏感，使得路由比RF系统更具挑战性。特别是平面内和平面间ISL表现出明显不同的稳定性和可行范围，形成动态、部分受限的连接结构，需要物理一致的路由策略。

Method: 提出轻量级几何与QoS感知路由框架，基于抖动感知高斯波束模型推导类别相关的可行性约束，将这些分析计算的门槛嵌入时变ISL图中，并通过深度强化学习(DRL)代理中的可行动作掩码强制执行。该方法利用局部几何进展、可行邻居结构和拥塞指标来选择下一跳中继，无需全局重新计算。

Result: 在类似Starlink的星座上的仿真结果表明，学习到的路径具有物理一致性，能够利用平面内稳定性，适应抖动限制的平面间连接性，并在动态拓扑演化下保持稳健的端到端延迟。

Conclusion: 该研究提出了一种将物理约束直接集成到路由决策中的有效方法，通过DRL和几何感知约束实现了光学LEO网络中动态、物理一致的路由优化。
...
### [9] [When Future Communications Shift Toward Narrow Beams: A Forward Looking Survey on Pointing Errors and Alignment Limits](https://arxiv.org/abs/2512.20874)
...
TL;DR: 该论文对自由空间光学、毫米波和太赫兹系统中的指向误差进行了统一综述，建立了跨技术的分类体系，分析了指向误差模型及其对系统性能的影响，并系统调研了缓解技术。


<details>
  <summary>Details</summary>
Motivation: 现有研究对指向误差的处理采用技术特定的定义、模型和缓解方法，阻碍了跨领域比较和可转移的设计见解。需要建立一个统一的框架来支持下一代定向通信系统的一致分析和稳健设计。

Method: 建立一致的术语和跨技术指向误差分类法，回顾角度失准和统计分布模型，分析其对系统性能的影响，系统调研缓解技术，特别关注光学系统及其与底层指向误差模型的联系。

Result: 提供了指向误差在轨道角动量链接和量子光通信中的详细分析，调研了针对模式相关损伤和量子测量约束的相应缓解方法，并概述了开放挑战和未来研究方向。

Conclusion: 通过将碎片化的文献整合到一个连贯的框架中，这项工作支持下一代定向通信系统的一致分析和稳健设计，为跨技术比较和设计提供了统一基础。
...
### [10] [Semantic Radio Access Networks: Architecture, State-of-the-Art, and Future Directions](https://arxiv.org/abs/2512.20917)
...
TL;DR: 本文系统综述了语义通信（SemCom）与无线接入网（RAN）的融合，提出了语义RAN（SemRAN）架构，分析了各层关键技术，展望了未来应用，并指出了研究挑战和发展方向。


<details>
  <summary>Details</summary>
Motivation: 传统RAN存在频谱效率低、抗干扰能力弱等问题，而语义通信通过意义级传输提供了改进方案。目前缺乏关于SemCom与RAN融合的全面综述，因此需要系统梳理这一新兴领域。

Method: 首先介绍RAN和SemCom基础，分析传统RAN局限，提出SemRAN整体架构。然后从物理层、数据链路层、网络层和安全平面四个维度回顾代表性技术。最后展望未来应用和标准化进展。

Result: 建立了SemRAN的系统性框架，识别了各层关键技术，包括物理层语义编码、链路层语义调度、网络层语义路由等。展示了SemRAN在提升频谱效率、抗干扰能力、资源分配灵活性和用户体验方面的潜力。

Conclusion: SemRAN代表了移动通信的新范式，但仍面临语义建模、跨层优化、安全隐私等挑战。未来需要进一步研究语义感知网络架构、标准化制定和实际部署方案，推动这一领域发展。
...
### [11] [BenchLink: An SoC-Based Benchmark for Resilient Communication Links in GPS-Denied Environments](https://arxiv.org/abs/2512.20942)
...
TL;DR: BenchLink：一种基于SoC的基准平台，用于在GPS拒止环境中实现弹性通信链路，支持自适应导频密度和调制，提供精确的延迟控制。


<details>
  <summary>Details</summary>
Motivation: 许多新兴无线通信应用需要在GPS信号不可靠或被干扰的环境中运行，这会导致振荡器漂移和载波频率损伤。传统基于通用处理器的软件定义无线电无法提供精确的延迟控制。

Method: 设计并实现了基于SoC的BenchLink系统，采用Zynq UltraScale+ MPSoCs平台，相比传统GPP方案提供更精确的延迟控制。系统支持自适应导频密度和调制，无需GPS即可工作。

Result: 在陆地和空中环境中验证了BenchLink的有效性，收集了各种条件下的综合数据集。系统设计将开源给无线研究社区。

Conclusion: BenchLink有望促进GPS拒止场景下的数据驱动链路自适应、弹性同步研究，以及需要精确延迟控制的新兴应用（如雷达传感与通信一体化）。
...
### [12] [Reconfigurable Intelligent Surface-Enhanced Satellite Networks: Deployment Strategies, Key Capabilities, Practical Solutions, and Future Directions](https://arxiv.org/abs/2512.20982)
*Ziyuan Zheng,Xiangyu Li,Shirui Zuo,Yu Wan,Bodong Shang,Wenpeng Jing,Qingqing Wu*

Main category: eess.SP

TL;DR: RIS技术可解决卫星网络中的遮挡中断和频谱共存问题，通过虚拟视距链路和角度选择性机制提升系统性能


<details>
  <summary>Details</summary>
Motivation: 卫星网络面临两大挑战：遮挡导致的服务中断，以及卫星层间和星地系统间日益严格的频谱共存需求。需要低功耗、可编程的解决方案来提升网络性能。

Method: 提出部署优先、运营感知的RIS使能卫星网络架构，将RIS同时用作卫星/终端天线以及星间或星地中继。开发了两种统一机制：通过虚拟视距链路恢复连接，以及通过角度选择性重塑干扰以扩大频谱复用。

Result: 系统级增益由两个统一机制驱动：1) 遮挡和移动性下通过虚拟视距链路保持连接的连接恢复；2) 重塑干扰以扩大频谱复用的角度选择性。讨论了高移动性下的实际运营考虑。

Conclusion: RIS技术为卫星网络提供了解决遮挡中断和频谱共存问题的有效方案。未来展望包括生成式AI范式、多功能RIS架构、无处不在的卫星集成感知通信以及可持续卫星物联网等方向。
...
### [13] [Low-Altitude ISAC with Rotatable Active and Passive Arrays](https://arxiv.org/abs/2512.20987)
*Ziyuan Zheng,Qingqing Wu,Yanze Zhu,Honghao Wang,Ying Gao,Wen Chen,Jian Xiong*

Main category: eess.SP

TL;DR: 论文研究低空ISAC系统，采用可旋转主动/被动阵列协同工作，通过联合优化波束成形、RIS相位和阵列旋转角度来最大化下行和速率，同时满足感知要求。


<details>
  <summary>Details</summary>
Motivation: 传统固定阵列在低空ISAC场景中性能受限，需要探索可旋转阵列架构来提升通信和感知性能，特别是在三维方向性天线和干扰受限环境中。

Method: 提出基于惩罚的交替优化框架：1) 使用QT和MM的凸优化方法更新BS预编码器；2) 在单位模流形上使用黎曼共轭梯度更新RIS相位；3) 使用带Barzilai-Borwein步长的投影梯度下降更新BS/RIS旋转角度。

Result: 数值仿真表明，提出的联合可旋转BS-RIS架构相比固定或部分可旋转基线方案，在保证感知要求的同时实现了显著的和速率增益，特别是在方向性天线和干扰受限场景中。

Conclusion: 可旋转主动/被动阵列协同架构是提升低空ISAC系统性能的有效方案，通过联合优化阵列旋转角度、波束成形和RIS相位，能同时满足通信和感知需求。
...
### [14] [LEO Constellations as a Decentralized GNSS Network: Optimizing PNT Corrections in Space](https://arxiv.org/abs/2512.21018)
*Xing Liu,Xue Xian Zheng,José A. López-Salcedo,Tareq Y. Al-Naffouri,Gonzalo Seco-Granados*

Main category: eess.SP

TL;DR: 提出了一种用于大规模低地球轨道星座的去中心化GNSS网络，通过卫星间信息交换实现精确轨道和时间确定，匹配集中式精度同时减少通信负担。


<details>
  <summary>Details</summary>
Motivation: 随着低地球轨道星座的快速扩张，数千颗卫星配备了GNSS接收器，形成了前所未有的空间GNSS网络。然而，当前星载GNSS处理多为独立运行，精度不足，而集中式融合面临计算瓶颈和轨道基础设施缺乏的挑战。
...
### [15] [Co-Existence of Private 5G Network and Wireless Hospital Systems](https://arxiv.org/abs/2512.21079)
*Mohsin Khan,Matti Hämäläinen,Timo J. Mäkelä,Erkki Harjula,Jani Katisko*

Main category: eess.SP

TL;DR: 在芬兰奥卢大学医院手术室部署私有5G网络的可行性研究，验证了3.9-4.1GHz频段与现有无线系统无干扰且符合电磁安全标准


<details>
  <summary>Details</summary>
Motivation: 评估医院环境中私有5G网络的部署可行性，重点关注手术室场景下的干扰风险和电磁安全性，确保与现有LTE和Wi-Fi系统的兼容性

Method: 采用先进仪器进行测量活动，结合高分辨率和长时间频谱扫描的方法，评估5G网络与相邻LTE/Wi-Fi频段的干扰情况以及射频暴露水平

Result: 测量结果显示医院私有5G网络与相邻LTE（4G）或Wi-Fi频段之间无可测量干扰，手术室内的射频暴露水平远低于ICNIRP、WHO和IEEE的安全阈值

Conclusion: 研究为医院环境中私有5G基础设施的集成提供了坚实基础，支持医疗数字化转型而不影响电磁兼容性或患者安全，并提出了频谱管理策略和可操作指南
...
### [16] [Cruising the Spectrum: Joint Spectrum Mobility and Antenna Array Management for Mobile (cm/mm)Wave Connectivity](https://arxiv.org/abs/2512.21203)
*Ece Bingöl,Eylem Ekici,Mehmet C. Vuran*

Main category: eess.SP

TL;DR: 研究毫米波频段下结合频谱移动性和天线阵列管理的波束成形优化，通过POMDP建模和PBVI算法解决移动场景中的频段和波束选择问题


<details>
  <summary>Details</summary>
Motivation: 毫米波FR2（24-71 GHz）和新兴FR3（7-24 GHz）频段虽然带宽大，但衰减严重需要高定向波束。现有天线和波束管理方案在车辆等高移动性场景中面临挑战，特别是在频段数量增加的情况下。

Method: 将频谱移动性问题建模为部分可观测马尔可夫决策过程（POMDP），使用基于点的值迭代（PBVI）算法寻找具有性能保证的策略。该问题旨在每个时隙选择最优信道频率和波束方向以最大化数据速率。

Result: 数值实验证实了所得策略对多个可用频段的有效性，即使在用户移动性显著偏离策略生成时假设模型的情况下仍能保持良好性能。

Conclusion: 通过结合频谱移动性和多频段天线阵列管理，可以有效改善移动场景下的波束成形性能，POMDP框架和PBVI算法为解决此类问题提供了有效方法。
...
### [17] [Neural Network-Assisted RIS Weight Optimization for Spatial Nulling in Distorted Reflector Antenna Systems](https://arxiv.org/abs/2512.21253)
*Xinrui Li,R. Michael Buehrer*

Main category: eess.SP

TL;DR: 提出ResNet-SA框架，通过残差学习网络结合模拟退火算法优化RIS权重，解决反射面天线失真导致的模型失配问题，提升干扰抑制性能。


<details>
  <summary>Details</summary>
Motivation: 实际反射面天线存在失真，导致理论天线模型与实际辐射模式不匹配，直接应用理论模型优化的权重会降低干扰消除性能。需要解决这种模型失配问题。

Method: 提出ResNet-SA框架：1) 使用神经网络学习理论天线增益与实际天线增益之间的残差差异；2) 将神经网络嵌入启发式优化算法（模拟退火）中，寻找最优权重向量。该方法不需要显式了解失真的电场信息。

Result: 仿真结果表明，相比仅基于理论模型优化权重的传统方法，所提方法在真实辐射模式中实现了更深的零陷深度，验证了ResNet-SA算法在反射面天线系统具有近似模式知识时的有效性。

Conclusion: ResNet-SA框架能够有效解决反射面天线系统中的模型失配问题，通过残差学习网络辅助的优化方法，在不需精确失真电场知识的情况下，显著提升干扰抑制性能。
