...
TL;DR: 本文研究非自适应信道感知中的信道码与子空间码问题，发现该问题可映射为非相干译码问题，揭示了最大似然角度估计器的性能由最小子空间距离和波束增益两个关键因素决定，推导了BPSK映射下二进制线性信道码的子空间距离精确表达式，指出仅靠良好汉明距离不足以保证感知性能（如Reed-Muller码若未经码本修剪会导致零子空间距离），并提出了基于Golomb标尺稀疏天线选择的波束空间子空间码，可实现近最优子空间距离，结合卷积波束空间获得硬件和样本高效的感知性能。


<details>
  <summary>Details</summary>
Motivation: 现有非自适应信道感知问题缺乏对信道码与子空间码关系的深入理解，不清楚哪些码本设计对感知有效，特别是未能解释为何仅具良好汉明距离的信道码在感知中表现不佳。

Method: 将问题建模为非相干译码问题，推导ML角度估计器的性能界；精确推导BPSK映射下二进制线性码的子空间距离表达式；提出基于Golomb标尺的波束空间子空间码设计，并利用卷积波束空间技术。

Result: 揭示了子空间距离与汉明距离的数学关系；指出Reed-Muller等经典码在未裁剪时子空间距离为零；所提Golomb标尺波束空间子空间码达到近最优子空间距离。

Conclusion: 精心设计的稀疏阵列（Golomb标尺）可同时提供波束赋形增益和子空间距离优势，结合卷积波束空间，可实现具有理论保证的大规模多天线通信中的硬件与样本高效信道感知。
...
TL;DR: 本文提出了一种混合高斯-马尔可夫与长短期记忆（GM-LSTM）移动性模型，用于室内光学无线通信（OWC）环境，该模型联合预测用户位置和设备方向，相比传统模型能更准确地预测移动性并提供更稳定的通信性能。


<details>
  <summary>Details</summary>
Motivation: OWC系统对用户移动性高度敏感，链路性能强烈依赖于发射器与接收器之间的空间对准，因此需要精确的用户位置和设备方向建模来实现可靠的信道估计和系统评估。

Method: 提出混合GM-LSTM模型：高斯-马尔可夫分量捕捉用户运动的时序相关性，LSTM学习残差行为以建模非线性运动模式和方向动态，实现用户位置和设备方向的联合预测。

Result: 所提出的GM-LSTM模型在预测准确性和每用户数据速率演化方面优于传统的Random Waypoint和Gauss-Markov模型，在动态室内环境中提供更准确的移动性预测和更稳定的通信性能。

Conclusion: 混合GM-LSTM模型能够更有效地表征OWC信道中的移动性，为未来室内OWC网络的性能评估和优化提供了更可靠的移动性建模工具。
...
TL;DR: 本文提出代数多样性（AD）原理，利用信号对称性通过群论方法从每观测中提取更多信息；通过匹配群确定自然变换，用群平均降低估计方差；并拓展到标量数据流、嵌套对称信号、盲群匹配（将DFT/DCT/KLT统一为变换流形上的点）、盲自适应处理（如盲均衡，预测CMA相位模糊至1.6°精度），以及Rényi-2结构容量κ，量化信息组织方式而非信息量。


<details>
  <summary>Details</summary>
Motivation: 经典信号处理方法依赖时间和空间多样性，但未充分利用信号内在的对称结构信息。为从每观测中提取更多信息，作者引入群论视角，利用信号统计不变性对应的变换群来改进估计效率。

Method: 1) 代数多样性（AD）：利用信号对称性（匹配群）确定自然变换，通过群平均降低估计方差；2) 秩推广（Rank Promotion）：将AD扩展到标量数据流，联合样本平均与群轨道平均；3) 本征张量层次（Eigentensor Hierarchy）：处理嵌套对称信号；4) 盲群匹配（Blind Group-Matching）：通过酉李代数上的广义特征值问题从数据中学习匹配群；5) 代价对称匹配（Cost-Symmetry Matching）：将AD扩展到盲自适应信号处理。

Result: 1) 在3GPP TDL多径信道上，盲均衡的恒模算法（CMA）的残余相位模糊被解析预测，匹配精度达1.6°；2) 将DFT、DCT和Karhunen–Loève变换统一为变换流形上的不同点；3) 形式化定义了Rényi-2结构容量κ，作为Shannon和von Neumann熵的Rényi-2模拟，量化信息组织方式。

Conclusion: 代数多样性（AD）提供了一种通用框架，通过群对称性补充经典时空多样性方法，在盲和自适应信号处理中具有广泛应用潜力，统一并拓展了不变估计、极小极大稳健估计、代数信号处理和压缩感知等先验工作。
...
TL;DR: 本文提出DU-PSISTA算法，结合深度学习展开与周期性随机投影降维技术，在保证稀疏信号恢复精度的同时显著降低计算复杂度，并提供了线性收敛性分析。


<details>
  <summary>Details</summary>
Motivation: 传统的ISTA及其深度展开版本虽然恢复精度高，但计算复杂度随信号维度增长而急剧增加。现有方法在计算效率与恢复精度之间难以取得良好平衡，需要一种可灵活调节计算-精度权衡的高效稀疏信号恢复算法。

Method: 提出DU-PSISTA算法，核心思想为：(1)引入随机矩阵投影（sketching）降低梯度计算维度；(2)周期性交替标准ISTA与sketched ISTA步骤；(3)通过深度展开技术（deep unfolding）数据驱动地优化步长、阈值等参数；(4)周期参数可预先配置，灵活控制精度与计算量的权衡。

Result: 理论证明在参数选择合适时，算法能以线性收敛速率收敛到真实稀疏信号的一个邻域内。数值实验表明，在合理选择周期参数和sketch大小时，DU-PSISTA在恢复精度上与传统的深度展开ISTA相当，但计算复杂度显著降低，实验结果与理论分析一致。

Conclusion: DU-PSISTA通过周期性混合结构与深度展开机制，有效实现了稀疏信号恢复中计算效率与恢复精度之间的灵活权衡，为高维稀疏信号处理提供了一种实用且可理论保证的高效方案。
...
TL;DR: HYMN数据集是一个包含UWB、BLE、WiFi、5G和GNSS五种定位系统同步测量数据的室内外混合场景数据集，专为无缝定位和多传感器融合研究设计。


<details>
  <summary>Details</summary>
Motivation: 现有数据集多聚焦单一技术或纯室内/室外场景，缺乏同时涵盖多种定位技术且包含室内外过渡区域的综合数据集，限制了多传感器融合和无缝定位研究的发展。

Method: 在工业大厅场景中，同步采集UWB、BLE、WiFi、5G和GNSS五种系统的测量数据，每个数据实例通过唯一测量ID标识，包含时间戳观测值和地面真值信息，覆盖室内外过渡区域。

Result: 构建了HYMN数据集，提供了五种异构定位系统的同步测量数据，支持多传感器指纹识别、跨技术融合和无缝室内外定位等多种定位任务。

Conclusion: HYMN数据集填补了多技术、室内外混合场景定位数据集的空白，使研究者能够探索异构信号如何互补，克服单一技术（如GNSS在遮挡区域退化、地面系统在动态环境下不稳定性）的局限性。
...
TL;DR: RespirFi利用商用WiFi设备的信道状态信息，通过理论人体反射模型和自适应子载波选择，实现了鲁棒的高保真呼吸波形重构，在临床相关指标上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有WiFi呼吸监测系统存在硬件扩展性差、精度有限、对用户位置高度敏感等问题，难以达到部署级性能和临床精度。

Method: 1) 建立理论人体反射模型，刻画CSI变化与子载波频率和空间位置的关系；2) 提出位置鲁棒的波形构建方法，自适应选择高质量子载波并对齐波形趋势；3) 提出基于子载波间CSI差异的呼吸相位识别方法，区分吸气和呼气。

Result: 在商用WiFi设备上实现，实验表明在多项临床相关呼吸指标上均优于现有最优方法。

Conclusion: RespirFi克服了硬件扩展性和位置敏感性问题，将WiFi感知推向临床意义的呼吸监测精度。
...
TL;DR: 本文提出多偏移量外环链路自适应（MO-OLLA）框架，解决高秩多点传输下多连接XR设备的链路自适应问题，并引入WiFi时延模型分析实际系留链路约束。仿真表明MO-OLLA相比传统OLLA提升20%性能，系留组在高秩传输下实现165-200%的容量增益。


<details>
  <summary>Details</summary>
Motivation: 现有研究局限于秩1传输与理想系留假设，未考虑高秩PTM传输及实际系留链路时延对XR设备性能的影响。

Method: 提出MO-OLLA框架，引入依赖秩的SINR校正以提高链路自适应精度；结合WiFi时延模型表征系留链路带宽与吞吐量约束。

Result: MO-OLLA提供最高20%性能提升；理想系留下TGr实现180-200%容量增益；实际高吞吐量系留下增益仍达165-180%。

Conclusion: 基于系留组的多连接XR设备在考虑实际系留链路约束下仍能显著提升容量，具备实际部署可行性。
...
TL;DR: 提出AMUD框架，在卫星-空中-地面一体化网络中动态部署多无人机中继与LEO卫星协同，提升覆盖、缓解拥塞，并优化能效、容量及公平性。


<details>
  <summary>Details</summary>
Motivation: 6G网络在密集城区和灾区的传统地面网络面临信号遮挡、拥塞和动态用户分布等挑战，现有方案难以满足需求。

Method: 提出AMUD框架，联合优化多架放大转发无人机中继部署与LEO卫星协同，最大化网络能效与总容量，同时保证容量公平性和用户需求。

Result: 相比传统LEO-地面基站系统，AMUD提升了总容量、能效和容量公平性。

Conclusion: AMUD框架在SAGINs中有效克服了非视距和高需求场景下的通信瓶颈，为6G网络提供了一种可行的增强方案。
...
TL;DR: 提出了一种基于扩散模型的近场宽带XL-MIMO信道估计方法，利用多尺度注意力机制的去噪网络来学习信道的先验分布，从而在贝叶斯后验推断框架下实现高鲁棒性的信道估计。


<details>
  <summary>Details</summary>
Motivation: XL-MIMO系统中阵列孔径和信号带宽的增加给宽带信道估计带来了新的挑战，特别是近场宽带信道同时存在空间非平稳性和波束分裂效应，传统方法难以有效处理。

Method: 1) 分析近场宽带信道的统计相关性，揭示空间非平稳性和波束分裂效应；2) 将信道估计建模为贝叶斯后验推断问题，使用扩散模型学习信道先验分布；3) 设计多尺度注意力机制去噪网络，通过并行卷积分支提取多尺度空频特征，并融合特征注意力和空间注意力模块。

Result: 实验结果表明，该方法在不同实验设置下相比现有基线方案具有更优越的鲁棒性。

Conclusion: 扩散模型结合多尺度注意力机制能够有效建模近场宽带信道的复杂空频结构，提升XL-MIMO系统信道估计性能。
...
TL;DR: 本文提出一种基于CSI除法的Wi-Fi物理层隐写系统，通过引入CSI除法机制和编码器-解码器神经网络框架，解决了现有方法对环境变化敏感和隐写容量有限的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于CSI的Wi-Fi隐写方法存在两个关键限制：易受环境变化影响（鲁棒性差）和隐写容量有限，亟需一种能同时解决这两个问题的方法。

Method: 1) 提出CSI除法机制，将人工CSI分量与自然无线信道响应解耦，秘密信息嵌入在连续两次CSI测量的商中；2) 设计编码器-解码器神经网络框架，自动学习最优FIR滤波器生成策略和秘密恢复策略。

Result: 在商用现成硬件（SDR发射机、ANTSDR和ESP32接收机）上实现原型系统，实验表明在动态环境条件下具有鲁棒性，同时显著提升了隐写容量。

Conclusion: 该工作通过创新的CSI除法机制和神经网络框架，有效克服了现有Wi-Fi物理层隐写的两大瓶颈，为实际部署提供了可行方案。
...
TL;DR: 本文提出一种针对RIS辅助网络的多目标优化框架，统一分析通信、感知和安全性能之间的权衡，为6G安全ISAC系统提供RIS部署与配置策略指导。


<details>
  <summary>Details</summary>
Motivation: RIS、ISAC和PLS是下一代无线网络的关键技术，但联合优化三者存在复杂且相互冲突的设计权衡，现有研究缺乏统一的系统视角。

Method: 构建多目标优化框架，联合建模RIS部署位置、方向、尺寸以及ISAC配置权重（控制RIS反射增益在通信与感知任务间的分配），通过仿真分析三者间的权衡关系。

Result: 揭示了通信可靠性、感知精度和安全性能之间固有的权衡关系，表明RIS配置参数对三者性能有显著影响。

Conclusion: 该框架为6G安全ISAC网络中RIS的高效部署与配置提供了有价值的洞察，有助于设计兼顾通信、感知与安全性能的策略。
...
TL;DR: 本文研究了弯曲波束在近场无线通信中的物理层安全性能，证明其在对抗视距和非视距窃听方面优于传统波束成形技术。


<details>
  <summary>Details</summary>
Motivation: 探索弯曲波束在动态遮挡规避和防窃听方面的潜力，提升近场无线通信的物理层安全性能。

Method: 通过分析窃听者可能位置与弯曲波束设计参数之间的依赖关系，引入新的物理层安全性能评估指标。

Result: 弯曲波束在视距和非视距窃听场景下均表现出优于传统波束成形的物理层安全性能。

Conclusion: 弯曲波束是近场无线通信中一种有效的物理层安全增强方案，尤其适用于动态遮挡避免和防窃听场景。
...
TL;DR: 本文提出了一种图信号推广的样本熵（SampEn$_{G}$），通过多跳图嵌入替代传统时间延迟嵌入，量化连续状态空间上图信号的不规则性，并在有向路径图、逻辑斯蒂映射和Erdős--Rényi图信号上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有样本熵方法主要针对时间序列，缺乏对图结构数据（尤其是连续状态空间上的图信号）不规则性度量的通用框架，符号动力学方法存在局限。

Method: 将经典SampEn的时间延迟嵌入替换为基于图移位算子幂次计算的局部游走加权邻域平均的多跳图嵌入，聚合每个节点的模式信息。

Result: 在加权有向路径图上SampEn$_{G}$退化为经典一维SampEn；在逻辑斯蒂映射上验证了非线性灵敏度；在有向Erdős--Rényi图上展示了连通性和模式长度$m$的影响，且具备可处理数千节点规模的实用运行时。

Conclusion: SampEn$_{G}$成功将样本熵和条件熵的概念推广到图信号分析中，为网络数据的非线性分析开辟了新途径。
...
TL;DR: 本文提出了一种面向可重构天线辅助的集成感知与通信（ISAC）系统的解耦式三重混合波束赋形（Tri-HBF）框架，通过交替优化数字、模拟和电磁波束赋形器，在单用户单目标和多用户多目标场景下实现通信速率和感知SCNR的最大化，仿真表明该方法相比传统方案在频谱效率和天线开销上均有显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有ISAC系统的混合波束赋形（HBF）虽能降低硬件开销，但尚未充分利用可重构天线（RA）技术带来的额外波束赋形自由度，将RA集成到ISAC中的研究仍属空白。

Method: 提出解耦式三重混合波束赋形（Tri-HBF）框架，交替优化数字、模拟和电磁波束赋形器；将分数形式目标函数转化为无分数形式；采用交替优化求解变量块；对于MUMT场景中的EM波束赋形子问题，提出低复杂度迭代方法替代SDR。

Result: 相比传统全向和定向天线方案，频谱效率提升近100%，天线开销降低62.5%。

Conclusion: RA-assisted ISAC系统结合所提Tri-HBF框架能够显著提升系统性能，为未来ISAC系统设计提供了新的研究方向。
...
TL;DR: 本文提出了一种多用户语义通信系统，每个用户配备不同的深度学习联合源-信道解码器架构，并针对灾难性遗忘问题提出了一种与编码器对称的锚定解码器方法，通过冻结编码器参数训练不同用户的解码器，最后通过仿真实验验证了所提框架的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有语义通信研究大多只考虑单发射机与单接收机的场景，无法处理多用户场景下不同模型和能力差异的问题。

Method: 提出多用户语义通信系统，每个用户使用不同的DL联合源-信道解码器架构；针对灾难性遗忘问题，设计一个与编码器对称的锚定解码器，提供与编码器提取能力对齐的反馈；优化完成后冻结编码器参数，再训练不同用户的解码器以对齐编码器输出。

Result: 通过一系列仿真实验，验证了所提框架相较于其他基准方法的优越性能。

Conclusion: 所提出的多用户语义通信系统能够有效应对用户多样性和灾难性遗忘问题，提升多用户场景下的通信性能。
...
TL;DR: 本文比较了三种有限块长下退化窃听BSC信道的 secrecy-coding 方案：极化码、PAC码和可逆提取器框架，发现PAC码在不改变保密性的情况下显著提升了可靠性，且极化/PAC码比可逆提取器框架提供更紧的安全性保证。


<details>
  <summary>Details</summary>
Motivation: 在有限块长机制下，对退化窃听BSC信道比较三种 secrecy-coding 方案的安全性与可靠性表现，特别是PAC码与极化码的关系以及它们与可逆提取器框架的性能对比。

Method: (i) 使用Tal-Vardy构造计算Eve的极化子信道容量，得到显式有限长互信息泄漏上界；(ii) 证明PAC码的窃听者合成比特信道与极化码等价（仅置换），可复用相同泄漏界；(iii) 使用可逆提取器框架的闭式语义安全界。将强保密界转换为对称窃听信道的语义安全保证。

Result: PAC码在不改变极化码继承的保密性上界的情况下显著改善了可靠性（帧错误率）。在有限长界下，极化/PAC保密码比可逆提取器框架提供更紧的安全保证。

Conclusion: PAC码结合了极化码的强安全性与更好的纠错性能，且两者在安全性上优于可逆提取器框架。
...
TL;DR: 论文提出受限近似可逆性条件（RAIC）作为统一框架，保证从非线性观测中均匀恢复结构化信号。通过投影梯度下降（PGD）算法，在多种设定下（如稀疏恢复、1-bit量化）实现了与非均匀恢复相当的错误率（仅差对数因子）。


<details>
  <summary>Details</summary>
Motivation: 尽管RIP条件能保证线性测量下的均匀稀疏恢复，但非线性观测下结构化信号的均匀恢复理论仍不成熟。现有结果多为非均匀恢复（如Plan & Vershynin 2016），需要建立统一的均匀恢复理论框架。

Method: 引入RAIC条件作为梯度算子的分析工具；通过覆盖论证建立高斯单指标模型的均匀RAIC；对三种典型设定（ℓ0投影IHT、ℓ1投影稀疏恢复、ℓ1投影近似稀疏恢复）分析额外误差项；对1-bit量化使用VC维论证消除对数因子。

Result: 在所有三种典型设定下，均匀恢复的误差率与非均匀恢复误差率同阶（最多差对数因子）；1-bit量化下IHT的均匀恢复误差与非均匀恢复误差完全相同阶（无对数损失）；PGD对噪声和污染的鲁棒性可通过单个随机过程边界优雅刻画。

Conclusion: RAIC提供了非线性观测下结构化信号均匀恢复的统一理论框架，在多项设定下达到或接近非均匀恢复的最优率，优于Genzel & Stollenwerk (2023)的结果。
...
TL;DR: 本文研究了在保证感知质量的前提下，通过无噪信道和广播信道进行安全压缩传输时的率-失真-感知（RDP）权衡问题，提出了随机装箱编码方案，证明了其在强保密、低失真和高感知质量方面的有效性。


<details>
  <summary>Details</summary>
Motivation: 在神经图像压缩等应用中，重建数据需要保持感知质量，同时压缩数据在公共信道传输时面临安全风险，因此需要研究安全RDP权衡。

Method: 通过信息论分析，对无噪信道刻画了精确的安全RDP区域；对广播信道推导了内界并证明了一类更优广播信道的紧致性；展示了分离信源信道编码的最优性；引入共随机性分析其作用。

Result: 建立了无噪信道下的精确安全RDP区域；推导了广播信道的内界并证明了对一类更优广播信道的紧致性；证明了分离信源信道编码在无限共随机性下的最优性；通过二元和高斯示例展示了共随机性可显著降低通信速率。

Conclusion: 随机装箱编码能够同时实现强保密、低失真和高感知质量，共随机性在安全RDP设置中比标准率失真设置中更为重要。
...
TL;DR: 本文研究了可移动天线（MA）辅助的多用户上行通信系统中，基站通过优化天线移动轨迹来最大化所有用户的最小可达速率。针对理想无限快移动和有限移动速度两种情况，分别提出了基于拉格朗日对偶的最优解和启发式轨迹设计方案。


<details>
  <summary>Details</summary>
Motivation: 传统固定天线阵列无法保证用户间的速率公平性，而可移动天线技术提供了额外的空间自由度。本文旨在通过设计天线轨迹来最大化多用户系统中最小用户的可达速率，实现公平性优化。

Method: 1) 针对无限快MA移动的理想情况，利用拉格朗日对偶方法求解连续时间优化问题，发现最优策略是使用有限个部署模式并分配相应时间；2) 针对有限移动速度的一般情况，基于理想情况的优化模式设计启发式天线轨迹。

Result: 通过数值验证，所提出的设计在最大化最小用户可达速率方面优于竞争基准方案。

Conclusion: 基站应使用有限个MA部署模式并按最优时间分配来移动天线；理想情况下的优化模式可为有限速度场景提供启发式设计指导。
...
TL;DR: 本文研究了有限时域下一般随机控制系统（包括非线性系统）中速率受限控制的速率-代价权衡，通过非渐近界刻画了最小可达通信速率与有向信息最小化之间的关系，建立了有向信息作为速率受限控制操作相关量的普适性。


<details>
  <summary>Details</summary>
Motivation: 在速率受限的控制系统中，通信速率与控制代价之间存在根本性权衡。现有工作主要局限于线性二次高斯（LQG）系统或渐近分析，缺乏对一般非线性随机控制系统在非渐近意义下的理论刻画。本文旨在建立有向信息作为一般控制系统速率-代价权衡的核心量化指标。

Method: 利用强函数表示引理（strong functional representation lemma）在每个时间步构造编码与控制策略，从而证明上界。将问题建模为有向信息最小化问题 $F_n(D)$，并证明最小可达速率 $R_n(D)$ 与 $F_n(D)$ 之间仅存在对数加法间隙。

Result: 给出了非渐近界：$F_n(D) \le R_n(D) \le F_n(D)+\log(F_n(D)+3.4)+2+\frac{1}{n}$（单位为比特）。该界适用于一般非线性随机控制系统；作为特例，可导出序贯（因果）率失真和LQG控制的非渐近界。

Conclusion: 有向信息不仅是因果信源编码和LQG控制的操作相关量，更是一般非线性随机控制系统速率受限控制的普适量。本文提供的非渐近界为实际系统设计提供了理论基础。
...
TL;DR: 针对FDD大规模MIMO系统中下行信道状态信息反馈问题，提出一种将长期基反馈与短期系数量化分离的实用架构，通过随机向量量化推导出闭环均方误差表达式，优化速率分配并识别基更新的相变阈值，在相关高斯和COST2100信道上实现了接近最优的性能和复杂度显著降低。


<details>
  <summary>Details</summary>
Motivation: 在FDD大规模MIMO系统中，下行信道状态信息必须在有限的上行链路预算内反馈。变换编码（如KL变换和反注水法）虽在理论上对高斯信道是率失真最优的，但存在用户与基站之间的基失配问题，限制了实际性能。

Method: 提出一种将长期基反馈与短期系数量化分离的实用架构；采用随机向量量化（RVQ）；推导出端到端均方误差的闭式表达式；据此刻画最优速率分配，并识别出基更新的相变阈值。

Result: 在相关高斯信道和COST2100信道上进行仿真，该方法实现了接近最优的性能，对更新开销具有鲁棒性，并且相比基于深度学习的自编码器方法显著降低了复杂度。

Conclusion: 所提出的分离式反馈架构有效解决了基失配问题，通过理论分析指导最优速率分配和基更新策略，在实用性和性能之间取得了良好平衡。
...
TL;DR: 本文研究可移动天线（MA）技术中天线移动时长与数据传输时长之间的折中问题，提出优化移动时长和天线部署以最大化有效吞吐量的方案，并推导了最大移动速度的闭式条件。


<details>
  <summary>Details</summary>
Motivation: 可移动天线虽能通过灵活部署提升通信性能，但天线移动需要消耗时间，这段时间内通信可能因信道波动和多普勒效应受损，导致移动时长与数据传输时长之间存在根本性折中。

Method: 建立多用户下行场景的联合优化问题（高度非凸），通过一维搜索结合天线部署优化求解；提出基于少量速率-时长采样点的拟合方法得到闭式表达式；推导了最大天线移动速度的闭式条件。在2天线2用户特例中验证折中关系。

Result: 提出拟合方法降低了计算复杂度；推导出速度阈值条件：当速度低于该阈值时，最优策略是保持天线静止；仿真验证了所提方案的有效性。

Conclusion: 天线移动时长与数据传输时长存在本质折中，所提方法能有效优化有效吞吐量，且给出了静止策略的适用条件。
...
TL;DR: 本文介绍了一种名为可重构分布式天线与反射表面（RDARS）的新型混合架构，该架构结合了分布式有源天线和可重构无源反射表面的优势，能够在成本效益和能效方面提升未来无线网络的覆盖范围、频谱效率，并支持通信与感知一体化，相比传统可重构智能表面（RIS）具有更优的信噪比增益。


<details>
  <summary>Details</summary>
Motivation: 为了满足未来无线网络对覆盖增强、频谱效率提升以及通信与感知一体化等严格要求，需要一种兼具有源传输和无源波束控制优势、且成本效益高、能效优的新型架构。

Method: RDARS采用混合架构，将有源分布式天线单元与可重构无源反射表面集成在一起，实现有源传输与无源波束控制的协同工作。

Result: 在通信和感知的代表性应用场景中，RDARS相比传统RIS展现出明显的信噪比（SNR）增益优势。

Conclusion: 凭借独特的混合模式协同优势，RDARS有望在下一代智能通信系统的演进中发挥关键作用，但仍需解决实际实现和资源分配等方面的挑战。
...
TL;DR: 提出一种增强型Chase-Pyndiah译码器，通过基于分量译码器置信度缩放外部消息，在复杂度几乎不变的情况下获得0.1 dB增益。


<details>
  <summary>Details</summary>
Motivation: 在保持低复杂度的同时提升Chase-Pyndiah译码器的性能，通过更合理地利用分量译码器的置信度信息来改进外部消息的缩放策略。

Method: 在传统Chase-Pyndiah译码器基础上，引入基于分量译码器置信度的外部消息缩放机制，根据译码器对当前译码结果的可靠程度动态调整外部信息的缩放因子。

Result: 与原始Chase-Pyndiah译码器相比，所提方法获得了约0.1 dB的性能增益，且复杂度增加可忽略不计。

Conclusion: 基于置信度的外部消息缩放是一种有效且低成本的译码增强手段，可在几乎不增加计算负担的前提下改善译码性能。
...
TL;DR: 本文研究了高斯无源多址接入（UMA）信道中，在异构路径损耗条件下，基于位置感知的码本分区策略的最小每比特能量有限块长界。通过数值仿真和复制方法的大系统分析，量化了该策略相比传统统一码本方法的性能增益。


<details>
  <summary>Details</summary>
Motivation: 在无源多址接入（UMA）系统中，传统方法假设所有用户使用统一码本，忽略了用户间路径损耗的差异性。本文旨在利用用户已知的路径损耗信息（位置信息）来优化码本设计，从而降低每比特能量需求。

Method: 推导了有限块长下最小每比特能量的下界；利用位置信息进行码本分区（location-based codebook partitioning）；通过数值仿真和复制方法（replica method）进行大系统分析，比较位置感知策略与传统统一码本策略的性能。

Result: 基于位置感知的码本分区策略在有限块长和异构路径损耗场景下，能够显著降低每比特能量消耗，性能优于传统统一码本方法。

Conclusion: 利用位置信息进行码本分区是一种有效的UMA系统优化策略，可在异构路径损耗条件下提升能量效率，具有实际部署潜力。
...
TL;DR: 本文针对卫星上行链路链路预算有限的问题，提出了一种基于稀疏叠加编码（SSC）的无导频编码调制方案，利用Zadoff-Chu准正交字典实现高效宽带传输，并通过指示序列嵌入根索引信息降低解码复杂度，同时开发了带有重复和停止反馈的多码字传输框架，仿真表明相比传统窄带多维星座方法可获得吞吐量增益。


<details>
  <summary>Details</summary>
Motivation: 卫星直接上行链路受限于链路预算，限制了宽带资源的利用和吞吐量提升，亟需高效的宽带传输方案。

Method: 提出无导频的稀疏叠加编码（SSC）调制方案，利用Zadoff-Chu准正交（ZC-QO）字典支持可扩展传输；通过指示序列嵌入根索引信息限制解码搜索空间；开发多码字传输框架，结合重复和停止反馈机制。

Result: 仿真结果表明，所提方案相比传统的窄带多维星座方法实现了吞吐量增益。

Conclusion: 基于SSC的无导频编码调制方案能在链路预算受限的卫星上行场景中有效利用宽带资源，提升系统吞吐量。
...
TL;DR: 本文提出一种保留测序仪概率信息的DNA存储编码方案，通过集成HMM对齐、log-product融合和有序统计解码，在DT4DDS通道模拟器上实现了比现有技术高11%-52%的存储密度，并在25°C干燥状态下预测可解码存储长达282年。


<details>
  <summary>Details</summary>
Motivation: 现有DNA存储编解码方案在纠错前将测序输出的概率信息丢弃为离散碱基调用，丢失了关于哪些位置可靠的宝贵信息，导致无法充分发挥DNA存储的理论容量。

Method: 提出一种保留测序仪逐位置后验分布的编码方案，集成三个核心组件：profile隐马尔可夫模型对齐、跨读段的log-product融合、以及有序统计解码。

Result: 在DT4DDS通道模拟器上，高保真条件下恢复155.8 EB/g dsDNA（比先前最高密度高11%），低保真条件下恢复25.9 EB/g（高52%）；在25°C干燥状态下模拟预计可解码存储282年，密度17.1 EB/g。

Conclusion: DNA存储密度已接近底层通道的香农极限，该工作显著缩小了实际性能与理论容量之间的差距。
