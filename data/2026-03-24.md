...
TL;DR: 使用ERP模板初始化卷积核的Deep-MF模型在单试次ERP检测中表现优于标准模型，提高了跨被试的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 由于EEG信号信噪比低，单试次ERP检测具有挑战性。研究探索将ERP先验知识融入深度学习模型是否能提升检测性能。

Method: 采用Deep-Match框架，两阶段训练：先训练编码器-解码器重构EEG信号学习表示，再替换解码器为检测模块进行微调。比较标准随机初始化模型和用ERP模板初始化卷积核的Deep-MF模型。

Result: Deep-MF模型在留一被试验证中平均F1分数(0.37)高于标准模型(0.34)，最佳F1分数达0.71(标准模型0.59)，表明ERP信息初始化能提升跨被试检测性能。

Conclusion: 将领域知识（ERP模板）与深度学习结合能改善单试次ERP检测，为可穿戴EEG和被动脑机接口系统的实时认知监测提供了可行方向。
...
Method: 采用分步射线追踪仿真框架：使用SketchUp根据现场测量构建布局，Wireless Insite进行电磁传播仿真，MATLAB处理输出并绘制功率延迟剖面等信道参数

Result: 对作者所在部门的整个楼层进行建模，测试不同发射机-接收机位置，验证了框架在覆盖空洞预测等用例中的可行性

Conclusion: 建立了一个完整的室内无线传播建模工作流程，为室内网络部署和性能优化提供了有效的仿真工具和方法
...
TL;DR: 提出神经早停协议(NES)用于LC-OSD解码，通过单一参数平衡错误风险和搜索成本，显著减少测试错误模式数量，仅带来轻微FER性能损失。


<details>
  <summary>Details</summary>
Motivation: LC-OSD解码器虽然能提供强大的软判决性能，但其实际计算成本主要由测试错误模式(TEPs)数量决定。需要一种方法来控制搜索成本，同时保持解码性能。

Method: 提出神经早停协议(NES)，在预定义检查点使用FER对齐监督进行训练，学习判断是否继续搜索可能改善当前最佳候选。通过比较预测的继续需求与TEPs成本来决定停止搜索。

Result: 实验结果表明，该方法能显著减少平均TEP数量，仅带来边际FER性能下降，且单个全局模型适用于所有SNR范围。

Conclusion: 神经早停协议为LC-OSD解码提供了有效的成本控制机制，在保持解码性能的同时大幅降低计算复杂度。
...
TL;DR: 提出一种用于心电图分析的偏移不变基线方法，通过特征相关性评分映射到心脏相位，识别P波和T波在体力活动识别中的关键作用


<details>
  <summary>Details</summary>
Motivation: 在生物医学应用中，理解机器学习模型的决策过程对医疗专家至关重要。心电图分析中，需要确定哪些特征对正确决策贡献最大，这有助于理解机器试图解释的心脏相位或状况。当前方法需要合适的基线和直观的评分分布。

Method: 提出一种具有物理意义的偏移不变基线方法，用于计算心电图特征的相关性评分。通过聚合显著性评分，将其映射到心脏相位。使用残差网络从心脏活动推断体力活动，并分析特征相关性。

Result: 实验表明，在准确识别体力活动时，获得最高相关性评分（即贡献最大）的心电图样本与P波和T波相关。这验证了方法能够有效识别关键心脏相位特征。

Conclusion: 提出的偏移不变基线方法具有物理意义，能够产生直观解释的相关性评分分布，成功将显著性评分映射到心脏相位，为心电图分析的模型可解释性提供了有效工具。
...
TL;DR: 论文研究利用无线空中计算实现神经网络全连接层，通过多跳中继系统优化发射预编码、接收组合和中继增益，在功率约束下提升模仿精度。


<details>
  <summary>Details</summary>
Motivation: 动机是探索无线通信中的空中计算技术来实现神经网络计算，特别是利用多跳中继系统来扩展计算范围并提高计算效率，解决传统集中式计算在分布式环境中的局限性。

Method: 方法包括：1）建立多跳多天线系统模型；2）在发射功率和中继功率约束下，构建优化发射预编码器、接收组合器和中继放大转发增益的优化问题；3）提出交替优化框架来优化模仿精度。

Result: 仿真结果表明，多跳中继系统在神经网络中能够实现几乎完美的分类准确率，验证了无线空中计算实现神经网络全连接层的可行性。

Conclusion: 结论是无线多跳空中计算可以有效实现神经网络计算，多跳中继系统在功率约束下能够达到接近完美的分类性能，为分布式神经网络计算提供了新的实现途径。
...
TL;DR: 论文提出了一种基于可旋转有源RIS增强的安全方向调制设计，通过优化RIS方向提升安全性能，解决了传统RIS辅助DM网络中乘性路径损耗限制安全增益的问题。


<details>
  <summary>Details</summary>
Motivation: 传统RIS辅助方向调制网络由于RIS反射路径引入的乘性路径损耗，安全性能增益有限。需要设计新的方法来提升安全速率，同时考虑窃听者DOA估计性能、发射功率、RIS旋转范围和最大反射幅度等约束。

Method: 提出了三种算法：多流零空间投影和泄漏方法、增强泄漏方法、以及基于分布式软演员-评论家三次改进（DSAC-T）的优化方案，用于解决非凸的安全速率最大化问题。

Result: 仿真验证了所提算法的有效性，观察到窃听者DOA估计精度与可达到的安全速率之间的性能权衡。RIS的安全增强在配备少量天线的系统中更为显著，通过优化RIS方向可实现52.6%的安全速率性能提升。

Conclusion: 可旋转有源RIS能够有效增强方向调制网络的安全性能，特别是在天线数量较少的系统中。通过优化RIS方向可以显著提升安全速率，为解决传统RIS辅助DM网络的安全限制提供了有效方案。
...
TL;DR: 提出一种基于正交匹配追踪(OMP)的方法，通过使特征多项式系数稀疏化来策略性地破坏稳定但虚假的极点，从而消除稳定性图中的虚假极点。


<details>
  <summary>Details</summary>
Motivation: 在工程结构的模态参数识别中，LSCF方法虽然精度高，但随着多项式阶数增加会产生非物理的虚假极点，这些极点需要从稳定性图中消除，但现有方法存在不足。

Method: 采用正交匹配追踪(OMP)算法，通过使特征多项式的系数尽可能稀疏，策略性地破坏稳定但虚假的极点，从而生成稀疏的稳定性图，因为不稳定极点可以从图中消除。

Result: 方法首先应用于有限元模型获得的矩形板FRFs，验证了有效性；然后应用于低阻尼和高阻尼矩形板的实验FRFs；最后应用于电动汽车电机定子芯的FRFs。结果表明，在不影响精度的情况下，虚假极点可以从稳定性图中消除。

Conclusion: 提出的基于OMP的方法能够有效消除稳定性图中的虚假极点，适用于从简单结构到复杂工业应用的各种情况，且不损害模态参数识别的精度。
...
TL;DR: 提出US-CKM驱动的两阶段协调调度框架，利用位置信息与统计CSI的映射关系降低CSI获取开销，并通过可靠性引导机制增强动态散射环境下的鲁棒性。
...
Method: 开发用户调度导向的CKM（US-CKM）和两阶段协调调度框架：第一阶段基于US-CKM进行小区内活跃用户选择，第二阶段利用跨小区信道相关性抑制小区间干扰；增加可靠性引导机制处理不完美统计CSI。

Result: 系统能够快速检索和重用统计CSI，显著降低CSI获取开销；通过可靠性机制在动态散射环境下保持鲁棒性能。

Conclusion: US-CKM驱动的两阶段协调调度框架有效解决了多小区大规模MIMO系统中的用户调度问题，在降低开销的同时保证了系统性能。
...
TL;DR: 提出一种用于射频收发器的多代理神经符号AI系统，通过为每个电路组件分配AI代理来实现智能控制


<details>
  <summary>Details</summary>
Motivation: 现代和未来通信系统需要能够适应动态操作条件的射频收发器智能控制，传统方法难以应对复杂多变的运行环境

Method: 采用多代理神经符号AI架构，为每个电路组件分配专门的AI代理，每个代理包含内部模型和相应的控制算法

Result: 在IF放大器建模中显示出有希望的结果，表明该方法可以扩展到所有组件，从而创建完全智能的射频系统

Conclusion: 多代理神经符号AI系统为射频收发器的智能控制提供了可行的解决方案，能够适应动态操作条件，具有扩展到完整射频系统的潜力
...
TL;DR: 该研究指出当前ITTS模型的偏见评估多采用单变量测试，忽略了社会线索的组合结构。通过将提示建模为社会地位、职业刻板印象和人物描述符的组合，发现这些维度之间存在系统性交互效应，产生复杂偏见模式，且这些偏见与预训练文本编码器的语义先验和训练数据分布偏差密切相关。


<details>
  <summary>Details</summary>
Motivation: 当前指令文本转语音（ITTS）模型的偏见评估主要依赖单变量测试方法，这种方法忽略了社会线索的组合结构，无法捕捉复杂的偏见交互效应。研究者希望通过组合分析方法更全面地诊断生成语音中的潜在风险。

Method: 将提示建模为社会地位、职业刻板印象和人物描述符的组合结构，分析开源ITTS模型中的性别偏见。通过组合分析方法揭示不同社会维度之间的交互效应，并探究这些偏见与预训练文本编码器的语义先验和训练数据分布的关系。

Result: 研究发现社会维度之间存在系统性交互效应，产生复杂偏见模式，这些模式被单变量基线方法所忽略。这些偏见不仅限于表面特征，而且与预训练文本编码器的语义先验和训练数据的偏斜分布有强关联。通用多样性提示方法不足以覆盖这些根深蒂固的模式。

Conclusion: 需要采用组合分析方法来诊断生成语音中的潜在偏见风险，因为单变量测试方法无法捕捉社会线索之间的复杂交互效应。通用多样性提示策略不足以解决这些深层次的偏见问题，强调了在ITTS系统开发中考虑组合结构的重要性。
...
TL;DR: 提出4D-FSM框架，在ELAA系统中联合编码角度、深度、合成速度和QAM幅度，通过合成速度实现无物理运动的用户分离，显著提升频谱效率和用户容量。


<details>
  <summary>Details</summary>
Motivation: 现有方案无法充分利用ELAA在亚太赫兹频段产生的四维信号空间（角度、深度、速度、幅度），限制了系统性能。需要一种统一框架来同时利用这些维度进行信息编码和用户分离。

Method: 提出4D-FSM框架，通过空间-时间调制引入合成速度（线性相位斜坡），创建速度正交气泡来分离共址用户。使用离散菲涅尔变换分解信道矩阵，将预编码复杂度从O(N³)降低到O(NlogN)。

Result: 在fc=140GHz、N=4096条件下，实现ρ≈0.998的相关性、6.16bps/Hz的频谱效率（基线方案完全失效时）、Kmax=64个正交用户。在K=50用户时，相比TTD方案获得248倍的和速率优势。

Conclusion: 4D-FSM框架成功利用ELAA的四维信号空间，通过合成速度实现高效用户分离，显著降低计算复杂度，为未来亚太赫兹大规模天线系统提供了可行的解决方案。
...
TL;DR: 提出PRI-IGSS方法，通过预旋转初始化和迭代贪婪空间谱搜索，解决可旋转阵列传感中计算复杂度高和时间效率低的问题


<details>
  <summary>Details</summary>
Motivation: 传统可旋转阵列传感方法（如RR-Root-MUSIC）需要多次机械旋转和重复特征分解，导致计算复杂度高、时间效率低。天线元件的定向特性导致发射器偏离阵列视轴时接收信号功率严重衰减，影响传感性能。

Method: 提出三阶段框架：1）预旋转初始化：将阵列法向量旋转到候选方向，用Root-MUSIC算法计算最大传感能量对应的DOA值；2）机械旋转：将阵列机械旋转到初始估计方向并固定；3）迭代贪婪空间谱搜索：采用强化学习思想，缩小搜索范围，累加所有先前采样方差矩阵以提供递增性能增益。

Result: 仿真结果表明，提出的PRI-IGSS方法性能远优于RR-Root-MUSIC，在均方误差方面达到CRLB（Cramér-Rao下界）。推导了简化旋转模型的相应CRLB。

Conclusion: PRI-IGSS方法通过预旋转初始化和迭代贪婪搜索，显著降低了计算复杂度，提高了时间效率，在方向传感性能上优于现有方法并达到理论下界。
...
TL;DR: 提出基于KL展开的框架，将N维相关FAS信道分解为独立特征模，通过秩K截断将中断分析简化为K维积分，提供理论保证和最优性证明。


<details>
  <summary>Details</summary>
Motivation: 现有块相关模型(BCM)对信道协方差矩阵施加结构近似，可能引入乐观性能偏差，需要更精确的分析框架来处理FAS中的空间相关瑞利信道中断分析。

Method: 采用Karhunen-Loève展开框架，将N维相关FAS信道分解为独立特征模，进行受控的秩K截断，推导出秩1和秩2情况的闭式中断表达式，并提供任意K的高斯-埃尔米特求积公式。

Result: 理论证明KL近似总是高估中断概率，提供保守保证；仅需K* = 2⌈W⌉ + 1个特征模；KL截断达到高斯率失真界；数值结果验证理论预测匹配蒙特卡洛模拟，KL框架在近似精度上优于BCM。

Conclusion: KL框架为FAS中断分析提供了原则性方法，避免了BCM的乐观偏差，证明为信息论最优信道压缩，有效自由度随孔径而非端口数缩放。
...
<details>
  <summary>Details</summary>
Motivation: 传统流体天线系统分析通常采用离散端口模型和块相关近似，这可能导致乐观偏差。本文旨在开发一个更准确、基于连续孔径的分析框架，以更好地理解流体天线系统的中断性能。

Method: 1) 推导高斯核下的Karhunen-Loève展开，得到秩1和秩2截断的闭式中断表达式，以及任意秩K的高斯-埃尔米特公式；2) 通过Slepian不等式和高斯比较定理建立严格的双侧中断边界；3) 使用Adler-Taylor期望欧拉特征方法和Piterbarg定理开发连续孔径极值理论。

Result: 高斯近似在W≤2时相对中断误差小于10%；连续孔径公式仅需约10W个端口即可收敛；中断概率表达式仅依赖于孔径W和阈值x，与端口数N无关；Jakes和高斯模型由于共享第二谱矩λ₂=2π²而具有相同的中断性能。

Conclusion: 本文提出的连续孔径分析框架为流体天线系统中断概率提供了准确、高效的计算方法，揭示了系统性能主要取决于物理孔径而非端口数量，为实际系统设计提供了重要理论指导。
...
<details>
  <summary>Details</summary>
Motivation: 多卫星大规模MIMO系统需要解决加权和速率最大化问题，但传统方法存在计算复杂度高和卫星间信令开销大的问题，需要更高效的传输架构。

Method: 1) 采用基于统计CSI的多卫星WMMSE公式化，获得闭式更新的预编码和接收向量；2) 提出基于学习的WMMSE设计，结合张量等变性和闭式恢复，无需迭代更新；3) 开发去中心化多卫星传输方案，各卫星利用轨道位置和姿态等状态信息，通过双分支张量等变网络本地推断预编码器。

Result: 数值结果表明：1) 多卫星传输显著优于单卫星系统的和速率；2) 去中心化方案接近集中式方案的性能，同时大幅降低计算复杂度和卫星间开销；3) 基于学习的方案在不同场景下表现出强大的鲁棒性和可扩展性。

Conclusion: 提出的多卫星传输架构通过基于学习的WMMSE设计和去中心化方案，有效解决了大规模MIMO卫星系统中的复杂度、信令开销和性能优化问题，为未来卫星通信系统提供了高效解决方案。
...
TL;DR: 提出基于贝叶斯学习的统一波束成形与信道估计框架，针对太赫兹频段低分辨率ADC和频率相关传播效应，采用RRC脉冲成形的双宽带信道模型，利用Bussgang分解处理非线性失真，通过层次贝叶斯组稀疏回归进行信道学习，并利用TTD元件补偿波束斜视效应。


<details>
  <summary>Details</summary>
Motivation: 太赫兹频段通信面临低分辨率ADC带来的非线性失真和频率相关传播效应等挑战，需要有效的信道估计和波束成形方案来克服这些限制，特别是在多用户宽带场景下。

Method: 1) 建立包含RRC脉冲成形的双宽带信道模型；2) 使用Bussgang分解线性化低分辨率ADC引入的非线性失真；3) 提出层次贝叶斯组稀疏回归(HBG-SR)技术，利用太赫兹信道的组稀疏特性进行多用户信道学习；4) 基于估计的AoA/AoD主导索引配置TTD元件，实现精确波束对齐和波束斜视效应补偿。

Result: 仿真结果表明，所提出的信道估计器和TTD辅助波束成形架构在实用宽带太赫兹系统约束下具有高效性、鲁棒性和显著的性能增益。

Conclusion: 该框架成功解决了太赫兹宽带系统中的信道估计和波束成形挑战，通过贝叶斯学习和TTD技术有效补偿了波束斜视效应，为实际太赫兹通信系统提供了可行的解决方案。
...
TL;DR: 本文提出了一种自可持续信息承载可重构智能表面辅助的非正交多址认知无线电系统，通过联合优化主发射机波束成形、RIS反射系数和功率分配比，最大化加权和频谱效率。


<details>
  <summary>Details</summary>
Motivation: 传统通信系统面临能耗高和频谱资源有限的问题，需要绿色可持续的解决方案。信息承载可重构智能表面能够同时收集环境射频能量并通过被动反射嵌入信息，为自可持续通信提供了有前景的途径。

Method: 提出自可持续IB-RIS辅助的NOMA认知无线电网络，采用块坐标下降优化框架，结合拉格朗日对偶和二次变换的分数规划方法，以及凸差规划方法，联合优化主发射机波束成形向量、SIB-RIS反射系数和功率分配比。

Result: 数值结果表明，所提出的SIB-RIS辅助NOMA认知无线电系统相比正交多址和主动天线方案获得了显著的加权和频谱效率增益。2比特离散相位SIB-RIS实现能够达到接近连续相位的性能，验证了低分辨率架构的实用性。

Conclusion: 自可持续IB-RIS辅助的NOMA认知无线电系统能够有效提高频谱效率，同时实现能量收集和信息传输，为绿色可持续通信提供了可行的解决方案。低分辨率RIS架构的实用性能进一步降低系统成本和复杂度。
...
TL;DR: 研究在欺骗攻击下到达角作为物理层认证特征的鲁棒性，分析模型失配情况下的认证性能


<details>
  <summary>Details</summary>
Motivation: 研究到达角作为物理层认证特征在欺骗攻击下的鲁棒性，特别是当验证器假设合法用户单源模型而实际接收信号来自多天线攻击者时的模型失配问题

Method: 假设数字均匀线性阵列验证器，推导模型失配下的误指定克拉美罗界、认证决策阈值、欺骗检测、虚警和漏检概率的闭式表达式

Result: 仿真结果验证了理论发现，并突出了信噪比、阵列几何、欺骗预编码和快照数量对认证鲁棒性的影响

Conclusion: 到达角作为物理层认证特征在欺骗攻击下存在鲁棒性问题，模型失配会影响认证性能，需要综合考虑多种因素来评估认证系统的安全性
...
TL;DR: 提出一种在Hermite-Gaussian波束域中近似近场MIMO信道的方法，使用HG模式高效表示原生信道的主要奇异模式，显著降低信道估计开销


<details>
  <summary>Details</summary>
Motivation: 近场MIMO系统中获取详细原生信道信息通常困难且昂贵，需要一种能高效近似信道并降低估计开销的方法

Method: 使用Hermite-Gaussian模式在自由空间LOS环境下近似方形天线阵列间的信道，提出迭代算法通过逐步扩展波束空间维度来逼近最大信道容量

Result: HG模式能高效表示原生信道的主要奇异模式，方法收敛速度快，显著降低信道估计开销

Conclusion: 在HG波束空间中近似近场信道是有效的，能够实现准确的信道估计和容量计算，为近场MIMO系统提供了一种实用的信道处理方法
...
TL;DR: 该论文研究了振荡器相位噪声对亚太赫兹OFDM收发器架构的影响，比较了零中频和外差式设计，发现外差式架构通过低频振荡器分布频率转换来降低相位噪声方差，在更高亚太赫兹频率下提供更好的相位噪声鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 研究亚太赫兹无线系统中振荡器相位噪声对OFDM收发器架构的影响，为未来亚太赫兹系统设计提供架构选择依据，以放宽振荡器和锁相环的约束要求。

Method: 使用Hexa-X兼容的相位噪声模型进行理论分析，比较零中频和外差式架构的相位噪声累积方差；在70 GHz和140 GHz频率下进行OFDM仿真验证。

Result: 理论分析表明外差式架构通过低频振荡器分布频率转换，将主要损伤从载波间干扰转变为公共相位误差；仿真显示在毫米波频率零中频架构仍有竞争力，但在更高亚太赫兹频率下外差式架构对相位噪声更具鲁棒性。

Conclusion: 收发器架构是未来亚太赫兹无线系统的关键设计杠杆，外差式设计能够提供更好的相位噪声鲁棒性，有助于放宽振荡器和锁相环的性能约束。
...
TL;DR: 提出基于交叉注意力Transformer的网格无关无线电地图估计方法，通过轻量级空间嵌入模块和环境知识融合，在任意位置进行信号预测，相比现有方法降低RMSE达6%


<details>
  <summary>Details</summary>
Motivation: 现有无线电地图估计方法存在显著问题：网格化方法计算效率低且无法进行网格外预测；网格无关方法虽然灵活但需要大量预处理开销，复杂度高且适应性受限。需要一种既能灵活预测任意位置信号，又具有高效计算性能的新方法。

Method: 提出基于交叉注意力Transformer的网格无关模型：1）轻量级空间嵌入模块，将环境知识融入高维特征构建；2）交叉注意力Transformer，建模目标点与测量点之间的空间相关性；3）网格无关架构，直接利用坐标特征捕捉位置特定的空间依赖性。

Result: 仿真结果表明，所提方法相比基于网格和网格无关的基线方法，将RMSE降低了高达6%，在无线电地图估计任务中表现出优越性能。

Conclusion: 提出的交叉注意力网格无关Transformer模型有效解决了现有RME方法的局限性，实现了灵活、高效且准确的无线电信号预测，为智能无线网络提供了关键技术支撑。
...
TL;DR: 本文提出了一个基于物理的信道建模与优化框架，用于站点特定环境中RIS辅助的下行多用户MISO通信系统。该框架结合射线追踪和全波电磁分析，构建确定性信道模型，并通过交替优化方案联合更新基站波束成形器和RIS负载阻抗，以最大化最小可达速率。


<details>
  <summary>Details</summary>
Motivation: 在站点特定环境中，需要可靠评估RIS部署对系统级性能的影响。现有模型往往忽略RIS散射行为和互耦效应等实际因素，因此需要开发一个能够准确捕捉实际传播特性的物理基础框架，为RIS辅助MU-MISO系统的设计和部署提供指导。

Method: 1. 开发混合射线追踪和全波电磁分析方法，构建确定性信道模型，显式捕捉多径传播、RIS散射行为和通过非对角负载阻抗表示的互耦效应。
2. 提出交替优化方案，在总发射功率约束和实际电容限制下，联合更新基站波束成形器和RIS负载阻抗，最大化最小可达速率。
3. 采用列配对1比特控制模式操作RIS，实现对所有可实现配置的穷举评估。

Result: 1. 仿真和测量结果在分布级别上通过所有配置的可达速率直方图进行比较，并在小用户位置变化下进一步检验。
2. 观察到仿真与测量结果的一致性，表明所提框架能够可靠捕捉实际性能趋势。
3. 该框架为站点特定环境中RIS辅助MU-MISO系统的设计和部署提供了有用指导。

Conclusion: 所提出的物理基础信道建模和优化框架能够可靠评估RIS部署在现实传播场景中的系统级影响。仿真与测量结果的一致性验证了该框架的有效性，为RIS辅助MU-MISO系统在站点特定环境中的设计和部署提供了实用指导。
...
TL;DR: 本文系统阐述了里德堡原子接收器的基本原理、最新进展及在量子无线通信与传感中的新实验，首次实验验证了基于RARE的OFDM传输，并探讨了深度学习优化量子无线系统的潜力。


<details>
  <summary>Details</summary>
Motivation: 在全球推动信息通信技术绿色转型和利用技术创新实现净零碳目标的背景下，里德堡原子接收器因其在微型天线设计、高灵敏度、强抗干扰性和紧凑尺寸方面的显著优势，成为满足零碳通信需求的竞争性替代方案。

Method: 系统阐述RARE的基本原理、最新进展和实验验证，首次实现基于RARE的正交频分复用传输，并探索深度学习在优化量子无线系统中的应用潜力。

Result: 实验验证了RARE在OFDM传输中的可行性，揭示了深度学习在优化量子无线系统设计中的潜力，为里德堡原子接收器在无线通信和传感应用中的发展提供了实证基础。

Conclusion: RARE作为满足零碳通信需求的竞争性技术，具有广阔应用前景。未来需要将RARE与现有前沿应用场景深度融合，并规划里德堡无线系统发展的关键路径。
...
TL;DR: 本文分析了并行Hammerstein数字自干扰消除器的性能极限，通过广义拉盖尔多项式正交展开简化分析，并提出优化导频序列准则以减少重构误差。


<details>
  <summary>Details</summary>
Motivation: 数字自干扰消除在带内全双工无线电中至关重要，但其基本性能极限尚不明确。本文旨在探索实际中最常用的并行Hammerstein消除器的性能极限。

Method: 1. 使用广义拉盖尔多项式对PH消除器进行正交展开，简化分析；2. 分析LS估计器下残余自干扰功率；3. 提出优化导频序列准则，最小化Gram矩阵特征值扩展并最大化最小特征值。

Result: 1. 证明高斯导频序列下GLP消除器的LS估计器渐近无偏；2. 优化导频序列相比传统HE-LTF序列可获得超过10dB增益；3. 残余自干扰可低至-87.6dBm。

Conclusion: 本文通过正交展开方法系统分析了PH消除器的性能极限，提出的导频序列优化准则能显著提升数字自干扰消除性能，为实际系统设计提供了理论指导。
...
TL;DR: 研究发现电动汽车电池健康状态报告不可靠，BMS系统无法准确追踪实际容量差异，提出独立电化学标记方法实现更准确的退化分类


<details>
  <summary>Details</summary>
Motivation: 目前电动汽车电池管理系统报告的电池健康状态是车主和监管机构获取的主要指标，但缺乏跨制造商的可靠性验证研究，存在信息不对称问题

Method: 采用流行病学研究方法，分析1,114辆来自5家制造商的电动汽车，进行375天的监测；开发制造商独立的电化学标记方法；在实验室对与车队相同的电池进行受控验证

Result: BMS报告的SOH与真实容量相关性差（ρ=0.10-0.62），384辆车根本不报告SOH；独立电化学标记方法实现74-89%的退化分类准确率；部分电压窗口充电测量与参考容量相关性高（ρ>0.80）

Conclusion: 电池健康报告存在结构性信息不对称，对欧盟电池法规、加州清洁汽车要求、保修执行、二手车估值、维修权立法和二次电池市场有直接影响
...
TL;DR: 本文研究了256端口X-MIMO作为6G在7GHz频段的实用架构，通过系统仿真和原型测试验证了其高吞吐量性能，并分析了大规模数字孔径对空间复用的支持能力。


<details>
  <summary>Details</summary>
Motivation: 7GHz频段作为6G的上中频段，在覆盖范围和容量之间提供了良好平衡。然而，要充分利用这一频段，需要比当前5G 64端口商用部署更强的波束赋形和空间复用能力。

Method: 首先通过系统级仿真分析基站和用户设备数字天线端口数量增加带来的吞吐量收益和设计权衡；然后开发256端口7GHz基站和用户设备原型，在城市室外环境中进行现场试验；最后基于测量数据进行信道分析。

Result: 现场测量证明在100MHz带宽上实现8层下行单用户MIMO的可行性，在城市室外传播条件下为单个用户提供超过3Gbps的速率。信道分析表明X-MIMO的大规模数字孔径即使在有限主导角度簇情况下也能支持高阶空间复用。

Conclusion: 256端口X-MIMO是7GHz 6G操作的实用架构，能够提供显著吞吐量增益。研究识别了实际部署的关键挑战，并概述了未来研究方向。
...
TL;DR: 提出一种针对OFDM系统中XL-RIS辅助的宽带信道估计方法，通过频率无关正交字典和压缩感知框架解决混合近场-远场信道估计问题


<details>
  <summary>Details</summary>
Motivation: XL-RIS的大孔径导致用户设备可能工作在近场区域，而基站-XL-RIS链路仍在远场，形成混合近场-远场特性的级联信道，加上宽带效应使毫米波/太赫兹系统的信道估计更加复杂

Method: 提出频率无关正交字典，通过增强DFT矩阵并添加额外参数，实现宽带级联信道的二维块稀疏结构表示，并在定制压缩感知框架中解决信道估计问题

Result: 仿真结果表明，所提方法在估计精度方面显著优于传统的极域信道估计方法

Conclusion: 通过频率无关正交字典和压缩感知框架，有效解决了XL-RIS辅助OFDM系统中混合近场-远场宽带信道估计的挑战
...
TL;DR: 提出ANCHOR网络作为通用时间序列建模骨干，通过频率引导可变形模块和级联谐波偏移路由，有效处理非平稳信号的相位漂移和量化误差问题。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列分析方法直接从一维序列提取特征，难以处理动态相位漂移和离散量化误差等常见问题，需要更有效的通用建模方法。

Method: 1. 将时间演化解耦为宏观物理周期和微观相位扰动；2. 提出频率引导可变形模块(FGDM)补偿微观相位偏差；3. 基于FGDM构建ANCHOR网络，采用正交通道划分和渐进残差架构解耦多尺度谐波特征。

Result: ANCHOR在大多数短期预测子任务中达到最佳性能，在异常检测和时间序列分类的多个特定子任务中表现出强大竞争力。

Conclusion: ANCHOR通过频率域先验和相位补偿机制，有效解决了时间序列建模中的相位漂移问题，可作为通用的时间序列基础骨干网络。
...
TL;DR: 该研究通过聚类算法和模型评估技术优化托管Wi-Fi网络中的预测建模，在资源受限环境下实现更准确的预测和可扩展操作。


<details>
  <summary>Details</summary>
Motivation: 解决在内存和计算资源受限的中央控制器管理的大规模环境中部署预测算法的挑战，优化资源分配并提高预测准确性。

Method: 采用基于特征的聚类方法，结合主成分分析(PCA)和高级特征工程，将时间序列数据按共享特征分组，为每个集群开发特定的预测模型。

Result: 集群特定模型在高活动集群中始终比全局模型获得更低的平均绝对误差(MAE)，在模型复杂性和资源利用率之间取得良好平衡。

Conclusion: 研究支持采用自适应网络管理策略，通过选择性模型部署优化资源分配，提高预测准确性，确保大规模托管Wi-Fi环境的可扩展操作。
...
TL;DR: 提出一种基于无人机RIS辅助的安全RSMA方案，用于异构网络中最大化公平性保密能效，通过交替优化框架解决非凸问题，相比SDMA和NOMA基准方案具有优越性能。


<details>
  <summary>Details</summary>
Motivation: 在RIS辅助的异构网络中，存在合法信息接收器和不可信能量接收器，后者可能进行窃听。需要设计安全传输方案，在保证能量传输的同时增强物理层安全，最大化公平性保密能效。

Method: 提出无人机RIS辅助的安全RSMA方案，采用交替优化框架联合设计公共消息分配、主动预编码器和RIS相移，利用序列凸近似处理非凸问题，满足发射功率和能量收集约束。

Result: 仿真结果表明所提算法具有良好的可扩展性，在保密能效性能上优于空间分多址和非正交多址基准方案。

Conclusion: 无人机RIS辅助的安全RSMA方案能有效提升异构网络中的物理层安全性能，通过联合优化设计实现公平性保密能效最大化，为未来无线通信安全提供有效解决方案。
...
TL;DR: 提出APEG框架，结合生成式AI和信道外推技术，通过CCMDM和CADM模型增强动态无线环境下的物理层身份认证性能。


<details>
  <summary>Details</summary>
Motivation: 随着6G发展，身份认证对无线安全至关重要。现有物理层认证方案难以适应动态无线环境，需要更鲁棒的自适应认证方法。

Method: 提出APEG框架，利用生成式AI自适应生成CSI指纹。开发CCMDM模型，利用协作方指纹作为条件输入进行信道外推；开发CADM模型，采用交叉注意力机制对齐多尺度信道指纹特征。

Result: APEG框架在认证性能上优于现有基于时间序列的PLA方案。CCMDM在收敛速度上有显著优势，CADM相比无模型、时间序列和VAE方法在CSI指纹生成上达到更高精度。
...
TL;DR: 提出针对太赫兹频段多用户大规模MIMO系统的半盲信道估计方法MU-WD-SB和基于稀疏贝叶斯学习的混合接收机，显著降低训练开销并提升性能


<details>
  <summary>Details</summary>
Motivation: 太赫兹频段通信面临分子吸收、反射损耗和多径扩散等挑战，传统信道估计方法训练开销大且精度有限，需要更高效的信道获取技术

Method: 1) 开发太赫兹多用户大规模MIMO信道模型；2) 提出MU-WD-SB半盲信道估计技术，利用未知数据符号的二阶统计量和导频向量；3) 推导约束Cramer-Rao下界；4) 设计基于MMV-SBL的混合接收机，利用低分辨率ADC

Result: MU-WD-SB方案相比传统训练方法和其他半盲技术，在NMSE、BER和频谱效率方面均有显著提升，有效降低训练开销并提高信道学习精度

Conclusion: 所提出的半盲信道估计和混合接收机框架为太赫兹大规模MIMO系统提供了高效解决方案，平衡了训练开销和性能，具有实际应用价值
...
TL;DR: JCAS-MARL：一种面向多无人机网络的资源感知多智能体强化学习框架，用于联合通信与感知，专注于垃圾热点检测任务。


<details>
  <summary>Details</summary>
Motivation: 随着垃圾箱溢出和非法倾倒点迅速增加，需要高效检测垃圾热点。多无人机网络在协调感知可靠性、通信质量和能量约束方面面临挑战，需要一种能同时优化这些因素的解决方案。

Method: 提出JCAS-MARL框架，其中多个无人机在共享环境中运行，每个智能体联合控制其轨迹和用于同时感知与通信的OFDM波形资源分配。系统状态包含电池消耗、充电行为和CO₂排放，信息共享通过由无人机位置和无线信道条件决定的动态通信图进行。

Result: 模拟结果表明，智能体学习的自适应导频密度控制优于静态配置，特别是在感知精度和通信连接性在环境中变化较大的场景中。

Conclusion: JCAS-MARL框架能有效探索JCAS使能无人机网络中感知-通信-能量之间的权衡，自适应资源分配策略在动态环境中表现更优。
...
TL;DR: 提出一个用于建模和缓解V2V通信系统中传输延迟抖动的数学框架，通过自适应功率分配和链路分集策略提高系统对抖动的抵御能力，相比恒定资源分配方案可将平均风险暴露率提高约3倍。


<details>
  <summary>Details</summary>
Motivation: 研究V2V通信中由干扰随机演化和车辆间距动态变化引起的定时抖动问题，需要建立能够表征系统在多种随机劣化过程影响下的动态行为，并提高系统对抖动的抵御能力。

Method: 提出新颖的数学框架，引入极限状态指标来捕捉系统性能演变，定义系统的抖动承受能力和负载引起的容量退化。研究自适应功率分配和链路分集策略（多输入单输出）来增强系统对抖动的抵御能力。

Result: 数值结果验证了量化抖动退化和实现资源感知恢复的框架有效性。自适应资源分配方案相比恒定资源分配基线方案，可将平均风险暴露率提高约3倍。

Conclusion: 提出的数学框架能够有效建模和缓解V2V通信系统中的传输延迟抖动，自适应资源分配策略显著提高了系统对抖动的抵御能力，为V2V系统的弹性通信提供了理论和技术支持。
...
TL;DR: 本文扩展了变长solid codes的构造方法，从二进制码推广到任意n元码，并证明了特定子类的错误检测性质及其在二进制码中的应用。


<details>
  <summary>Details</summary>
Motivation: Solid codes（坚固码）具有即使在任意损坏的码字串中也能正确解码的特性。变长solid codes已有研究，但现有构造主要基于二进制码，需要扩展到更一般的n元码。

Method: 将基于二进制码的变长solid codes构造方法扩展到任意n元码，并分析特定子类（基于特定构造的变长solid codes）的错误检测性质。

Result: 成功将构造方法扩展到n元码，证明了特定子类具有有趣且实用的错误检测能力，并给出了在二进制码中的具体应用实例。

Conclusion: 本文扩展了变长solid codes的理论框架，提供了更通用的构造方法，并揭示了特定子类的错误检测性质，为实际编码应用提供了理论基础。
...
TL;DR: 研究低延迟负载交付的定价机制，参与者收益取决于重建负载所需时间。推导价格-速率边界，分析分片传输、擦除编码和RLNC快速通道的经济价值。


<details>
  <summary>Details</summary>
Motivation: 在低延迟负载交付场景中，参与者的收益取决于重建负载所需时间，解码时间分布决定了满足截止时间的概率，从而限制了参与者为额外交付速率付费的意愿。需要建立定价机制来量化符号有用性如何转化为经济价值。

Method: 使用平均场公式，从简单的随机到达模型推导价格-速率边界，并实例化三种情况：(1) 非分片传输；(2) 分片交付的三种机制：非编码分片、固定速率擦除编码、无速率编码。进一步分析包含基础通道和RLNC快速通道的双通道服务，在固定基础通道价格-速率对和总速率约束下推导快速通道定价边界。

Result: 推导出价格-速率边界，提供了符号有用性如何转化为经济价值的比较特征。分析显示，在双通道服务中，即使适度的额外RLNC速率也能产生可测量的效用增益，具体取决于基础通道的传播机制。框架可扩展到具有多个截止时间的逐步奖励计划。

Conclusion: 建立了低延迟负载交付的定价框架，量化了时间敏感环境中符号有用性的经济价值。该框架适用于区块链消息传播和延迟敏感竞争等场景，为设计高效的低延迟交付系统提供了理论基础。
...
TL;DR: 提出GAIQN和MAGAQN算法优化可移动天线和夹持天线的位置，以最大化有效秩指标，提升空间自由度，同时确保天线间无碰撞。
...
Method: 引入有效秩作为度量指标，针对MA和PA系统分别提出GAIQN（图注意力隐式分位数网络）和MAGAQN（多智能体图注意力Q网络）算法优化天线位置。设计top-k动作选择方法确保多天线在二维阵列平面或同一波导上无碰撞。

Result: 仿真结果表明，GAIQN和MAGAQN算法相比基准方法分别至少提升1.6%和1.3%的有效秩，同时始终确保柔性天线间无碰撞。在相同天线数量下，MA系统支持更高的有效秩，而PA系统在可达到的空间自由度方面提供更好的稳定性。

Conclusion: 提出的GAIQN和MAGAQN算法能有效优化柔性天线位置，提升空间自由度，为下一代无线通信中的柔性天线技术提供了有效的优化方法。
...
TL;DR: 提出一种分布式估计算法，用于处理重尾观测和通信噪声下的未知向量参数估计，属于共识+创新类估计器，通过非线性变换对抗重尾噪声影响。


<details>
  <summary>Details</summary>
Motivation: 在物联网和无线传感器网络等密集部署系统中，重尾噪声频繁出现，传统估计方法可能失效，需要开发能够处理重尾噪声的分布式估计算法。

Method: 采用共识+创新类估计器框架，在共识更新和创新更新部分添加一般非线性变换来对抗重尾噪声的影响，实现分布式参数估计。

Result: 证明了估计器的几乎必然收敛性和渐近正态性，并提供了新颖的分析研究，揭示了系统噪声与底层网络拓扑结构之间的权衡关系。

Conclusion: 该算法能够有效处理重尾噪声环境下的分布式参数估计问题，为密集部署的物联网和传感器网络系统提供了实用的估计解决方案。
...
TL;DR: DARA：基于Transformer路径状态预测和深度强化学习的多路径调度器，解决车载网络中无线链路质量快速波动问题


<details>
  <summary>Details</summary>
Motivation: 现代5G+多接入网络为移动终端提供额外容量，但在车载网络等高移动性环境中，支持多接入连接仍具挑战性。无线链路质量的快速波动往往超过现有多路径调度器和传输层协议的响应能力。

Method: 提出Deep Adaptive Rate Allocation (DARA)调度器，集成Transformer-based路径状态预测和深度强化学习引擎。使用DQN策略动态计算可用路径上的最优拥塞窗口分数，通过六分量归一化奖励函数进行权重调节冲突解决。

Result: 在基于Mininet的多路径数据报拥塞控制协议测试平台上评估，使用车载环境移动用户轨迹。DARA在中等波动轨迹下相比学习型调度器实现更好的文件传输时间减少；在缓冲视频流中，在所有测试条件下保持分辨率改进；在具有亚秒级缓冲区约束的受控突发场景中，DARA实现显著的重缓冲改进，而最先进的调度器表现出近乎连续的卡顿。

Conclusion: DARA通过集成Transformer预测和深度强化学习，有效解决了车载网络中多路径调制的响应滞后问题，在文件传输和视频流场景中均表现出优越性能。
...
TL;DR: 本文研究了两种差分隐私约束同时成立时的精确组合机制，提出了隐私区域的精确表示方法，并扩展到任意数量DP约束的组合，最后应用于近似f-DP组合。


<details>
  <summary>Details</summary>
Motivation: 研究差分隐私机制组合时，当两种不同的隐私约束同时成立的情况，需要理解这种组合的精确隐私保证，为更复杂的多约束组合提供理论基础。

Method: 通过二元假设检验的混合结构引理，将隐私区域表示为异构DP保证机制的混合组合，构建了可推广到任意数量DP约束组合的框架。

Result: 得到了两种DP约束同时成立时的精确隐私区域表示，该表示可以自然推广到任意数量DP约束的组合情况。

Conclusion: 提出的方法为差分隐私机制组合提供了精确的理论框架，特别适用于近似f-DP的组合分析，扩展了差分隐私组合理论。
...
TL;DR: 本文研究了随机线性码为MDS码的概率阈值，以及随机矩阵为超正则矩阵的概率阈值，并给出了精确的渐近概率公式。


<details>
  <summary>Details</summary>
Motivation: 研究随机线性码成为最大距离可分(MDS)码的概率阈值问题，以及相关的随机矩阵超正则性问题，这些在编码理论和矩阵理论中具有重要意义。

Method: 采用概率方法和渐近分析技术，研究当有限域大小q、码长n和维数k趋于无穷时，随机线性码为MDS码的概率行为。通过分析参数的不同渐近关系，建立精确的概率阈值。

Result: 证明了当1/q * C(n,k) → 0时，P(C是MDS) → 1；当1/q * C(n,k) → ∞时，P(C是MDS) → 0。对于超正则矩阵，当4^k/√k/q → 0时，P(A是超正则) → 1；当4^k/√k/q → ∞时，P(A是超正则) → 0。还得到了中间阈值的精确渐近概率公式。

Conclusion: 建立了随机线性码MDS性质和随机矩阵超正则性质的完整概率阈值理论，揭示了这些性质的相变行为，并发现了超正则矩阵计数函数的非多项式特性。
...
TL;DR: 首次从理论上解释了相干区域内多径干扰如何重塑PAM-4星座图，从而增加数据中心内比特错误率的波动


<details>
  <summary>Details</summary>
Motivation: 数据中心内光通信系统中，多径干扰对高阶调制格式（如PAM-4）的影响机制尚不明确，需要理论解释其如何导致比特错误率波动

Method: 采用理论分析方法，研究相干区域内多径干扰对PAM-4星座图的影响机制，分析星座图重塑与比特错误率波动的关系

Result: 首次从理论上阐明了多径干扰在相干区域内会重塑PAM-4星座图，这种星座图变形直接导致了比特错误率的增加和波动

Conclusion: 多径干扰在相干区域内对PAM-4星座图的重塑是导致数据中心内比特错误率波动增加的关键机制，这一理论解释对优化数据中心光通信系统设计具有重要意义
...
TL;DR: 本文提出了一种通用的两阶段方法来推导[0,1]^K上分布的最小散度问题的对偶表示，适用于广泛的f-散度和积分约束，并应用于序列测试、估计和变化检测的最优算法设计。


<details>
  <summary>Details</summary>
Motivation: 在统计学和概率论中，积分约束下的最小散度问题广泛存在于序列推断、多臂赌博机理论和分布鲁棒优化等领域。对偶表示是将信息论下界转化为计算可行（且通常接近最优）算法的关键步骤。

Method: 提出两阶段方法：第一阶段使用有限维凸对偶技术推导有限支撑分布的对偶表示；第二阶段建立抽象交换论证，将离散化对偶提升到任意分布。从均值约束的最小相对熵(KL_inf)开始，将单维情况推广到任意维度，再扩展到广泛的f-散度和一般积分约束泛函。

Result: 建立了[0,1]^K上分布的最小散度问题的通用对偶表示框架，能够处理广泛的f-散度和积分约束。展示了该方法在序列测试、估计和变化检测中的统计应用，构造了这些问题的优化程序。

Conclusion: 本文提供了一个系统的方法来推导约束最小散度问题的对偶表示，显著扩展了现有技术的适用范围，为序列推断、多臂赌博机等领域的算法设计提供了理论基础和实用工具。
...
TL;DR: 本文提出了广义超码引理，用于下界估计线性码的广义覆盖半径，简化了BCH(2,m)码的已知下界证明，并建立了新的下界，同时证明了广义覆盖半径的层次结构满足2k ≤ ρ_k ≤ 2k+1。


<details>
  <summary>Details</summary>
Motivation: 二进制原始双纠错BCH码的广义覆盖半径研究虽然已有进展，但证明依赖复杂的组合论证，且对更大阶数k的行为缺乏探索。需要更系统的方法来研究广义覆盖半径的层次结构。

Method: 提出广义超码引理，利用适当超码的广义汉明重量来下界估计码的广义覆盖半径。结合组合论证和Weil型指数和估计，研究一般k的广义覆盖半径层次结构。

Result: 简化了ρ₂和ρ₃下界的证明，建立了ρ₄的新下界，证明了当m相对于k足够大时，2k ≤ ρ_k(BCH(2,m)) ≤ 2k+1。

Conclusion: 广义超码引理为研究广义覆盖半径提供了有效工具，揭示了BCH(2,m)码广义覆盖半径的层次结构特性，为更一般的码类研究奠定了基础。
...
TL;DR: 提出基于锚定仿射似然比律的匿名洗牌实验几何框架，证明二元随机响应在隐私保护中具有极值性，并在低预算条件下给出最优设计


<details>
  <summary>Details</summary>
Motivation: 为匿名洗牌实验建立统一的几何理论框架，解决在本地差分隐私约束下的最优信道设计和隐私分析问题

Method: 基于锚定仿射似然比律的几何框架，将有限输出d元信道与正则单纯形上的零均值测度对应，利用凸f-散度和曲棍球剖面分析隐私极值性

Result: 证明二元随机响应在ε₀-LDP信道中对所有凸f-散度和洗牌后的曲棍球剖面具有普遍极值性；在低预算条件下，增强随机响应在所有信道和估计器中达到极小极大最优

Conclusion: 建立了匿名洗牌实验的完整几何理论，解决了隐私极值性和最优信道设计问题，为实际应用提供了理论基础和具体设计方法
...
TL;DR: 本文为幂律分布建立了一个构造性概率框架，从非线性微分方程 dy/dx = y^q 出发，推导出广义二项分布，证明了在 0<q<1 时的大偏差原理（以α-散度为速率函数），并证明了广义棣莫弗-拉普拉斯定理（收敛到q-高斯分布）。


<details>
  <summary>Details</summary>
Motivation: 传统的大偏差原理和中心极限定理基于独立同分布假设，而幂律分布的概率基础主要通过描述性模型或变分原理发展，缺乏类似经典二项过程的构造性推导。本文旨在为幂律分布建立一个构造性概率框架。

Method: 从非线性微分方程 dy/dx = y^q 出发，不预先假设特定分布，建立代数和组合基础，推导出基于有限计数的广义二项分布。在 0<q<1 时证明大偏差原理，识别α-散度为速率函数，并证明广义棣莫弗-拉普拉斯定理，显示收敛到q-高斯分布。

Result: 证明了广义二项分布的大偏差原理（α-散度为速率函数），揭示了在较重尾部（q>1）时宏观标度的失效。证明了广义棣莫弗-拉普拉斯定理，显示收敛到q-高斯分布，标度律为 n^{q/2}。数值验证了 q∈(0,2) 的结果。

Conclusion: 该框架为幂律分布提供了构造性基础，统一了平移不变的指数族和重标度不变的幂律族，将构造性框架与信息几何结构联系起来。
...
Result: 系统感知延迟25ns，刷新率超过10kHz，比传统毫米波传感器快两个数量级，能效达17 TOPS/W。在开放道路场景中，对人和车辆的实时分类准确率达91-97%。

Conclusion: P-SPNN为现实世界高速感知任务提供了一个可编程、可扩展、低功耗的平台，对自动驾驶、智能感知和下一代AI硬件具有广泛意义。
...
<details>
  <summary>Details</summary>
Motivation: 无人机中继网络中，无人机间的间歇性信息交换导致系统状态获取延迟，阻碍了有效协作，影响了网络吞吐量。需要解决信息延迟和不可靠信道条件下的信息丢失问题。

Method: 1) 提出延迟容忍的多智能体深度强化学习算法，集成延迟惩罚奖励机制鼓励无人机间信息共享；2) 提出基于时空注意力的预测方法，恢复因不可靠信道条件丢失的信息；3) 联合优化无人机轨迹规划、网络形成和传输控制策略。

Result: 相比传统MADRL，新方法实现了超过50%的信息延迟减少和75%的吞吐量增益。改善信息共享不仅不会牺牲网络容量，反而能同时显著提升学习性能和吞吐量，并减少无人机信息交换需求。

Conclusion: 提出的延迟容忍MADRL算法和时空注意力预测方法能有效提升无人机辅助无线网络的容量，减少信息交换需求，促进MADRL在实际无人机网络中的部署应用。
...
TL;DR: 提出非线性变换无速率信源信道编码(NTRSCC)，一种基于无速率码的广播信道可变长度JSCC框架，实现失真、速率和解码复杂度的可控权衡。


<details>
  <summary>Details</summary>
Motivation: 无线边缘出现大量数据密集型广播应用，需要灵活权衡失真、传输速率和处理复杂度。现有DeepJSCC方案多为最坏情况解、缺乏自适应复杂度、广播场景效率低。

Method: 将学习到的信源变换与物理层LT码结合，开发利用解码器边信息的非均匀保护方案，设计近似方法实现无速率参数端到端优化。

Result: 仿真结果表明，该方法在异构边缘设备的严格通信和处理预算下，提升了图像广播质量。

Conclusion: NTRSCC框架使异构接收机能够自适应调整接收的无速率符号数量和置信传播解码迭代次数，实现了失真、速率和解码复杂度的可控权衡。
...
TL;DR: 该论文提出了一种考虑元素间相似性结构的新型散度度量方法，相比传统信息论量（如熵和f-散度）能更好地处理具有几何结构的概率分布。


<details>
  <summary>Details</summary>
Motivation: 许多自然和社会科学系统涉及具有相似关系的元素（如共享技能的职业或相似特征的物种），传统信息论量（熵和f-散度）忽略元素间的相似性结构，需要能感知几何结构的度量方法。

Method: 引入结构感知散度族，这些散度是结构感知熵的Bregman散度，保留了Kullback-Leibler散度和Shannon熵的多个优点，同时考虑底层域的几何结构。

Result: 结构感知散度在合成聚类任务中能发现传统散度遗漏的模式，比最优传输距离快几个数量级；在经济地理学中能产生不同的职业地理分布区域化结果；在生态学中能复制最优传输方法获得的功能性β多样性见解。

Conclusion: 提出的结构感知散度为处理具有相似性结构的概率分布提供了高效框架，在多个学科中展示了实用价值，填补了传统信息论量忽略几何结构的空白。
