...
TL;DR: AI驱动的Wi-Fi感知系统在硬件受限条件下通过先验信息和时序相关性突破传统雷达理论的分辨率限制，使用单收发器对实现了人体姿态估计和室内定位的高精度感知。


<details>
  <summary>Details</summary>
Motivation: 研究AI如何帮助Wi-Fi感知系统在带宽和天线数量受限的情况下实现高精度感知，并探索其超越传统雷达理论分辨率限制的理论基础。

Method: 开发基于AI的Wi-Fi感知系统，使用单收发器对，通过人体姿态估计和室内定位实验验证先验信息和时序相关性对性能提升的贡献。

Result: 实验结果证实了时序相关性和先验信息确实能够提升Wi-Fi感知系统的性能，使系统在硬件受限条件下仍能实现高精度感知。

Conclusion: AI驱动的Wi-Fi感知系统通过利用先验信息和时序相关性，能够在硬件约束条件下突破传统分辨率限制，为下一代Wi-Fi技术的大规模部署提供理论支持。
...
TL;DR: 提出了一种基于时空注意力网络(STAN)的深度学习框架，用于癫痫发作预测，通过建模脑电信号的时空相关性并利用对抗鉴别器区分发作前和发作间期模式，实现患者特异性学习。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖特征工程和/或假设固定的发作前持续时间，无法有效建模复杂的时空相关性，需要开发能够自适应学习患者特异性模式的方法。
...
Conclusion: 该框架能够可靠地在发作前至少15分钟检测到发作前状态，患者特异性窗口可延长至45分钟，为临床应用提供了足够的干预时间。
...
TL;DR: EEGReXferNet是一个轻量级生成AI框架，通过跨被试迁移学习进行EEG子空间重建，解决了传统EEG信号处理中噪声干扰和计算复杂度高的问题。


<details>
  <summary>Details</summary>
Motivation: 传统EEG信号处理存在信噪比低、需要人工干预、可能抑制关键神经特征等问题，而现有的生成模型方法缺乏时空谱敏感性且计算量大，不适合实时脑机接口应用。

Method: 采用模块化架构，利用邻近通道的容积传导、频带特定卷积编码和滑动窗口动态潜在特征提取，结合参考缩放确保窗口间连续性，实现跨被试泛化。

Result: 提高了时空谱分辨率（平均PSD相关性≥0.95；平均谱图RV系数≥0.85），总权重减少约45%以减轻过拟合，保持计算效率。
...
TL;DR: 本研究改进了T1D数据集DiaData的数据质量，通过异常值处理、缺失值插补等方法，并分析了血糖与心率的相关性，建立了低血糖分类基准模型。


<details>
  <summary>Details</summary>
Motivation: T1D个体化治疗需要高质量数据支持，但现有数据存在异常值、噪声和小样本问题，影响分析可靠性。

Method: 使用IQR方法识别异常值并用缺失值替换；小间隙用线性插值，大间隙用Stineman插值；分析血糖与心率相关性；用ResNet模型建立低血糖分类基准。

Result: Stineman插值比线性插值提供更真实的血糖估计；血糖与心率在低血糖前15-60分钟呈中度相关；使用更多数据和精炼数据分别提升模型性能7%和2-3%。

Conclusion: 数据质量改进显著提升T1D数据分析可靠性，为个体化治疗提供更好支持。
...
TL;DR: ECGXtract是一种基于深度学习的可解释心电特征提取方法，通过卷积神经网络提取与临床验证基准强相关的时域和形态特征，在多个实验设置下表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决传统信号处理和黑盒机器学习方法在心电特征提取中的局限性，开发可解释且准确的特征提取方法。

Method: 使用卷积神经网络模型，先训练单个特征提取模型确保精确性和可解释性，然后探索多特征同时提取的可行性，包括语义分组和大规模分组策略。

Result: ECGXtract在全局特征上平均相关分数0.80，导联II表现最佳；导联特定特征平均相关分数0.822；在90%的特征上优于ECGdeli；语义分组对全局特征有效。

Conclusion: 结构化分组策略能在计算效率和模型精度之间取得平衡，为资源受限环境下开发可扩展且临床可解释的心电特征提取系统铺平道路。
...
<details>
  <summary>Details</summary>
Motivation: 先进的心脏AI在日常监测中面临医疗数据需求大和计算资源高的问题，而低成本心脏智能系统虽然使用可穿戴设备数据，但与高成本系统存在显著的诊断性能差距。

Method: 提出LiteHeart框架，包含区域感知蒸馏模块（模拟心脏病专家关注诊断相关ECG区域）和跨层互信息模块（对齐LCCI和HCCI系统的决策过程），采用半监督训练策略提高模型鲁棒性。

Result: 在涵盖38种心血管疾病的5个数据集上评估，LiteHeart将LCCI与HCCI之间的性能差距显著缩小，在宏观F1分数上比现有方法提高4.27%至7.10%。
...
TL;DR: 提出了一种实时交互式混合海洋模拟方法，将全局FFT背景与局部波粒子区域结合，在统一频谱参数下实现大规模光谱真实性和细粒度交互性。


<details>
  <summary>Details</summary>
Motivation: 传统FFT方法假设全局平稳性和空间均匀性，难以表示非均匀海洋和近场交互；而波粒子方法虽然能捕捉局部波浪但计算成本高且难以匹配全局光谱统计。

Method: 采用混合海洋表示：全局FFT背景与局部WP区域在统一频谱下耦合；基于频率分桶的粒子采样和GPU并行合成方案。

Result: 实现了大规模光谱一致性和局部波浪细节的实时交互模拟，边界处粒子注入与背景频谱对齐，能量密度匹配且不影响远场。

Conclusion: 该混合框架在实时交互性能下同时提供了大规模光谱真实性和细粒度交互能力。
...
TL;DR: 提出了一种基于心电图（ECG）的意识状态估计系统，使用解耦查询注意力transformer来捕捉心率变异性特征，在睡眠分期和麻醉监测任务中优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 传统意识状态监测主要依赖脑电图（EEG），但EEG易受噪声干扰且需要受控环境。本研究旨在开发基于ECG的非侵入式、可靠意识状态估计方法，更适合动态临床环境。

Method: 使用解耦查询注意力transformer架构，从ECG信号中提取心率变异性特征来区分意识和无意识状态，并实现实时监测系统。

Result: 在睡眠分期任务中准确率达到0.877，麻醉水平监测准确率达到0.880，AUC值分别为0.786和0.895，均优于基线模型。

Conclusion: 基于ECG的意识状态监测系统为EEG方法提供了实用且稳健的替代方案，有望提高患者安全性并增进对意识状态的理解。
...
TL;DR: NEF-NET+是一个增强的ECG全景合成框架，能够从任意视角合成任意长度的ECG信号，解决了Nef-Net在真实场景中的局限性，包括长时程ECG建模、设备特定伪影鲁棒性和电极放置校准问题。


<details>
  <summary>Details</summary>
Motivation: 传统多导联ECG系统只能从固定的解剖视角捕获信号，某些心脏疾病需要非标准视角来显示诊断关键模式。Nef-Net虽然能重建连续心电场，但在真实场景中面临长时程建模、设备伪影和电极放置偏差等挑战。

Method: 设计了新的模型架构执行直接视角变换，包含离线预训练、设备校准调优步骤以及用于患者特定适应的实时校准步骤。构建了包含5367个记录、每受试者48个视角的Panobench基准数据集。

Result: 实验结果显示NEF-NET+相比Nef-Net在真实世界设置中PSNR提高了约6 dB，显著提升了全景ECG合成性能。

Conclusion: NEF-NET+通过增强的框架设计，成功解决了全景ECG合成在真实场景中的关键挑战，为从任意视角观察ECG信号提供了更实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 毫米波FMCW雷达在内部温度变化时硬件会产生漂移，影响测量可靠性，特别是在密集网络部署中需要确保性能稳定性。

Method: 利用内部传感器数据和信号处理技术建立温度补偿模型，通过处理中频信号来校正温度引起的漂移。

Result: 实验结果显示，该方法将中频信号幅度与内部温度漂移的皮尔逊相关性降低了84%，显著减轻了温度漂移影响，且计算开销小。

Conclusion: 该框架有效提升了毫米波雷达在温度变化下的鲁棒性，适用于密集网络环境，并遵循伦理设计原则避免依赖敏感外部数据。
...
TL;DR: 提出一种基于ViT自编码器的数据驱动方法，将窄带超声RF信号映射为宽带频谱图，显著提升超声图像质量。


<details>
  <summary>Details</summary>
Motivation: 解决低成本窄带超声探头带宽不足导致的脉冲展宽和高频细节丢失问题，通过纯软件升级实现宽带性能。

Method: 使用Tiny ViT自编码器变体，在仿真数据上训练，采用课程加权损失函数，学习从窄带到宽带RF频谱图的映射。

Result: 在异质斑点-囊肿模型中，图像域MSE降低90%，PSNR提升6.7dB，SSIM达到0.965；在未见过的分辨率模型中也能锐化点目标，展现强泛化能力。

Conclusion: 纯软件升级可使现有窄带探头获得类似宽带性能，有望在资源受限环境中推广高分辨率超声应用。
...
TL;DR: 提出一种基于加权调和均值密度(HMD)的水下目标跟踪方法，通过最小化KL散度分配最优权重，在分布式跟踪中优于现有融合方法


<details>
  <summary>Details</summary>
Motivation: 解决水下目标跟踪问题，其中大量声纳浮标传感器部署在监视区域，需要实现分布式跟踪器之间的共识

Method: 使用加权调和均值密度(HMD)进行跟踪共识，通过最小化Kullback-Leibler散度度量来分配最优权重，提供高斯密度融合解决方案

Result: 仿真结果显示，优化的HMD融合方法在均方根误差、轨迹发散百分比和归一化估计误差平方等指标上优于现有融合方法

Conclusion: 提出的优化HMD融合方法在分布式跟踪中表现出优越性能
...
TL;DR: 提出了一种多智能反射面协作混合定位系统，通过联合时延和角度估计实现目标定位，设计了高效的角度和位置估计算法，在低信噪比条件下表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决目标定位问题，特别是在低信噪比环境下的精确定位需求，需要开发能够充分利用多IRS协作和混合定位优势的系统。

Method: 使用多个IRS部署在目标区域附近，通过接收所有反射单元的反射信号来联合估计时延和角度参数。设计了基于原子范数最小化的角度估计算法和三阶段位置估计算法（加权最小二乘、总体最小二乘和二次校正）。

Result: 数值仿真验证了系统的优越性，多IRS协作、混合定位和分布式部署提供了显著优势，所提出的估计算法在低信噪比条件下仍能保持高精度。

Conclusion: 该多IRS协作混合定位系统能够有效实现目标定位，特别是在挑战性环境中，所提出的算法能够接近克拉美罗界，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 为无线通信研究提供全面的多模态数据集，支持通信和协同感知等研究方向。

Method: 基于CARLA模拟器和Sionna框架构建集成可定制数据管道，在四个虚拟城镇、十六个通信场景和三种天气条件下收集数据。

Result: 成功构建包含约16万帧多模态数据的数据集，涵盖多种感知模态和场景条件。

Conclusion: 该数据集为无线通信研究提供了宝贵资源，展示了在通信和协同感知等领域的应用潜力，如使用多模态大语言模型进行波束预测。
...
TL;DR: 提出了一种去中心化共识ADMM算法，用于优化无人机群位置，以平衡通信速率最大化和感知CRB最小化，显著提升了集成感知与通信性能。


<details>
  <summary>Details</summary>
Motivation: 无人机群可以形成虚拟天线阵列来增强集成感知与通信能力，但由于群集的分布式特性和单个无人机缺乏全局视野，位置优化具有挑战性。

Method: 开发了去中心化共识ADMM算法，将全局目标分解为局部投影更新、代理辅助的共识协调和轻量级对偶更新，使无人机能够并行决策位置并达成全局最优群集几何的共识。

Result: 仿真表明，所提出的共识ADMM算法具有快速收敛性和强可扩展性，无人机群在通信和感知性能上显著优于固定阵列基线。

Conclusion: 该去中心化优化框架有效解决了无人机群位置优化问题，实现了通信与感知性能的平衡提升，为集成感知与通信系统提供了可行的解决方案。
...
TL;DR: 提出了一种分布式聚合权重优化算法，用于去中心化联邦学习，通过本地通信优化权重以最小化收敛界，实现完全分布式的DFL系统。


<details>
  <summary>Details</summary>
Motivation: 传统DFL依赖中央实体收集本地信息进行系统优化，这与DFL的去中心化本质不符，需要开发分布式权重优化方法。

Method: 分析聚合权重对收敛的影响，将学习性能优化问题转化为特征值优化问题，提出基于次梯度的分布式算法，仅需本地信息和D2D通信。

Result: 数值结果表明所提算法在实际DFL部署中具有优越性，实现了真正分布式的DFL系统。

Conclusion: 成功开发了与DFL去中心化特性一致的分布式聚合权重优化算法，使优化、通信和学习过程都能以分布式方式进行。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究往往忽略飞机高速移动产生的湍流对太赫兹通信的额外传播损耗，而这对空对地场景中的超高速数据传输至关重要。

Method: 建立基于流体动力学的衰减模型表征飞机湍流，构建联合功率-姿态优化问题，使用扩散算法学习飞行配置与湍流衰减的非线性关系。

Result: 湍流引起的衰减在-10度到10度攻击角、0.7马赫速度下达到18-28dB；所提框架平均容量达11.241bps/Hz，比现有策略提升22.8%和66.5%，接近理论容量极限的98%。

Conclusion: 飞机诱导湍流对太赫兹传播具有显著影响，所提出的AI赋能框架能有效缓解湍流衰减并显著提升通信性能。
...
TL;DR: 该论文研究了去中心化机器学习在随机拓扑网络中的收敛性问题，提出了基于谱优化的通信可靠性提升方法，并在卫星星座场景中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 去中心化机器学习在大规模网络中面临通信不可靠的问题，随机拓扑变化会影响学习收敛性能，需要建立通信质量与全局性能的直接联系。

Method: 通过谱优化最小化期望二阶混合矩阵的谱半径，设计了结合切比雪夫加速特征向量估计的分布式次梯度算法，在保持对称性和随机性约束下调整本地更新和聚合权重。

Result: 在低地球轨道卫星星座和真实遥感数据上的实验表明，该方法相比基线显著提高了分类准确率和收敛效率。

Conclusion: 所提出的谱优化方法能有效提升去中心化系统在不可靠通信环境下的学习性能，适用于卫星等分布式系统。
...
TL;DR: 提出了一种基于OFDM通信信号的协作式感知与通信一体化SAR成像框架，采用两阶段CS-SAGE方案解决NLOS环境下成像退化问题


<details>
  <summary>Details</summary>
Motivation: 解决在非视距环境下使用OFDM通信信号进行SAR成像时出现的严重成像退化问题，提高感知与通信一体化的性能

Method: 开发了两阶段压缩感知-空间交替广义期望最大化(CS-SAGE)方案：第一阶段使用正交匹配追踪(OMP)进行粗估计，第二阶段使用SAGE算法进行精细参数提取

Result: 仿真结果验证了所提协作式ISAC框架的有效性，为实际系统设计提供了有价值的见解

Conclusion: 提出的协作式ISAC框架能够有效解决NLOS环境下的成像退化问题，实现高精度的散射体定位
...
TL;DR: 本文提出了一种新型的CIS-RAN架构，将传统C-RAN扩展为包含协作感知和协作AI的智能服务型无线接入网络，通过全流程协作提升网络性能。
...
Method: 在C-RAN基础上提出CIS-RAN架构，集成协作通信、协作感知和协作AI，实现采集、传输和处理全流程的网络协作。

Result: 通过网络协作MIMO案例研究表明，CIS-RAN相比传统架构具有更优越的性能，数值结果验证了其有效性。

Conclusion: CIS-RAN架构为6G网络发展提供了新方向，未来研究应继续探索和推进网络协作的增强。
...
TL;DR: 本文介绍了无线供电的夹持天线系统(PAS)，分析了其在无线供电通信(WPC)系统中的可靠性优势，推导了中断概率和遍历速率的闭式表达式，并研究了波导和用户的最优部署策略。


<details>
  <summary>Details</summary>
Motivation: 夹持天线系统通过灵活调整天线位置建立强视距链路来减少自由空间路径损耗，本文旨在探索夹持天线在改善无线供电通信系统性能方面的优势。

Method: 推导了实际有损波导和理想无损波导情况下的中断概率和遍历速率的闭式表达式，分析了波导和用户的最优部署策略。

Result: 结果表明，吸收系数和用户区域尺寸的增加会导致更高的波导内和自由空间传播损耗，从而提高中断概率、降低遍历速率。但在高吸收系数和长波导条件下，无线供电PAS的遍历速率优于传统WPC系统。

Conclusion: 无线供电PAS存在最优时间分配因子和PS与AP之间的最优距离，能够最小化中断概率或最大化遍历速率。PS和AP分离在最优距离时的系统性能优于集成到混合接入点的情况。
...
TL;DR: 提出一种改进的OFDM波形，可降低频谱整形技术的优化和实现成本，优化系数和乘积运算最多可减少50%。


<details>
  <summary>Details</summary>
Motivation: OFDM存在高带外发射问题，现有频谱整形技术虽然有效但涉及复杂的优化过程和实时实现成本较高。

Method: 对传统OFDM波形进行修改，为频谱整形技术建立降低成本的框架。

Result: 该方法可将优化涉及的系数数量和实现所需的乘积运算数量最多减少50%。

Conclusion: 该改进方法显著降低了频谱整形技术的成本和复杂度，为未来相关研究提供了有益框架。
...
TL;DR: 本文提出了一种用于6G集成感知与通信(ISAC)系统的单静态背景信道随机模型，通过多参考点叠加的方法解决了标准兼容的ISAC信道建模难题。


<details>
  <summary>Details</summary>
Motivation: 现有通信标准主要针对分离的收发器，而ISAC单静态模式(收发器共址)的背景信道建模仍是一个紧迫挑战，这对评估复杂环境中的感知性能至关重要。

Method: 首先进行28GHz室内ISAC单静态背景信道测量，提取真实信道参数；然后提出将单静态背景信道建模为单静态收发器与多个通信接收器类参考点之间子信道叠加的随机模型；最后使用遗传算法优化多参考点的数量和位置。

Result: 测量结果显示明显的单跳传播和离散多径分布特性；提出的模型通过比较实测和仿真信道参数得到验证，能有效捕捉单静态背景信道特征。

Conclusion: 该模型填补了ISAC信道建模的关键空白，支持6G标准化，为ISAC系统设计提供了现实且标准兼容的信道模型解决方案。
...
TL;DR: 提出了一个分布式集成感知与通信(DISAC)系统中的协作用户跟踪框架，通过全局PHD滤波器和视场感知AP管理策略实现厘米级精度的用户跟踪，并证明无需始终保持所有AP激活即可维持高跟踪精度。


<details>
  <summary>Details</summary>
Motivation: 随着ISAC成为6G网络的重要组成部分，分布式ISAC有望通过去中心化架构提升感知和通信性能，需要解决DISAC系统中的协作用户跟踪挑战。

Method: 采用全局概率假设密度(PHD)滤波器和视场感知接入点(AP)管理策略，结合分布式MIMO信道测量来评估框架有效性。

Result: 实现了厘米级的均方根轨迹误差，并表明无需始终保持所有AP激活即可维持高跟踪精度。

Conclusion: 这些发现为DISAC系统中协作用户跟踪技术的实际部署和进一步发展提供了有价值的见解，强调了稳健高效AP管理的必要性。
...
TL;DR: 提出了一种用于量子低密度奇偶校验码的多基置信传播列表解码器，在保持线性时间复杂度的同时显著降低了逻辑错误率。


<details>
  <summary>Details</summary>
Motivation: 现有的QLDPC码解码器在性能和计算复杂度之间存在权衡，需要一种既能保持线性时间复杂度又能改善逻辑错误率的解码方法。

Method: 扩展了多基置信传播框架，结合新的后处理列表解码器决策规则，替代传统的LMS准则。同时探索了单变量自行车码这一新的子类。

Result: 对于[[144,12,12]]双变量自行车码，MBBP-LD解码器相比最先进的BP-OSD解码器降低了40%的逻辑错误率，同时保持线性时间复杂度。

Conclusion: MBBP-LD解码器为QLDPC码提供了一种高效且高性能的解码解决方案，同时单变量自行车码在降低搜索空间方面显示出潜力。
...
TL;DR: 本文提出了Tsallis q阶F统计量F_q，作为经典F_ST统计量的信息论推广，通过调节q值可以分别强调稀有或常见变异，在等位基因频率谱偏斜时提供比F_ST更精细的种群分化视图。


<details>
  <summary>Details</summary>
Motivation: 经典F_ST统计量基于方差分析，在等位基因频率谱偏斜时可能无法充分捕捉种群分化模式。需要一种更通用的统计量，能够通过参数调节来强调不同频率范围的变异。

Method: 引入Tsallis q阶F统计量F_q，测量亚群相对于合并种群的Tsallis q熵损失。该家族在q=2时嵌套经典F_ST，在q=1时对应香农熵版本。通过调节q值，可以分别强调稀有变异(q<1)或常见变异(q>1)。

Result: 在真实数据(865个大洋洲基因组)和受控系谱模拟中，F_q在One-vs-Rest和Leave-One-Out模式下能够清晰识别驱动区域结构的亚群，并敏感地时间标记隔离迁移事件和奠基者效应。

Conclusion: F_q作为更高分辨率的补充工具，可用于模拟审计和种群结构总结，提供比传统F_ST更精细的种群分化视角。
...
TL;DR: 提出了两种抗干扰算法PGD-DDQN和NQC-DDQN，在多时间尺度决策模型中考虑状态不确定性，在存在感知误差的情况下保持抗干扰性能。


<details>
  <summary>Details</summary>
Motivation: 现有抗干扰方法假设精确感知且单时间尺度优化，忽略了异构动作执行延迟不匹配和传感器缺陷导致的测量误差问题，特别是DRL方法对输入扰动敏感，可能导致次优决策。

Method: 建立包含状态不确定性的多时间尺度决策模型，提出两种鲁棒方案：1) PGD-DDQN算法在范数有界误差模型下推导最坏情况扰动，训练时应用投影梯度下降进行鲁棒优化；2) NQC-DDQN算法引入非线性压缩机制自适应压缩Q值范围以消除动作混叠。

Result: 仿真结果表明，与完美感知基线相比，所提算法在抗干扰性能上仅有轻微下降，在各种扰动下保持鲁棒性，验证了其在非完美感知条件下的实用性。

Conclusion: 所提出的多时间尺度鲁棒抗干扰算法能够有效应对实际通信系统中的感知误差问题，为无线通信在干扰环境下的可靠传输提供了实用解决方案。
...
TL;DR: 本文构造了多个无限族的q元常循环码，这些码具有长度n、维度约n/2、最小距离至少为cn/log_q n的优良特性，包含许多参数最优或接近最优的常循环码。


<details>
  <summary>Details</summary>
Motivation: 研究具有良好参数特性的常循环码，特别是那些维度接近长度一半且最小距离较大的码，这类码在编码理论中具有重要应用价值。

Method: 通过构造多个无限族的q元常循环码，考虑不同形式的长度n，确保码的维度约为n/2且最小距离满足cn/log_q n的下界。

Result: 成功构造了包含许多最优、接近最优或已知最佳参数的常循环码的无限族，证明了这些码的存在性和参数特性。

Conclusion: 本文提供了一种系统构造具有优良参数特性的常循环码的方法，丰富了编码理论中的码构造技术。
...
TL;DR: 本文研究了(L,P)-扭曲广义Reed-Solomon码的NMDS条件、非RS性质以及自对偶/自正交性质，部分解决了Hu等人2025年提出的两个开放问题。
...
Method: 首先给出(L,P)-TGRS码的校验矩阵，然后分析其NMDS条件、非RS性质，以及自对偶/自正交条件，并通过构造实例验证。

Result: 给出了C是NMDS码的充要条件，证明了当2k>n时C是非RS码，给出了C不自对偶/不自正交的充分条件，并构造了两类自正交码。

Conclusion: 本文部分解决了Hu等人2025年提出的两个开放问题，改进了Hu和Ding等人的相关结果，为(L,P)-TGRS码的理论研究提供了重要进展。
...
TL;DR: 本文建立了流体天线系统(FAS)在空间相关信道中的符号错误率(SER)基本缩放定律，揭示了SER与信道空间相关结构之间的根本关系。


<details>
  <summary>Details</summary>
Motivation: 流体天线系统通过利用空间分集来增强无线通信，但此前缺乏对其错误概率的严格分析框架。

Method: 推导了适用于通用调制方案的紧致闭式渐近SER表达式，基于此框架完整表征了分集增益和编码增益。

Result: 建立了SER与信道空间相关结构的基本缩放定律，发现扩大天线移动空间可提高分集增益，而在受限空间内仅增加端口密度收益递减。

Conclusion: SER的根本改进需要通过扩大天线移动空间来增加分集，而仅在受限空间内增加端口密度收效甚微。
...
TL;DR: 提出了一种基于多普勒感知的稀疏神经网络波束成形模型，在MU-SIMO系统中通过通道自适应的稀疏注意力机制，在高移动性场景下显著优于传统波束成形方法。


<details>
  <summary>Details</summary>
Motivation: 传统波束成形方法在恶劣信道条件下性能下降，而现有的稀疏注意力机制未充分考虑无线通信场景中的信道动态特性。

Method: 设计了一种通道自适应的稀疏注意力机制，在2D时频轴上可配置稀疏结构，理论证明在p个注意力头内确保完全连接性。

Result: 在UMa信道条件下，Doppler-aware Sparse NNBF在高移动性场景下显著优于标准稀疏NNBF以及传统ZFBF和MMSE波束成形方法。

Conclusion: 所提出的多普勒感知稀疏神经网络波束成形模型能够有效处理高移动性场景，同时保持结构化稀疏性和可控的计算复杂度。
